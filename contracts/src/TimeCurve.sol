...
contract TimeCurve is Initializable, OwnableUpgradeable, ReentrancyGuard, UUPSUpgradeable {
    ...
    /// @dev GitLab #133 — `PodiumPool` had **zero** balance at settlement; `prizesDistributed` latches (**no refill/retry`).
    ...
    ///      GitLab #133: **zero** `prizePool` still requires **`reservePodiumPayoutsEnabled`**; emits
    ///      `PrizesSettledEmptyPodiumPool` and sets `prizesDistributed` (**no** later refill + retry). Non-zero path
    ///      emits `PrizesDistributed` only after full drain.
