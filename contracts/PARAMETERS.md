...
| `doubOwedForBuy(charmWad)` | **`view`** — staticcall-safe gross DOUB preview ([#315](https://gitlab.com/PlasticDigits/yieldomega/-/issues/315)) | Matches immediate `buy` pull; in hard-reset band samples TWAP/spot anchor (same as `_reanchorEpochCharmPrice`) instead of `effectiveCharmPriceWad()` |
