...
#   export RESERVE_ASSET_ADDRESS=0x... TIMECURVE_ADDRESS=0x... RABBIT_TREASURY_ADDRESS=0x...
...
export RESERVE_ASSET_ADDRESS USDM_ADDRESS TIMECURVE_ADDRESS RABBIT_TREASURY_ADDRESS
...
if [[ "${ANVIL_RICH_END_SALE_ONLY:-}" == "1" ]]; then
  ...
  forge script script/SimulateAnvilRichState.s.sol:SimulateAnvilRichStatePart2EndSaleOnly \
    --rpc-url "$RPC" --broadcast --slow --code-size-limit 524288 -vv
  echo "ANVIL_RICH_END_SALE_ONLY: skipping Part2 full simulation + finalizeEpoch — re-deploy or re-run without this flag for indexer events (legacy TimeCurve post-end verify removed; Arena v2 #324)."
  echo "Done."
  exit 0
fi
