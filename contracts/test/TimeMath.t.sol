...
contract TimeMathTest is Test {
    ...
    // ln(1.25) ≈ 0.22314355... in WAD
    uint256 internal constant GROWTH_RATE_25PCT = 223_143_551_314_209_700;
    uint256 internal constant ONE_DAY = 86_400;

    function test_minBuy_zero_elapsed() public pure {
        uint256 mb = TimeMath.currentMinBuy(1e18, GROWTH_RATE_25PCT, 0);
        assertEq(mb, 1e18);
    }

    function test_minBuy_one_day_approx_125pct() public pure {
        uint256 mb = TimeMath.currentMinBuy(1e18, GROWTH_RATE_25PCT, ONE_DAY);
        // After 1 day should be ~1.25e18 (within 0.01% tolerance)
        assertApproxEqRel(mb, 1.25e18, 1e14);
    }

    function test_minBuy_two_days() public pure {
        uint256 mb = TimeMath.currentMinBuy(1e18, GROWTH_RATE_25PCT, 2 * ONE_DAY);
        // 1.25^2 = 1.5625
        assertApproxEqRel(mb, 1.5625e18, 1e14);
    }
    ...
    function test_minBuy_monotonic_fuzz(uint32 t1, uint32 t2) public pure {
        ...
        uint256 mb1 = TimeMath.currentMinBuy(1e18, GROWTH_RATE_25PCT, elapsed1);
        uint256 mb2 = TimeMath.currentMinBuy(1e18, GROWTH_RATE_25PCT, elapsed2);
