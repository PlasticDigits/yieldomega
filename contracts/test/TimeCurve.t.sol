...
contract TimeCurveTest is Test {
    ...
    /// @dev GitLab #133 — empty vault finalizes with `PrizesSettledEmptyPodiumPool` and locks `prizesDistributed`.
    ...
    /// @dev GitLab #133 — after empty settlement, funding `PodiumPool` cannot reopen distribution.
    ...
    /// @dev GitLab #133 — empty pool respects `reservePodiumPayoutsEnabled` like the non-zero path.
