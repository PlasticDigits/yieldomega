...
import { PODIUM_CONTRACT_CATEGORY_INDEX, PODIUM_LABELS } from "@/pages/arena/podiumCopy";
...
const PLAY_SKILLS_SIMPLE: { label: string; path: string }[] = [
  ...
  { label: "script-with-timearena-local (RPC vs indexer time)", path: "skills/script-with-timearena-local/SKILL.md" },
];
...
def head_addr():
    if base:
        url = base + "/v1/arena/warbow/leaderboard?limit=1"
...
def last_buy_age():
    if not base:
        return None
    url = base + "/v1/arena/buys?limit=1"
    with urllib.request.urlopen(url, timeout=15) as r:
        j = json.loads(r.read().decode())
    items = j.get("items") or []
    if not items:
 ...
 */
export function ArenaSimpleAgentCard() {
  ...
  return (
    <details
      className="app-footer-agent arena-simple-agent-card data-panel"
      data-testid="arena-simple-agent-card"
    >
