...
function tickerCardTheme(buy: BuyItem, band: ReturnType<typeof buyBandPosition>) {
  ...
  if (flagPenalty > 0n || buy.flag_planted === true) {
    return {
      className: "arena-simple__ticker-row--flag",
      badge: "WarBow flag",
  ...
  if (buy.timer_hard_reset) {
    return {
      className: "arena-simple__ticker-row--reset",
      badge: "Clock reset",
  ...
  if (activeStreak >= 2n) {
    return {
      className: "arena-simple__ticker-row--streak",
      badge: "Streak",
  ...
  if (bandFill >= 75) {
    return {
      className: "arena-simple__ticker-row--max",
      badge: "Big bite",
  ...
  if (bandFill >= 35) {
    return {
      className: "arena-simple__ticker-row--mid",
      badge: "Momentum",
      artSrc: "/art/hat-coin-front.png",
    };
  }
  return {
    className: "arena-simple__ticker-row--min",
    badge: "Fresh buy",
...
export function ArenaLiveBuysActivitySection({
  dataTestId = "arena-live-buys-activity",
  recentBuys,
  decimals,
        ...
        <>
          ...
          >
              ...
              {recentBuys.map((b) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className="arena-simple__ticker-band"
                      title={band?.title ?? "Min/max band position unavailable for this indexed buy"}
                    >
                      <div className="arena-simple__ticker-band-head">
                      ...
                      <div className="arena-simple__ticker-band-track" aria-hidden="true">
                        <span className="arena-simple__ticker-band-min-marker" />
                        <span
                          className={
                            bandFillPercent !== null
                              ? "arena-simple__ticker-band-fill"
                              : "arena-simple__ticker-band-fill arena-simple__ticker-band-fill--unknown"
                          }
                        />
                      </div>
                      <div className="arena-simple__ticker-band-foot">
                        {band
                          ? `${band.sizeLabel} · ${Math.round(band.positionPercent)}% through min-max`
                          : "Waiting for band math"}
                      </div>
                    </div>
                    <ul className="arena-simple__ticker-effects" aria-label="Buy effects">
                      {ticks.map((tick) => {
            ...
            {buysNextOffset !== null ? (
              ...
              <p className="muted arena-simple__activity-load-more">End of loaded history</p>
            ) : null}
          </div>
        </>
