...
/** Slices match `TimeCurve.distributePrizes`: [last buy, time booster, defended streak]. */
export function podiumCategorySlices(pool: bigint): [bigint, bigint, bigint] {
  if (pool <= 0n) {
    return [0n, 0n, 0n];
  }
  const last = (pool * 50n) / 100n;
  const timeBooster = (pool * 25n) / 100n;
  const defended = pool - last - timeBooster;
  return [last, timeBooster, defended];
}
