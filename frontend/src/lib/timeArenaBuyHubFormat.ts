 ...
 *
 * **Not** used for: hero “price now / at launch” tiles ({@link ARENA_HERO_RATE_SIGFIGS}
 * sigfigs via {@link formatHeroRateFromWad} (truncated, trailing-zero padded) — see `ArenaSimplePage` /
 ...
 *
 * Returns a label like `+27.50% GAIN` using {@link ARENA_BUY_HUB_DERIVED_SIGFIGS}. When spend and
...
export function formatBuyHubLaunchVsClearingGainPercentLabel(input: {
  ...
  const compact = formatPercentMagnitudeSigfigs(trimmed, ARENA_BUY_HUB_DERIVED_SIGFIGS);
  const neg = trimmed.startsWith("-");
