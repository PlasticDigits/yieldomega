 ...
 */
export function EmptyDataPlaceholder({ children, className, role = "status", ...rest }: Props) {
  return (
    <span className={["empty-data-placeholder", className].filter(Boolean).join(" ")} role={role} {...rest}>
      {children}
    </span>
  );
}
