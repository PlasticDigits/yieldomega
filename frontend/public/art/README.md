...
- **Generation:** see [`scripts/replicate-art/`](../../../scripts/replicate-art/) and the issue-scoped batch driver [`scripts/replicate-art/issue45_batch.py`](../../../scripts/replicate-art/issue45_batch.py). Drops land in `pending_manual_review/` (gitignored) and only get promoted into a purpose folder after maintainer review.
...
| [`referrals-network.jpg`](./scenes/referrals-network.jpg) | [`ReferralsPage.tsx`](../../src/pages/ReferralsPage.tsx) | Network/threads motif for the Referrals placeholder.                        |
...
| `status-prelanch.png`             | [`PageBadge`](../../src/components/ui/PageBadge.tsx) tone `soon`           | Phase badge (Pre-launch). **Note** the source filename keeps the issue #45 typo (`prelanch`); CSS aliases it. |
...
| Slug                          | Wired in                                              |
|-------------------------------|-------------------------------------------------------|
| `og-wide.jpg`                 | `vite.config.ts` `imagePath` for Open Graph.         |
| `og-square.jpg`               | `index.html` Twitter `summary` card.                 |
...
| `opengraph.jpg` / `opengraph-src.png` | `vite.config.ts` `imagePath`.                                                        |
...
1. Generate the image via `scripts/replicate-art/issue45_batch.py` (or a follow-up batch that mirrors its argparse / README contract).
...
- **Generation pipeline:** [`scripts/replicate-art/issue45_batch.py`](../../../scripts/replicate-art/issue45_batch.py)
