...
pick_indexer_port() {
  ...
  for try in $(seq 0 15); do
    ...
    if ! yieldomega_tcp_port_listening "${cand}"; then
      echo "${cand}"
      return 0
    fi
  done
...
for var_name in TA DOUB PV RR; do
...
echo "=== Merge ${FRONTEND}/.env.local (stack VITE_* only) ==="
yieldomega_frontend_merge_anvil_stack_env \
  "${FRONTEND}/.env.local" "${RPC_URL}" "${TA}" "${PV}" "${RR}" "${INDEXER_PORT}"
...
if [[ -n "${LAUNCH_OFFSET_SEC:-}" ]]; then
  ...
  _yieldomega_env_set_line "${FRONTEND}/.env.local" "VITE_LAUNCH_TIMESTAMP" "${LAUNCH_TS}" "${_yieldomega_env_anvil_stack_marker}"
