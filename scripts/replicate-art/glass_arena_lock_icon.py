...
"""Regenerate the Glass Arena lock icon.

Output replaces the stable public asset path used by ``LockedUntilLevel``:

  frontend/public/art/icons/arena-lock-level.png

Prediction ID is written to ``glass_arena_lock_icon.ledger.json`` so reconnects
or retries do not duplicate Replicate jobs.
"""
...
LEDGER = SCRIPT_DIR / "glass_arena_lock_icon.ledger.json"
ART_ROOT = ga.REPO_ROOT / "frontend" / "public" / "art"
ICONS_DIR = ART_ROOT / "icons"
SCRATCH_DIR = ART_ROOT / "pending_manual_review" / "glass-arena-lock-icon"
FILENAME = "arena-lock-level.png"
LEDGER_KEY = "arena-lock-level"

LOCK_PROMPT = """
Yield Omega Glass Arena lock icon.
Create a single square 256x256 transparent PNG icon for a locked gameplay
mechanic overlay. Subject: a premium glass padlock with a chunky gold shackle,
deep navy translucent body, emerald/teal live-state edge glow, warm gold DOUB
reward accents, and a tiny purple opponent-energy glint used sparingly. The
shape must read clearly at 44px and 64px. Cyberminimalist command-console
finish: crisp silhouette, luminous bevels, subtle glass reflections, no text,
no numbers, no UI labels, no watermarks, no photorealism. Center the lock with
transparent padding.
""".strip()
...
def _reference_paths() -> list[Path]:
    refs = [
        ...
        ga.REPO_ROOT / "frontend" / "public" / "art" / "icons" / "arena-podium-rank-first.png",
    ]
...
def main() -> int:
    ...
    inp: dict = {
        "prompt": ga.augment_prompt_chroma_backdrop(LOCK_PROMPT),
        "aspect_ratio": "1:1",
        "quality": "high",
        "background": "opaque",
        "moderation": "low",
