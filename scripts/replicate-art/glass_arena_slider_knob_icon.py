...
"""Regenerate the Glass Arena buy-spend range slider thumb.

Output replaces the stable public asset path used by ``ArenaBuySpendRangeInput``:

  frontend/public/art/icons/icon-slider-knob.png

Prediction ID is written to ``glass_arena_slider_knob_icon.ledger.json`` so
reconnects do not duplicate Replicate jobs. One create attempt per invocation;
...
LEDGER = SCRIPT_DIR / "glass_arena_slider_knob_icon.ledger.json"
ART_ROOT = ga.REPO_ROOT / "frontend" / "public" / "art"
ICONS_DIR = ART_ROOT / "icons"
SCRATCH_DIR = ART_ROOT / "pending_manual_review" / "glass-arena-slider-knob-icon"
FILENAME = "icon-slider-knob.png"
LEDGER_KEY = "icon-slider-knob"

SLIDER_KNOB_PROMPT = """
Yield Omega Glass Arena range-slider thumb knob.
Create a single square 256x256 transparent PNG circular drag handle for an HTML
range input (32 px display on a teal glass track). Subject: a frosted deep-navy
glass disc with a thin luminous emerald/teal rim ring, subtle warm gold DOUB
accent highlights on the upper-left bevel, and a minimal horizontal grip
affordance — three short etched glass dashes or one thin frosted bar across the
center (drag hint only, not a cartoon dumbbell or mascot shape).

Cyberminimalist glassmorphism command-console finish: semi-transparent frosted
body, soft internal glow, crisp circular silhouette, delicate edge lighting, no
thick black outlines, no arcade cartoon rings, no solid plastic yellow/green
layers, no skeuomorphic 3D button, no characters, no text, no numerals, no UI
frame, no watermark, no photorealism. Center the round knob with generous
transparent padding.
""".strip()
...
def _reference_paths() -> list[Path]:
    refs = [
        ...
        ICONS_DIR / "header-wallet-connect.png",
        ICONS_DIR / "arena-lock-level.png",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--no-refs", action="store_true", help="Skip Glass Arena reference images")
    ...
    inp: dict = {
        "prompt": ga.augment_prompt_chroma_backdrop(SLIDER_KNOB_PROMPT),
        "aspect_ratio": "1:1",
        "quality": "high",
        "background": "opaque",
        "moderation": "low",
