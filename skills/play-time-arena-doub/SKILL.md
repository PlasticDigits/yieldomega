...
- `buy(charmWad)` or `buy(charmWad, codeHash)` pulls **DOUB** = `charmWad × charmPriceWad / 1e18`. **Production (4326):** initial **`charmPriceWad`** from **Kumbaya V3 TWAP** (~**$1** DOUB notional per 1 CHARM — [#303](https://gitlab.com/PlasticDigits/yieldomega/-/issues/303) · **`INV-TIME-ARENA-CHARM-TWAP-INIT`** · `bash scripts/compute-arena-charm-price-twap.sh`). **Anvil / DeployDev:** **`1000e18`** (1000 DOUB per 1 CHARM). Implied spend band: **`0.99×`–`10× charmPriceWad`**. Invariants: [**§246**](../../docs/testing/invariants-and-business-logic.md#timearena-core-gitlab-246) · [**§303**](../../docs/testing/invariants-and-business-logic.md#arena-charm-twap-init-gitlab-303).
