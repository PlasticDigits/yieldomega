...
- `buy(charmWad)` or `buy(charmWad, codeHash)` pulls **DOUB** = `charmWad × effectiveCharmPriceWad() / 1e18` (after any hard-reset re-anchor on that buy). **Integrator / router preview:** **`doubOwedForBuy(charmWad)`** (`view`, `eth_call`-safe) — matches immediate buy DOUB including at the hard-reset boundary ([#315](https://gitlab.com/PlasticDigits/yieldomega/-/issues/315) · **`INV-TIME-ARENA-DOUB-OWED-PREVIEW`**). **Production (4326):** each Last Buy epoch **re-anchors** from **Kumbaya V3 TWAP**, then grows **+10%/day** until the next hard reset ([#305](https://gitlab.com/PlasticDigits/yieldomega/-/issues/305) · **`INV-TIME-ARENA-EPOCH-CHARM-GROWTH`**). **Epoch-0 anchor** from TWAP (~**$1** DOUB notional per 1 CHARM — [#303](https://gitlab.com/PlasticDigits/yieldomega/-/issues/303) · **`INV-TIME-ARENA-CHARM-TWAP-INIT`** · `bash scripts/compute-arena-charm-price-twap.sh`). **Anvil / DeployDev:** **`1000e18`** anchor baseline. Implied spend band: **`0.99×`–`10× effectiveCharmPriceWad()`**. Invariants: [**§246**](../../docs/testing/invariants-and-business-logic.md#timearena-core-gitlab-246) · [**§305**](../../docs/testing/invariants-and-business-logic.md#timearena-epoch-charm-price-gitlab-305) · [**§303**](../../docs/testing/invariants-and-business-logic.md#arena-charm-twap-init-gitlab-303) · [**§315**](../../docs/testing/invariants-and-business-logic.md#timearena-doub-owed-preview-gitlab-315).
- **`buyWithCred(charmWad)`** burns Play CRED (no approve); **`/`** play UI ([#269](https://gitlab.com/PlasticDigits/yieldomega/-/issues/269)) — [`arenaCredBurn.ts`](../../frontend/src/lib/arenaCredBurn.ts). Onchain burn: **100 CRED / 1e18 CHARM** ([#268](https://gitlab.com/PlasticDigits/yieldomega/-/issues/268) · [`INV-TIME-ARENA-CRED-BURN-BUY`](../../docs/testing/invariants-and-business-logic.md#timearena-cred-buy-gitlab-268)); verify: `bash scripts/verify-cred-buy-anvil.sh`.
...
- Each buy routes **100%** of DOUB to four podium tracks: **25%** per category (`amount / 4`; remainder wei → **Last Buy** cat 0), split **70% / 20% / 10%** across current and next two epochs ([#300](https://gitlab.com/PlasticDigits/yieldomega/-/issues/300) · [#313](https://gitlab.com/PlasticDigits/yieldomega/-/issues/313) · [`ArenaBuyRouting`](../../contracts/src/arena/libraries/ArenaBuyRouting.sol) · **`INV-ARENA-PRIZE-ROUTING-300-*`** · [invariants §300](../../docs/testing/invariants-and-business-logic.md#arena-prize-routing-gitlab-300)). **0%** admin take on buys ([#314](https://gitlab.com/PlasticDigits/yieldomega/-/issues/314) retires **`AdminSellVault`** from deploy).
...
- **First buy ever** (this wallet): schedules **`FIRST_BUY_CRED_BONUS = 1100e18`** for the **next** `lastBuyEpoch` (DOUB or CRED path); one-time — [#268](https://gitlab.com/PlasticDigits/yieldomega/-/issues/268), [#299](https://gitlab.com/PlasticDigits/yieldomega/-/issues/299) · [`INV-TIME-ARENA-FIRST-BUY-CRED-BONUS`](../../docs/testing/invariants-and-business-logic.md#timearena-cred-buy-gitlab-268) · [manual QA §268](../../docs/testing/manual-qa-checklists.md#manual-qa-issue-268).
...
- **Post-buy UX:** successful buys on **`/`** show glass effect toasts (timer, XP, level, WarBow) via **`ArenaEffectToastStack`** — [#337](https://gitlab.com/PlasticDigits/yieldomega/-/issues/337) · **`INV-FRONTEND-337-BUY-EFFECT-TOASTS`** · [arena-views §337](../../docs/frontend/arena-views.md#post-buy-effect-toasts-gitlab-337).
...
- Each **DOUB or CRED** buy adds **35 CRED** to `epochCredPool[lastBuyEpoch]`; pro-rata by `epochCharmWad[epoch][user] / epochCharmTotal[epoch]`.
...
On **`/`** and **`/arena/protocol`**, click any participant wallet on live buy rows or podium rankings to open **`WalletProfileModal`** — stats from **`GET /v1/arena/wallet/{address}/stats`** (Overview, Podium wins, Spending, XP/Level, **Level history** ([#336](https://gitlab.com/PlasticDigits/yieldomega/-/issues/336)), WarBow, Referrals, Fun facts). Malformed wallet paths (`0x` + 42 chars but non-hex) return **400** `invalid_address` ([#329](https://gitlab.com/PlasticDigits/yieldomega/-/issues/329)). The “Just +Ns by …” timer chip needs **`actual_seconds_added`** on **`GET /v1/arena/buys`** ([#282](https://gitlab.com/PlasticDigits/yieldomega/-/issues/282)); buy detail and stable list keys need **`new_deadline`**, **`buy_index`**, **`log_index`**, **`block_timestamp`** ([#283](https://gitlab.com/PlasticDigits/yieldomega/-/issues/283)): `bash scripts/verify-wallet-profile-anvil.sh`. Invariants: [**`INV-FRONTEND-258-WALLET-PROFILE`**](../../docs/testing/invariants-and-business-logic.md#wallet-profile-modal-gitlab-258), [**`INV-INDEXER-336-WALLET-LEVEL-HISTORY`**](../../docs/testing/invariants-and-business-logic.md#wallet-profile-modal-gitlab-258), [**`INV-INDEXER-282-ARENA-BUYS-SECONDS`**](../../docs/testing/invariants-and-business-logic.md#wallet-profile-modal-gitlab-258), [**`INV-INDEXER-283-ARENA-BUYS-PARITY`**](../../docs/testing/invariants-and-business-logic.md#wallet-profile-modal-gitlab-258), [**`INV-INDEXER-329-ADDRESS-VALIDATE`**](../../docs/testing/invariants-and-business-logic.md#wallet-address-validation-and-uups-ownable2step-gitlab-329) · [arena-views § wallet-profile](../../docs/frontend/arena-views.md#wallet-profile-modal-gitlab-258).
