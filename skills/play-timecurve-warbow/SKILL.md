...
4. **Indexer / frontend** — discovery and history; **do not** override chain for eligibility or balances. The Arena hero's suggested steal targets (issue #101) are convenience rows from `warbowLadderPodium()` plus the indexed leaderboard; selecting one should still be verified against live `battlePoints` and `stealsReceivedOnDay`. On **`/timecurve/arena`**, indexed **leaderboard rank**, **chasing-pack** (**full** ladder list with scroll — [GitLab #189](https://gitlab.com/PlasticDigits/yieldomega/-/issues/189), [`INV-FRONTEND-189-WARBOW-CHASING-PACK`](../../docs/testing/invariants-and-business-logic.md#timecurve-arena-warbow-chasing-pack-gitlab-189)), and **rivalry / battle feed** refresh after your txs complete **and** on a short poll while the page is open ([GitLab #182](https://gitlab.com/PlasticDigits/yieldomega/-/issues/182), [`INV-FRONTEND-182-WARBOW-IDX`](../../docs/testing/invariants-and-business-logic.md#timecurve-arena-warbow-indexer-refresh-gitlab-182), [`timecurve-views §182`](../../docs/frontend/timecurve-views.md#arena-warbow-indexer-leaderboard-feed-refresh-gitlab-182)) — still treat **live RPC** as authoritative for steal/guard/revenge gates.
...
- **Web wallet — between timer end and `endSale` ([GitLab #188](https://gitlab.com/PlasticDigits/yieldomega/-/issues/188)):** On **`/timecurve/arena`**, **`saleExpiredAwaitingEnd`** shows **End sale** plus **Redeem charms** / **Distribute prizes** (disabled until **`TimeCurve.ended()`** is **`true`**) so the permissionless settle path is visible next to podiums — [**`INV-FRONTEND-188-ARENA-SETTLEMENT`**](../../docs/testing/invariants-and-business-logic.md#timecurve-arena-settlement-panel-timer-expired-gitlab-188), [timecurve-views §188](../../docs/frontend/timecurve-views.md#arena-settlement-panel-timer-expired-gitlab-188). **`redeemCharms`** / **`distributePrizes`** still **require** **`ended`** onchain.
...
## Governance podium finalize ([GitLab #129](https://gitlab.com/PlasticDigits/yieldomega/-/issues/129), [**#172**](https://gitlab.com/PlasticDigits/yieldomega/-/issues/172))

The **`warbowLadderPodium()`** triple reflects **governance-finalized** ranks after **`endSale`**; during the sale it can still lag live **`battlePoints`** until steals / buys settle — there is **no** permissionless onchain “refresh heap” repair anymore (**#172**).

- **`finalizeWarbowPodium(first, second, third)`** — **`onlyOwner`**, **`ended`**, **`!prizesDistributed`**; validates **left-packed** **`address(0)`** suffixes and **strict** live **`battlePoints`** ordering along occupied ranks; writes **`_warbowPodium`**, sets **`warbowPodiumFinalized = true`**, emits **`WarbowPodiumFinalized`**. Not required on the **zero `PodiumPool` balance** **`distributePrizes`** path ([GitLab #133](https://gitlab.com/PlasticDigits/yieldomega/-/issues/133)).
- **Protocol / indexer-assisted reference set ([GitLab #160](https://gitlab.com/PlasticDigits/yieldomega/-/issues/160), [#170](https://gitlab.com/PlasticDigits/yieldomega/-/issues/170), [#172](https://gitlab.com/PlasticDigits/yieldomega/-/issues/172)):** **`GET /v1/timecurve/warbow/refresh-candidates`** returns a deduped wallet list (unbounded DISTINCT from schema **≥ 1.18.0**) merged with head WarBow podium hints **only while `sale_ended` is false** in the API. The shipped **`/timecurve/protocol`** panel loads it as **operator reference** while the owner submits **`finalizeWarbowPodium`** post-end. **UI paging guard ([GitLab #174](https://gitlab.com/PlasticDigits/yieldomega/-/issues/174)):** if the client hits its max page count while **`next_offset`** stays set, the panel shows a **non-fatal warning** — [`INV-FRONTEND-174-WARBOW-REFRESH-GUARD`](../../docs/testing/invariants-and-business-logic.md#gitlab-174-warbow-refresh-pagination-guard).

Contributor map — [`INV-WARBOW-172-*`](../../docs/testing/invariants-and-business-logic.md#gitlab-149-warbow-arena-indexer-hardening) · [`INV-WARBOW-129-DIST`](../../docs/testing/invariants-and-business-logic.md#warbow-podium-snapshot-drifts-gitlab-129) · UX — [`timecurve-views #129 / #172`](../../docs/frontend/timecurve-views.md#warbow-ladder-podium-snapshot-mismatch-issue-129).
