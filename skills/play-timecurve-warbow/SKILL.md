...
- **Protocol / indexer-assisted candidates ([GitLab #160](https://gitlab.com/PlasticDigits/yieldomega/-/issues/160)):** **`GET /v1/timecurve/warbow/refresh-candidates`** returns a deduped wallet list for **`refreshWarbowPodium`** calldata (indexer read-model + head WarBow podium hint). The shipped **`/timecurve/protocol`** panel can fetch it before submit. **Onchain rule unchanged:** **`refreshWarbowPodium`** reverts after **`endSale`** — use owner **`finalizeWarbowPodium`** post-end ([GitLab #149](https://gitlab.com/PlasticDigits/yieldomega/-/issues/149)).
