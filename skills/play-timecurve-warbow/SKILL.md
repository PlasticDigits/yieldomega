...
4. **Indexer / frontend** — discovery and history; **do not** override chain for eligibility or balances. The Arena hero's suggested steal targets (issue #101) are convenience rows from `warbowLadderPodium()` plus the indexed leaderboard; selecting one should still be verified against live `battlePoints` and `stealsReceivedOnDay`. On **`/timecurve/arena`**, indexed **leaderboard rank**, **chasing-pack hints**, and **rivalry / battle feed** refresh after your txs complete **and** on a short poll while the page is open ([GitLab #182](https://gitlab.com/PlasticDigits/yieldomega/-/issues/182), [`INV-FRONTEND-182-WARBOW-IDX`](../../docs/testing/invariants-and-business-logic.md#timecurve-arena-warbow-indexer-refresh-gitlab-182), [`timecurve-views §182`](../../docs/frontend/timecurve-views.md#arena-warbow-indexer-leaderboard-feed-refresh-gitlab-182)) — still treat **live RPC** as authoritative for steal/guard/revenge gates.
