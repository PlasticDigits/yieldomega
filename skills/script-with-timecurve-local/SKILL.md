...
If you **are** editing this repo and need **Postgres + Anvil + indexer + Vite** in one flow, use **[`docs/testing/qa-local-full-stack.md`](../../docs/testing/qa-local-full-stack.md)** and [`scripts/start-qa-local-full-stack.sh`](../../scripts/start-qa-local-full-stack.sh) ([GitLab #104](https://gitlab.com/PlasticDigits/yieldomega/-/issues/104)). **`--help`** prints **banner comments only** — no stray **`set …`** shell lines ([GitLab #105](https://gitlab.com/PlasticDigits/yieldomega/-/issues/105)). **Vite:** traps + **`curl --max-time`** during readiness prevent orphaned dev servers on Ctrl+C or abort ([GitLab #153](https://gitlab.com/PlasticDigits/yieldomega/-/issues/153), [`scripts/lib/qa_local_full_stack_frontend.sh`](../../scripts/lib/qa_local_full_stack_frontend.sh)). **Participants** automating against a **remote** RPC do not need that path.
