...
- **Protocol revenue (`receiveFee`)** — Documented deploy defaults target about **25% burn / 75%** to protocol-owned backing (tunable toward **20–40% / 60–80%**); **verify** live parameters. Ingress uses **balance-delta** accounting on canonical ERC20 ([GitLab #123](https://gitlab.com/PlasticDigits/yieldomega/-/issues/123)).
- **Epochs** — Operations depend on **open epochs** and **finalization** rules; read events (`Burrow*` and related) per rabbit-treasury doc for indexer-visible metrics. On-chain **`epochId`** increments are **rolling accounting windows** (snapshots, repricing, cooldown indexing), not a separate "next product epoch" roadmap stage ([`rabbit-treasury.md` § epochId](../../docs/product/rabbit-treasury.md#epochid-is-accounting-windows-not-a-multi-stage-product-roadmap)); governed Burrow knobs stay inside documented WAD envelopes ([GitLab #119](https://gitlab.com/PlasticDigits/yieldomega/-/issues/119), [`invariants §119`](../../docs/testing/invariants-and-business-logic.md#rabbit-treasury-burrow-param-bounds-gitlab-119)).
- **Fees** — Rabbit Treasury may receive a **share** of ecosystem fees per fee routing; verify live configuration. If governance routes the fifth **`FeeRouter`** sink through **`RabbitTreasuryVault`** ([GitLab #159](https://gitlab.com/PlasticDigits/yieldomega/-/issues/159), [`docs/onchain/treasury-contracts.md`](../../docs/onchain/treasury-contracts.md#rabbit-treasury-vault-gitlab-159)), **vault ERC-20 balances are not `protocolOwnedBacking`** until operators sweep into **`RabbitTreasury.receiveFee`** or repoint **`FeeRouter`** at the **`RabbitTreasury`** proxy.
