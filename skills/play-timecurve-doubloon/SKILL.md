...
2. **Relevant events** — e.g. **`SaleStarted(startTimestamp=epoch, …)`** (owner **`startSaleAt(epoch)`** at sale open — [GitLab #114](https://gitlab.com/PlasticDigits/yieldomega/-/issues/114), [`docs/testing/invariants-and-business-logic.md` §114](../../docs/testing/invariants-and-business-logic.md#timecurve-startsaleat-issue-114)), `Buy`, `SaleEnded`, `CharmsRedeemed`, **`PrizesDistributed`** (vault drained successfully), **`PrizesSettledEmptyPodiumPool`** (zero vault at settlement — [GitLab #133](https://gitlab.com/PlasticDigits/yieldomega/-/issues/133), [`§133`](../../docs/testing/invariants-and-business-logic.md#timecurve-prizes-empty-podium-pool-issue-133)), `ReferralApplied`, `PodiumPaid` as emitted per [`docs/product/primitives.md`](../../docs/product/primitives.md) and deployment.
