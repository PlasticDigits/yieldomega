...
- **Unredeemed DOUB in `TimeCurve`:** After **`saleEndedAt + 7 days`**, **`onlyOwner` `sweepUnredeemedLaunchedToken`** may send **all** remaining **`launchedToken`** to **`unredeemedLaunchedTokenRecipient`** (governance routing); participants who have not called **`redeemCharms`** before then forfeit that remainder onchain — [GitLab #128](https://gitlab.com/PlasticDigits/yieldomega/-/issues/128), [primitives §128](../../docs/product/primitives.md#unredeemed-launch-allocation-sweep-gitlab-128), [invariants §128](../../docs/testing/invariants-and-business-logic.md#timecurve-unredeemed-launch-allocation-sweep-gitlab-128).
