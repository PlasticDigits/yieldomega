...
- **Presale CHARM +15% weight boost ([GitLab #202](https://gitlab.com/PlasticDigits/yieldomega/-/issues/202)):** Some deployments add **+15%** of the purchased CHARM amount to **`charmWeight` / `totalCharmWeight`** for buyers where **`isBeneficiary(buyer)`** is true on the contract at **`TimeCurve.doubPresaleVesting()`** (historical name — may be **`DoubPresaleVesting`** or a dedicated **`PresaleCharmBeneficiaryRegistry`** when vesting rows and boost lists differ). **Gross spend in the reserve asset is unchanged.** The global header may show **Presale +15% CHARM** on TimeCurve routes when env resolves a matching contract. **`/vesting`** remains **DOUB claims** on the vesting proxy only — [`INV-TC-PRESALE-CHARM-BOOST`](../docs/testing/invariants-and-business-logic.md#timecurve-presale-charm-weight-boost) · [`timecurve-views` — presale header](../docs/frontend/timecurve-views.md#timecurve-presale-charm-header-hint); players should verify onchain **`isBeneficiary`** and the deployment runbook.
