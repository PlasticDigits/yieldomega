...
- **Go-live gates (issue #55):** Onchain `onlyOwner` flags may block **sale `buy` fee routing**, **`redeemCharms`**, and **non-zero `distributePrizes`** until operators enable them. Read `buyFeeRoutingEnabled`, `charmRedemptionEnabled`, and `reservePodiumPayoutsEnabled` (and `claimsEnabled` on presale vesting) on the deployment — [final signoff runbook](../../docs/operations/final-signoff-and-value-movement.md). **Dev/local scripts** may turn these on for E2E; mainnet order is a human runbook concern.
