...
- **CHARM min/max band** (exponential envelope, canonical **~20%/day** unless deployment differs) rises over time, separately from **per-CHARM price** (default linear-in-time DOUB module); each buy has **min/max** CHARM bounds; **timer** extends on buys up to a **cap**; **`initialTimerSec`** may be shorter than the cap so early activity can still grow remaining time (see deployed parameters). **300-day sale wall ([GitLab #124](https://gitlab.com/PlasticDigits/yieldomega/-/issues/124)):** onchain **`buy` / `buyFor`** and WarBow **CL8Y** paths stop when **`block.timestamp > saleStart + MAX_SALE_ELAPSED_SEC`**; the check is **`<=`** at the boundary. **Round countdown `deadline()` ([GitLab #136](https://gitlab.com/PlasticDigits/yieldomega/-/issues/136)):** buys and **`TimeCurveBuyRouter`** paths stay **`<= deadline()`** through the inclusive **last countdown second**; **`endSale`** succeeds only **`> deadline()`**. Docs: [`invariants §124`](../docs/testing/invariants-and-business-logic.md#timecurve-max-sale-elapsed-gitlab-124) · [`invariants §136`](../docs/testing/invariants-and-business-logic.md#timecurve-round-deadline-inclusive-warbow-gitlab-136).
...
<a id="presale-charm-15-weight-boost-gitlab-202"></a>
- **Presale CHARM +15% weight boost ([GitLab #202](https://gitlab.com/PlasticDigits/yieldomega/-/issues/202)):** Some deployments add **+15%** of the purchased CHARM amount to **`charmWeight` / `totalCharmWeight`** for buyers where **`isBeneficiary(buyer)`** is true on the contract at **`TimeCurve.doubPresaleVesting()`** (historical name — may be **`DoubPresaleVesting`** or a dedicated **`PresaleCharmBeneficiaryRegistry`** when vesting rows and boost lists differ). **Gross spend in the reserve asset is unchanged.** There is **no** global header badge for this; verify onchain **`isBeneficiary`** on the pointer **`TimeCurve`** uses. **`/vesting`** remains **DOUB claims** on the vesting proxy only — [`INV-TC-PRESALE-CHARM-BOOST`](../docs/testing/invariants-and-business-logic.md#timecurve-presale-charm-weight-boost) · [`timecurve-views` — presale CHARM boost](../docs/frontend/timecurve-views.md#timecurve-presale-charm-header-hint); players should confirm the deployment runbook.
- **Unredeemed DOUB in `TimeCurve`:** After **`saleEndedAt + 7 days`**, **`onlyOwner` `sweepUnredeemedLaunchedToken`** may send **all** remaining **`launchedToken`** to **`unredeemedLaunchedTokenRecipient`** (governance routing); participants who have not called **`redeemCharms`** before then forfeit that remainder onchain — [GitLab #128](https://gitlab.com/PlasticDigits/yieldomega/-/issues/128), [primitives §128](../../docs/product/primitives.md#unredeemed-launch-allocation-sweep-gitlab-128), [invariants §128](../../docs/testing/invariants-and-business-logic.md#timecurve-unredeemed-launch-allocation-sweep-gitlab-128). **Contributors** mirroring sweep events in Postgres: the **`idx_timecurve_unredeemed_launched_token_*`** migration includes a **paired SQLx down** for operator rollback ([GitLab #152](https://gitlab.com/PlasticDigits/yieldomega/-/issues/152)).
- **ETH / USDm → CL8Y (Kumbaya) on the Simple UI:** Optional pay modes run an **`exactOutput`** swap into CL8Y before `TimeCurve.buy`. The primary **Buy** control shows **Refreshing quote…** and stays **disabled** while the quoter read is in flight for the current slider amount (including background refetches), so participants do not click through a stale preview — [integrations/kumbaya.md](../../docs/integrations/kumbaya.md), [timecurve-views — Buy quote refresh](../../docs/frontend/timecurve-views.md#buy-quote-refresh-kumbaya-issue-56) ([issue #56](https://gitlab.com/PlasticDigits/yieldomega/-/issues/56)). **`approve` targets** use slippage-bounded swap **`maxIn`** to routers (not unlimited) per [GitLab #143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143); **CL8Y → `TimeCurve`** defaults to **exact** gross per buy with an optional unlimited opt-in in-app — [`wallet-connection §143`](../../docs/frontend/wallet-connection.md#erc20-approval-sizing-h-01-gitlab-143). **Wallet hygiene ([GitLab #144](https://gitlab.com/PlasticDigits/yieldomega/-/issues/144)):** multi-step buys may take **several** confirmations; **do not switch accounts or networks** mid-purchase — the app should abort with **“Wallet or network changed…”** rather than route funds to the wrong session. See [`wallet-connection.md` § #144](../../docs/frontend/wallet-connection.md#wallet-session-continuity-during-buy-gitlab-144).
- **Go-live gates (issue #55):** Onchain `onlyOwner` flags: **`buyFeeRoutingEnabled`** blocks **sale `buy` → `FeeRouter` and WarBow CL8Y spend** (steal / revenge / guard) until enabled; **`charmRedemptionEnabled`** / **`reservePodiumPayoutsEnabled`** for DOUB `redeemCharms` and **CL8Y reserve** `distributePrizes`; presale **`claimsEnabled`**. Read flags on the deployment — [final signoff runbook](../../docs/operations/final-signoff-and-value-movement.md). **Dev/local scripts** may turn post-end flags on for E2E; mainnet order is a human runbook concern.
...
- **Launch-anchor invariant (1.275× rule, [GitLab #158](https://gitlab.com/PlasticDigits/yieldomega/-/issues/158)):** `DoubLPIncentives` seeds **DOUB/CL8Y locked liquidity** at **1.275× the per-CHARM clearing price**, so a participant's CHARM is projected to be worth **`charmWeight × pricePerCharm × 1275 / (1000 × 1e18)`** CL8Y at launch — independent of the DOUB count it redeems for. **Worked example:** if the final buyer pays `2 CL8Y` for `1 CHARM` and `1 CHARM` redeems for `100 DOUB`, those `100 DOUB` are worth **`2 × 1.275 = 2.55 CL8Y`** at launch. The number is **non-decreasing** during the sale because the per-CHARM price (e.g. `LinearCharmPrice`) is non-decreasing in elapsed time, so when surfacing this to participants prefer **CL8Y-at-launch** over **projected DOUB count** (the latter dilutes as `totalCharmWeight` grows). Helpers: [`launchLiquidityAnchorWad`](../../frontend/src/lib/timeCurvePodiumMath.ts), [`participantLaunchValueCl8yWei`](../../frontend/src/lib/timeCurvePodiumMath.ts); test: `frontend/src/lib/timeCurvePodiumMath.test.ts` (`launch-anchor invariant: launch price = final per-CHARM price × 1.275`); policy: [`launchplan-timecurve.md`](../../launchplan-timecurve.md), [`docs/onchain/fee-routing-and-governance.md`](../../docs/onchain/fee-routing-and-governance.md), [`contracts/src/sinks/DoubLPIncentives.sol`](../../contracts/src/sinks/DoubLPIncentives.sol), [`docs/testing/invariants-and-business-logic.md`](../../docs/testing/invariants-and-business-logic.md#timecurve-launch-anchor-gitlab-158) (**Launch-anchor 1.275× rule**).
- **Fees:** full **gross** reserve per buy routes through **`FeeRouter`** (five sink slots: **30%** DOUB/CL8Y locked LP · **40%** CL8Y burned · **20%** podium pool · **0%** team/reserved · **10%** Rabbit Treasury at documented launch default) per [fee routing](../../docs/onchain/fee-routing-and-governance.md)—**verify** live `FeeRouter` on the target chain. **`FeeRouter` is not a wallet:** do not transfer arbitrary ERC-20 there expecting custody; protocol flow is **`TimeCurve` → `FeeRouter` → sinks** for the accepted asset ([GitLab #122](https://gitlab.com/PlasticDigits/yieldomega/-/issues/122)).
- **Accepted asset accounting:** Deployments assume **canonical ERC20** for CL8Y / stable rails; **`totalRaised`** sums **credited** CL8Y per successful **`buy`** (matches priced gross on standard tokens; non-standard tokens are intentionally unsupported — [GitLab #123](https://gitlab.com/PlasticDigits/yieldomega/-/issues/123), [`invariants §123`](../../docs/testing/invariants-and-business-logic.md#erc20-balance-delta-ingress-gitlab-123)).
...
When helping someone interpret standings, prefer **contract reads** (`podium(category)`, per-wallet mappings) and **`Buy` events** for **final settlement** after the sale. **During the live sale**, the app’s **Live reserve podiums** card ([issue #113](https://gitlab.com/PlasticDigits/yieldomega/-/issues/113)) reflects **`GET /v1/timecurve/podiums`** — **indexer DB predictions** for all four categories while **`sale_ended`** is false (**`INV-INDEXER-PODIUM-PREDICT-LIVE`** in [`invariants` §113](../../docs/testing/invariants-and-business-logic.md#indexer-timecurve-podiums-live-prediction)) — which can run ahead of empty onchain WarBow **`podium()`** slots until **`finalizeWarbowPodium`**. Treat that feed as **convenience / discovery**, not the payout source of truth. The UI does **not** surface an **“Indexer-backed snapshot”** status pill on that card ([GitLab #200](https://gitlab.com/PlasticDigits/yieldomega/-/issues/200) ↔ **`INV-FRONTEND-200-NO-INDEXER-PILL`** in [`invariants` §200](../../docs/testing/invariants-and-business-logic.md#frontend-stats-charts-empty-states-gitlab-200)).
...
**Simple buy projected-effects pills ([GitLab #227](https://gitlab.com/PlasticDigits/yieldomega/-/issues/227)):** Before submit, **`/timecurve`** shows **one `+{x}s` timer pill** (live hard-reset math), **per-source BP** pills, **defended-streak** continue/break/start chips, **`-{3dp} CL8Y/ETH/USDM` spend**, and **Become Last Buyer** — not combined BP prose or duplicate **time-booster credit** — [`INV-FRONTEND-227-BUY-PROJECTED`](../../docs/testing/invariants-and-business-logic.md#timecurve-buy-projected-effects-gitlab-227), [timecurve-views §227](../../docs/frontend/timecurve-views.md#timecurve-buy-projected-effects-gitlab-227), [manual QA #227](../../docs/testing/manual-qa-checklists.md#manual-qa-issue-227).
...
Resolve truth in this order; **indexers must not override** onchain state for **balances, final podium snapshots after `endSale`, or sale outcome**.

1. **Deployed contracts** — `TimeCurve` / `FeeRouter` (and related) **addresses** and **reads** for the target chain.
2. **Relevant events** — e.g. **`SaleStarted(startTimestamp=epoch, …)`** (owner **`startSaleAt(epoch)`** at sale open — [GitLab #114](https://gitlab.com/PlasticDigits/yieldomega/-/issues/114), [`docs/testing/invariants-and-business-logic.md` §114](../../docs/testing/invariants-and-business-logic.md#timecurve-startsaleat-issue-114)), `Buy`, `SaleEnded`, `CharmsRedeemed`, **`PrizesDistributed`** or **`PrizesSettledEmptyPodiumPool`** (empty pool — [GitLab #133](https://gitlab.com/PlasticDigits/yieldomega/-/issues/133)), `ReferralApplied`, `PodiumPaid` as emitted per [`docs/product/primitives.md`](../../docs/product/primitives.md) and deployment.
...
4. **Indexers / frontend** — **discovery** and **live-sale podium predictions** (`GET /v1/timecurve/podiums` while **`!sale_ended`**); **flag** if they disagree with RPC on **post-end** reads or onchain balances.
