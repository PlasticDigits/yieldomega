...
Indexer: when `VITE_INDEXER_URL` is set, use **`GET /v1/arena/timers`** (deadline, total DOUB raised, paused), **`GET /v1/arena/buys`**, and **`GET /v1/arena/wallet/{address}/stats`** for participant profile aggregates ([#255](https://gitlab.com/PlasticDigits/yieldomega/-/issues/255)) — legacy **`/v1/arena/*`** HTTP was removed ([#266](https://gitlab.com/PlasticDigits/yieldomega/-/issues/266), [#254](https://gitlab.com/PlasticDigits/yieldomega/-/issues/254)).
