...
9b. **CRED buy UI ([#269](https://gitlab.com/PlasticDigits/yieldomega/-/issues/269)):** `buyWithCred` via pay picker; burn from onchain `CRED_BUY_BURN` / `CRED_PER_CHARM_WAD` — [`arenaCredBurn.ts`](../../frontend/src/lib/arenaCredBurn.ts); optional `VITE_PLAY_CRED_ADDRESS`.
