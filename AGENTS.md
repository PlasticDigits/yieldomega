...
| **glab** | Always on `PATH` (`scripts/bin/glab` wrapper + system binary). **`GITLAB_TOKEN`** (PlasticDigits Cursor secret) is exported at bootstrap and used for **every** `glab` call. Configure: `bootstrap-cloud-vm-toolchain.sh`. Use **`glab`** for the **GitLab mirror** (historical issues/MRs) — **not** GitLab MCP or GitHub PR tooling. Product merge PRs open on Forgejo (`git.cl8y.com/code/yieldomega`); `scripts/glab-mr-create.sh` must not be the vehicle that lands those PRs ([ADR 0001](docs/architecture/adr/0001-remove-catchall-codeowners.md)). Verify: `bash scripts/verify-cloud-vm-toolchain.sh`. |
...
- **Frontend:** `http://127.0.0.1:5173` — primary play route **`/`** (AUDIT: **`/arena/protocol`**; legacy **`/arena`** redirects)
...
| Full product stack (indexer ingest, `/` play UI) | `bash scripts/start-qa-local-full-stack.sh …` | Native or Docker Postgres + Rust + optional Vite — only when acceptance criteria need indexer/UI. |
| **Forgejo pull requests** (Cloud agents) | Open on [git.cl8y.com/code/yieldomega](https://git.cl8y.com/code/yieldomega) (UI or `POST /api/v1/repos/code/yieldomega/pulls`). Landing PR for [ADR 0001](docs/architecture/adr/0001-remove-catchall-codeowners.md) adds `scripts/forgejo-pr-create.sh`. Do **not** use `scripts/glab-mr-create.sh` as the product merge vehicle — that helper files PlasticDigits GitLab MRs and will not delete Forgejo `CODEOWNERS`. |
...
| **Rabby import** | `node scripts/setup-rabby-dev-wallets.mjs` (from `frontend/` so Playwright resolves); password `RABBY_DEV_PASSWORD` (default `YieldomegaDevOnly1!`, **local only**). Manual fallback: `bash scripts/launch-chrome-with-rabby.sh http://127.0.0.1:5173/` |
...
```bash
bash scripts/launch-chrome-with-rabby.sh http://127.0.0.1:5173/
```
