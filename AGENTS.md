...
| **glab** | Installed by `bootstrap-cloud-vm-toolchain.sh`. Requires Cursor secret **`GITLAB_TOKEN`** (same token that passes `GET /projects/PlasticDigits%2Fyieldomega`). Sets `remote.origin_url` to `https://gitlab.com/PlasticDigits/yieldomega.git`. Verify: `glab auth status` · `glab mr list --per-page 1`. |
