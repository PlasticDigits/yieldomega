...
- **TimeCurve** — Token launch primitive combining bonding-curve-like pricing, timer extension, and strategic participation. **Minimum buy** (charm price floor) rises over time on a defined schedule (canonical **25% per day** target unless governance changes it); each purchase is **capped** at a fixed multiple of that minimum; purchases extend a **countdown** (or **hard-reset** remaining time toward **15 minutes** when under **13 minutes** left) up to a cap; the sale **ends** when the timer hits zero. Buys add **CHARM weight** (including referral bonuses); after the sale, **`redeemCharms`** converts CHARM into launched **DOUB** pro-rata to **`totalCharmWeight`**. **Reserve podium** payouts (accepted asset) come from **`PodiumPool`** after `endSale` across **three** fixed v1 categories — **last buy**, **time booster**, **defended streak** — see [primitives](product/primitives.md). **WarBow Ladder** is separate **Battle Points** PvP scoring (steal, guard, revenge, flag claim, streak-break bonuses); it is **not** a fourth podium slice. Canonical **TimeCurve** fee routing (of gross spend in the accepted asset): **DOUB locked LP** 25% · **CL8Y buy-and-burn** 35% · **podium pool** 20% · **team sink** 0% (reserved) · **Rabbit Treasury** 20% — [fee routing](onchain/fee-routing-and-governance.md#fee-sinks).
