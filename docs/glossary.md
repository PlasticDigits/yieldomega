...
- **TimeCurve** — Token launch primitive combining bonding-curve-like pricing, timer extension, and strategic participation. **Minimum buy** (charm price floor) rises over time on a defined schedule (canonical **20% per day** exponential envelope on the reference unless governance changes it); each purchase is **capped** at **10×** the nominal minimum CHARM at the same scale; purchases extend a **countdown** (or **hard-reset** remaining time toward **15 minutes** when under **13 minutes** left) up to a cap; the sale **ends** when the timer hits zero. Buys add **CHARM weight** (including referral bonuses); after the sale, **`redeemCharms`** converts CHARM into launched **DOUB** pro-rata to **`totalCharmWeight`**. **Reserve podium** payouts (accepted asset) come from **`PodiumPool`** after `endSale` across **four** categories — **last buy**, **WarBow** (top Battle Points), **defended streak**, **time booster** — see [primitives](product/primitives.md). **WarBow** combines **Battle Points** PvP scoring (steal, guard, revenge, flag claim, streak-break bonuses) with a **reserve-funded** top-3 slice from the podium pool; **from 2026-05-19**, WarBow CL8Y spend (steal / revenge / guard) routes via **`FeeRouter`** like buys — [historical `WarBowCl8yBurned` event](product/primitives.md#historical-warbowcl8yburned-event-name-2026-05-19-upgrade). Canonical **TimeCurve** fee routing (of gross spend in the accepted asset): **DOUB/CL8Y locked LP** 30% · **CL8Y burned** 40% · **podium pool** 20% · **team sink** 0% (reserved) · **Rabbit Treasury** 10% — [fee routing](onchain/fee-routing-and-governance.md#fee-sinks).
