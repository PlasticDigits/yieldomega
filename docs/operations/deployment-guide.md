...
**`DoubPresaleVesting` (DOUB):** non-empty **`PRESALE_BENEFICIARIES`** / matching **`PRESALE_AMOUNTS_WAD`** deploys vesting ( **`sum(amounts) == PRESALE_TOTAL_ALLOCATION_WAD`** ). The **allocation row can differ** from **`PRESALE_CHARM_BOOST_ADDRESSES`** (e.g. first participant’s **10M** DOUB vests to **`0x0965…`** while **`0xA5F4…`** keeps the **+15% CHARM** boost on buys). **MegaETH (4326):** the wrapper defaults the canonical vesting row when **`PRESALE_BENEFICIARIES`** is unset. Export **`PRESALE_BENEFICIARIES=''`** to skip vesting. Default schedule: **30%** at **`startVesting`**, **70%** linear over **`PRESALE_VESTING_DURATION_SEC`** (canonical **180 days**). For **instant presale payout** on live mainnet, UUPS-upgrade and owner **`sendNow()`** — [`presale-doub-instant-distribution.md`](presale-doub-instant-distribution.md).

**Frontend:** set **`VITE_PRESALE_CHARM_BENEFICIARY_REGISTRY`** and **`VITE_DOUB_PRESALE_VESTING_ADDRESS`** from the deployment registry so build-time env matches **`TimeCurve`** (registry vs vesting split) while **`/vesting`** reads the vesting proxy.
...
- After a **`DoubPresaleVesting`** UUPS upgrade, confirm the **verified implementation** on MegaETH Etherscan matches the [root README](https://gitlab.com/PlasticDigits/yieldomega/-/blob/main/README.md) reference (`0xFE4C7A3BadA9790dE52146D8fB05012c735B7247`); update that README line when you publish a newer verified impl. Instant presale payout: [`presale-doub-instant-distribution.md`](presale-doub-instant-distribution.md) (`sendNow` + `scripts/uups-upgrade-doub-presale-vesting-mainnet.sh`).
