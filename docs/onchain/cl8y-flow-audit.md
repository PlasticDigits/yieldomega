...
5. **No hidden rescue/sweep paths were found** beyond **governed, evented** recovery: production `contracts/src` CL8Y-relevant transfer sites are enumerable via `safeTransfer`, `safeTransferFrom`, **`distributeFees`** (allowlisted tokens only), **`rescueERC20`** (`FeeRouter`, `GOVERNOR_ROLE`), `distributePrizes`, `withdraw`, and `receiveFee`. Local Anvil fixtures and mock CL8Y minting are out of production scope.
