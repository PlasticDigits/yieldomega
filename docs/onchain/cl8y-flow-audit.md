...
3. **(Addressed — #70 / #117)** **`TimeCurveBuyRouter` custody.** CL8Y post-`buyFor` surplus routes to **`cl8yProtocolTreasury`** (**#70**). **ETH / stable** refund only marginal unused **`msg.value`/allowance-margin** (**#117**, audit M‑02); stranded input recovers via owner **`rescue*`** functions. See [invariants §117](../testing/invariants-and-business-logic.md#timecurve-buy-router-net-refund-rescue-issue-117).
