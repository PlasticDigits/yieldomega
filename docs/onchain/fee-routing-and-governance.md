...
**Interim custody ([GitLab #159](https://gitlab.com/PlasticDigits/yieldomega/-/issues/159)):** until **`RabbitTreasury.receiveFee`** integration is proven end-to-end, governance may point the **fifth sink** at **`RabbitTreasuryVault`** ([`treasury-contracts.md`](treasury-contracts.md#rabbit-treasury-vault-gitlab-159)). Vault-held **`ERC20` is not Burrow `protocolOwnedBacking`** until swept through **`receiveFee`** or sinks are repointed at **`RabbitTreasury`**.

Each **buy** routes the **full gross** amount in the accepted asset through **`FeeRouter`** (referral economics use **CHARM weight**, not reserve carve-outs — see [referrals](../product/referrals.md)). The **30%** slice lands at **`DoubLPIncentives`** for **locked** DOUB/**CL8Y** liquidity policy (paired at **1.275×** the projected **final reserve-per-DOUB** clearing anchor; **Kumbaya v3** uses a **0.8×–∞** band around the **launch anchor** — see [launchplan-timecurve.md](../../launchplan-timecurve.md) and product UX). The **podium pool** is the **`PodiumPool`** contract; **`TimeCurve.distributePrizes`** pays winners in **reserve** (CL8Y), not DOUB. **Charm redemption** (`redeemCharms`) is **DOUB-only** and is **separate** from this routing (sale allocation, not fee slice).

**WarBow CL8Y spend (2026-05-19):** **`warbowSteal`**, **`warbowRevenge`**, and **`warbowActivateGuard`** pull fixed WAD amounts from the payer and call the same **`_routeAcceptedAssetViaFeeRouter`** path as **`buy`** (canonical five-sink split; **`totalRaised`** includes gross WarBow spend). **`claimWarBowFlag`** has no CL8Y leg. Onchain identifiers **`WARBOW_*_BURN_WAD`**, **`WarBowCl8yBurned`**, and indexer **`cl8y_burned`** are **historical names** — post-upgrade **`amountWad`** is nominal spend, not “100% to burn sink” ([primitives — historical event](../product/primitives.md#historical-warbowcl8yburned-event-name-2026-05-19-upgrade)).
...
| **Rabbit Treasury** | 10% | **`RabbitTreasury`** proxy ([treasury-contracts.md](treasury-contracts.md), [rabbit-treasury.md](../product/rabbit-treasury.md)); optional interim **`RabbitTreasuryVault`** fifth sink per [#159](treasury-contracts.md#rabbit-treasury-vault-gitlab-159) | **[CL8Y](#governance-fee-split-weights)** — **TimeCurve** fee slice; **repricing** and other Burrow knobs are a separate class ([Rabbit Treasury repricing](#governance-rabbit-repricing)). |
