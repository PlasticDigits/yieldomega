...
|------|-------------|----------------------------|
| **TimeCurve `distributePrizes`** | **Empty-pool settlement visibility + lock ([GitLab #133](https://gitlab.com/PlasticDigits/yieldomega/-/issues/133))** — A **zero** `PodiumPool` balance at `distributePrizes` previously **returned silently** (no event, `prizesDistributed` false), which complicated indexer/ops signalling and allowed **repeat** owner calls after donations. **Mitigation:** **`reservePodiumPayoutsEnabled`** applies to **both** empty and non-empty paths; **empty** path emits **`PrizesSettledEmptyPodiumPool`** and sets **`prizesDistributed`** (**no** retry). **Non-empty** path emits **`PrizesDistributed`** only after full drain (GitLab #116 residual rules). | Forge: `test_distributePrizes_empty_vault_emits_empty_settlement_and_locks`, `test_distributePrizes_after_empty_settlement_refill_does_not_retry`, `test_distributePrizes_reduces_vault_and_sets_flag`; [invariants §133](../testing/invariants-and-business-logic.md#timecurve-prizes-empty-podium-pool-issue-133). |
