...
| **`ArenaBuyRouting.splitBuyAmount`** | **Rounding residue** — Integer division can leave wei unmatched to documented 100% podium split ([#300](https://gitlab.com/PlasticDigits/yieldomega/-/issues/300)). | Category remainder → **Time Booster (cat 3)**; within-category remainder → **+2 tranche**; fuzz in `ArenaPrizeRouting.t.sol`. **0%** admin on buys. |
