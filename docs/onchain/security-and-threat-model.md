...
| 4 | **Rounding / split errors** — DOUB buy routing (100% podiums · 25% × 4 · 70/20/10 epoch tranches ([#300](https://gitlab.com/PlasticDigits/yieldomega/-/issues/300))) and podium settlement can **drift** from documented intent (rounding direction). | **Single canonical split** in [`ArenaBuyRouting`](../../contracts/src/arena/libraries/ArenaBuyRouting.sol); **documented rounding** (category remainder → Last Buy ([#313](https://gitlab.com/PlasticDigits/yieldomega/-/issues/313)); within-category remainder → +2 tranche); **WAD** discipline; tests in `ArenaPrizeRouting.t.sol`. |
...
| **`TimeArena` WarBow DOUB spends** | **Insufficient balance / reentrancy / stale price assumption** — steal, guard, and revenge derive their DOUB cost from the epoch charm anchor after autoroll; the bypass remains fixed. | Revert on short balance; CEI pattern; clients read `WARBOW_*_DOUB()` at submission; tests cover anchor-derived ratios ([#252](https://gitlab.com/PlasticDigits/yieldomega/-/issues/252), [#367](https://gitlab.com/PlasticDigits/yieldomega/-/issues/367)). |
| **`TimeArena.warbowSteal` BP eligibility** | **Whale targeting / stale client preflight** — a widened band can be misrepresented by a stale UI or bot. | Enforce inclusive `attackerBP ≤ victimBP ≤ 50 × attackerBP` onchain; surface the contract result before signing; equal BP is eligible ([#366](https://gitlab.com/PlasticDigits/yieldomega/-/issues/366)). |
| **`ArenaBuyRouting.splitBuyAmount`** | **Rounding residue** — Integer division can leave wei unmatched to documented 100% podium split ([#300](https://gitlab.com/PlasticDigits/yieldomega/-/issues/300)). | Category remainder → **Last Buy (cat 0)** ([#313](https://gitlab.com/PlasticDigits/yieldomega/-/issues/313)); within-category remainder → **+2 tranche**; fuzz in `ArenaPrizeRouting.t.sol`. **0%** admin on buys. |
