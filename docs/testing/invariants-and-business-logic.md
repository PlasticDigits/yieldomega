...
| **`INV-TIME-ARENA-PODIUM-EPOCH-SCORE-RESET`** | Time Booster ranks by **`epochTimerSecAdded`** (not lifetime `totalEffectiveTimerSecAdded`); Defended Streak ranks by **`epochBestDefendedStreak`**; stale generation reads zero; **`migrateEpochPodiumScores`** seeds in-flight slot holders | `test_time_booster_score_resets_on_epoch_roll`, `test_time_booster_displaced_reinsert_uses_epoch_metric`, `test_defended_streak_best_resets_on_epoch_roll`, `test_migration_seeds_slot_holders` · indexer `arena_wallet_stats.rs` epoch-window simulation |
