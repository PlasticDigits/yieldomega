...
| Invariant | Check |
|----------|--------|
| **Preconditions** | `TimeCurve.ended == true`; `charmRedemptionEnabled` and `reservePodiumPayoutsEnabled` start **false** (after [DeployDev](../../contracts/script/DeployDev.s.sol)’s E2E convenience flags, the setup script [resets them](https://gitlab.com/PlasticDigits/yieldomega/-/issues/79)); `prizesDistributed == false`; **non-zero** `acceptedAsset` balance of `podiumPool` so the script can exercise the **reserve gate** on a non-empty pool ([GitLab #133](https://gitlab.com/PlasticDigits/yieldomega/-/issues/133): **zero** balance still calls `distributePrizes` once the gate is on — emits **`PrizesSettledEmptyPodiumPool`** and sets **`prizesDistributed`**; that path is **Forge** / manual `cast`, not this script). |
...
| **Podium settlement — zero pool latch ([GitLab #133](https://gitlab.com/PlasticDigits/yieldomega/-/issues/133))** <a id="timecurve-prizes-empty-podium-pool-issue-133"></a> | When **`distributePrizes`** runs with **`acceptedAsset.balanceOf(podiumPool) == 0`**, **`reservePodiumPayoutsEnabled`** must still be **true** (same gate as non-empty path). **`prizesDistributed := true`**; emit **`PrizesSettledEmptyPodiumPool(podiumPool)`**; **`PrizesDistributed`** is **not** emitted. Second **`distributePrizes`** reverts **`TimeCurve: prizes done`** — **no ops refill + retry.** Non-zero pool: drains per category, then **`PrizesDistributed`**. **`INV-INDEXER-133`:** indexer persists empty settlement in **`idx_timecurve_prizes_settled_empty_podium_pool`**; API **`kind`**: **`empty_podium`** \| **`drained`**. | `test_distributePrizes_empty_vault_emits_empty_settlement_and_locks`, `test_distributePrizes_after_empty_settlement_refill_does_not_retry`, `test_distributePrizes_empty_pool_reverts_when_reserve_podium_payouts_disabled` |
