...
| **`INV-TIME-ARENA-CRED-POOL-ACCRUE`** | Each `buyWithCred` adds **35 CRED** (`CRED_PER_BUY`) to `epochCredPool[lastBuyEpoch]`; CHARM weight accrual unchanged ([#311](https://gitlab.com/PlasticDigits/yieldomega/-/issues/311)) | `test_buy_with_cred`, `test_buyWithCred_accrues_epoch_cred_pool`, `test_cred_pro_rata_mixed_doub_and_cred`, `test_buyWithCred_epoch_boundary_credits_correct_pool`, `verify-cred-buy-anvil.sh` |
