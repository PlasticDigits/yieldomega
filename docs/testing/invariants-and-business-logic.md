...
| **Velocity** | Count buys with **`block_timestamp >= anchor - window`** (**`1h`** / **`24h`**) or **`block_timestamp >= sale_start_sec`** (**`sale`**); **`avg_buys_per_hour = count / window_hours`** where **`sale`** derives **`window_hours = floor((anchor - sale_start) / 3600)`** (min **1**). Anchor: head **`chain_timer.block_timestamp_sec`**, else **`MAX(block_timestamp)`** on buys. Sale-window pre-open (**`sale_start = 0`** or **`anchor < sale_start`**) returns zero buys without divide-by-zero. |
