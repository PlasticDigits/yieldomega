...
| **`INV-TIME-ARENA-CRED-POOL-ACCRUE`** | Each **`buyWithCred`** adds **`CRED_PER_BUY`** (35e18) to `epochCredPool[lastBuyEpoch]`; same CHARM accrual as DOUB buys; **no** DOUB routing | `test_buy_with_cred`, `test_buyWithCred_mixed_pro_rata_with_doub`, `test_buyWithCred_at_epoch_boundary`, `test_cred_only_buyer_fair_share` ([#311](https://gitlab.com/PlasticDigits/yieldomega/-/issues/311)) · `bash scripts/verify-cred-buy-anvil.sh` |
