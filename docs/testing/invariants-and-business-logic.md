...
| **`INV-TIME-ARENA-ALWAYS-LIVE`** | No `endSale` / `redeemCharms`; only `paused` gates | `TimeArena.sol` + negative grep in arena contracts |
...
| **`INV-INDEXER-260-NO-TIMECURVE-DECODE`** | No `TimeCurve*` `DecodedEvent` variants; Arena + referral registry only | `decoder.rs`, `cargo test` |

**Pay-mode E2E:** `arena-paywith-{cl8y,eth,usdm}` on [`TimeCurveSimplePage.tsx`](../../frontend/src/pages/TimeCurveSimplePage.tsx) (wrapped by `/arena`). ETH route E2E gates on `VITE_KUMBAYA_TIMECURVE_BUY_ROUTER` until [#251](https://gitlab.com/PlasticDigits/yieldomega/-/issues/251).
...
GitHub Actions sets `YIELDOMEGA_PG_TEST_URL` against a **service container** so `postgres_stage2_persist_all_events_and_rollback_after` connects, runs migrations, inserts **every non-Unknown Arena v2** [`DecodedEvent`](../../indexer/src/decoder.rs) variant, checks idempotency, calls `rollback_after`, then HTTP API smoke for **`GET /v1/arena/*`** and **`GET /v1/referrals/*`** ([ci.md](ci.md)).
...
**Intent:** MegaETH-scale Arena viewers must not **spam** the indexer on every sub-second block; WarBow HTTP handlers must keep **btree** predicates aligned with lowercase **`addr_hex`** rows; **GitLab #172** removed **`refreshWarbowPodium`** — post-**`endSale`**, only the owner **`finalizeWarbowPodium(first, second, third)`** may set **`warbowPodiumFinalized`**; **`pendingRevengeStealer`** must align with the **soonest-expiring** window; post-end gate scripts must validate **`ADDR_ALICE`**.
...
| **`INV-INDEXER-170-REFERRAL-LB-TIE`** | **`GET /v1/referrals/referrer-leaderboard`** row order: **`COALESCE(SUM(referrer_amount),0) DESC NULLS LAST, referrer ASC`** — stable **`LIMIT`/`OFFSET`** when totals tie. **`rank`** in JSON is **dense competitive `RANK()`** over **Σ CHARM DESC only** (ties share a rank; next rank skips — [GitLab #177](https://gitlab.com/PlasticDigits/yieldomega/-/issues/177); `postgres_gitlab177_referrer_leaderboard_dense_rank` in **`integration_stage2.rs`**), **not** page ordinal (**`offset + row_index + 1`**). **`referrer ASC` must stay on the outer `ORDER BY` only** — including it inside the window `ORDER BY` prevents ties (every referrer is unique) and regresses to **`1, 2, 3, 4`** ([GitLab #204](https://gitlab.com/PlasticDigits/yieldomega/-/issues/204) dense-rank fix, commit **`f0d89b2`**). |
...
| **Frontend — header mascot nav clearance (#171)** <a id="frontend-header-mascot-nav-clearance-gitlab-171"></a> | **`INV-FRONTEND-171-HEADER-MASCOT`**: the decorative `RootLayout` circular mascot reserves a desktop/landscape gutter via **`--app-header-mascot-clearance`** and **`padding-inline-end`** on `.app-header__brand`, is positioned inside that gutter, and uses **`pointer-events: none`** so it cannot cover the **TimeCurve** nav hit area. At **`max-width: 720px`**, the mascot remains hidden and the reserved gutter resets to zero. | [`index.css`](../../frontend/src/index.css), [`headerLayoutCss.test.ts`](../../frontend/src/layout/headerLayoutCss.test.ts), [design — Global header layout](../frontend/design.md#accessibility-and-ux), [manual QA (#171)](manual-qa-checklists.md#manual-qa-issue-171) |
...
| **Frontend — placeholder split hero figure (#163)** | **`UnderConstruction`** and **`ThirdPartyDexPage`** use **`.split-layout > .placeholder-figure`**. CSS Grid defaults **`align-items: stretch`**, so the figure matched the **taller** **`PageSection`** column while the **`<img>`** kept **intrinsic height** — empty bordered panel below the art on **wide / landscape** viewports. **`INV-FRONTEND-163`:** direct child rule sets **`align-self: start`**, **`width: 100%`**, **`max-width: min(42rem, 100%)`**, **`min-width: 0`**. | [design — Placeholder split panels](../frontend/design.md#placeholder-split-panels-gitlab-163), [§ #163](#placeholder-split-layout-figure-gitlab-163), [`index.css`](../../frontend/src/index.css), [`placeholderSplitLayoutCss.test.ts`](../../frontend/src/lib/placeholderSplitLayoutCss.test.ts) ([issue #163](https://gitlab.com/PlasticDigits/yieldomega/-/issues/163)) |
| **Frontend — home product card grid (#198)** <a id="frontend-home-product-card-grid-gitlab-198"></a> | **`INV-FRONTEND-198-HOME-CARDS`**: at desktop width, `.home-cta-grid` uses stretched equal implicit rows, `.home-cta-grid__item` is the stretching wrapper, and `.home-cta-card` fills its item (`height: 100%`) so product cards stay the same visual height across TimeCurve, Rabbit Treasury, Collection, Referrals, Kumbaya, and Sir despite different copy lengths. Referral overview cards keep the same stretch contract in their responsive grid. | [design — Home product card grid](../frontend/design.md#home-product-card-grid-gitlab-198), [`index.css`](../../frontend/src/index.css), [`homeCardGridCss.test.ts`](../../frontend/src/lib/homeCardGridCss.test.ts), [manual QA (#198)](manual-qa-checklists.md#manual-qa-issue-198) ([issue #198](https://gitlab.com/PlasticDigits/yieldomega/-/issues/198)) |
...
| **Frontend — `LaunchGate` `/home` hub route ([GitLab #199](https://gitlab.com/PlasticDigits/yieldomega/-/issues/199))** <a id="inv-frontend-199-home-route"></a> | **`INV-FRONTEND-199-HOME-ROUTE`:** With **`VITE_LAUNCH_TIMESTAMP` unset** (no-env), **`/`** remains the canonical **`HomePage`** URL, but **`/home`** must still register the same **`HomePage`** so **`RootLayout` `<Outlet />`** is not empty (avoids shell-only green grid). With **`VITE_LAUNCH_TIMESTAMP` in the past** (post-launch), **`/`** is **`TimeCurveSimplePage`** and **`/home`** is the hub — unchanged. | [`LaunchGate.tsx`](../../frontend/src/app/LaunchGate.tsx), [§ LaunchGate `/home`](#launchgate-home-route--no-env-parity-gitlab-199), [`timecurve-views` — Launch handoff](../frontend/timecurve-views.md#launchcountdown--simple-handoff), [manual QA (#199)](manual-qa-checklists.md#manual-qa-issue-199-home-route-launchgate), [`launch-countdown.spec.ts`](../../frontend/e2e/launch-countdown.spec.ts) |
| **Frontend — branded 404 catch-all ([GitLab #223](https://gitlab.com/PlasticDigits/yieldomega/-/issues/223))** <a id="inv-frontend-223-not-found"></a> | **`INV-FRONTEND-223-NOT-FOUND`:** **`ShellRoutes`** registers **`path="*"`** **last** under **`RootLayout`** → **`NotFoundPage`** (`data-testid="not-found-page"`). Valid **`/timecurve`**, **`/timecurve/arena`**, **`/timecurve/protocol`**, and **`/timecurve/{referral}`** unchanged. Unknown paths (**`/nope`**, **`/luck777`**, extra segments) 404 inside the shell. **`ReferralPathSync`** still captures **`?ref=`** on 404 URLs. Invalid referral slugs under **`/timecurve/{segment}`** still render Simple (no 404). SPA HTTP status may remain **200** — out of scope. | [`LaunchGate.tsx`](../../frontend/src/app/LaunchGate.tsx), [`NotFoundPage.tsx`](../../frontend/src/pages/NotFoundPage.tsx), [§ #223](#branded-404-catch-all-gitlab-223), [referrals — link capture](../product/referrals.md#client-link-capture-frontend), [manual QA (#223)](manual-qa-checklists.md#manual-qa-issue-223), [`navigation.spec.ts`](../../frontend/e2e/navigation.spec.ts) |
...
| **Frontend — Arena `Buy CHARM` wrong-chain visual ([GitLab #194](https://gitlab.com/PlasticDigits/yieldomega/-/issues/194))** <a id="inv-frontend-194-arena-buy-chain"></a> | **`INV-FRONTEND-194-ARENA-BUY-CHAIN`:** On **`/timecurve/arena`** with **`useWalletTargetChainMismatch()`** true, the primary arcade **`Buy CHARM`** CTA remains **`disabled`** (same cascade as before) but must **read as disabled**: extra **`timecurve-simple__cta--wrong-network`** styling (stronger opacity / grayscale vs default **`btn-primary:disabled`** alone), native **`title={chainMismatchWriteMessage(walletChainId)}`**, **`data-testid="timecurve-arena-buy-charm-cta"`**, **no** Framer **`whileHover` / `whileTap`** lift while mismatched, and the buy-panel **`ChainMismatchWriteBarrier`** overlay stacks above z-indexed panel chrome (scoped **`z-index`** bump). Complements [#95](#frontend-wallet-chain-write-gating-issue-95) / [§194](#arena-buy-charm-wrong-chain-visual-gitlab-194). | [`TimeCurveArenaView.tsx`](../../frontend/src/pages/timeCurveArena/TimeCurveArenaView.tsx), [`index.css`](../../frontend/src/index.css), [timecurve-views §194](../frontend/timecurve-views.md#arena-buy-charm-wrong-chain-visual-gitlab-194), [manual QA (#194)](manual-qa-checklists.md#manual-qa-issue-194-arena-buy-chain-visual) |
| **Frontend — ERC-20 approval sizing (#143)** | In-app **`approve`** defaults to **exact** amounts for Kumbaya router legs (**`maxIn`**), **`ReferralRegistry`** burn (**`registrationBurnAmount`**), and **CL8Y → `TimeCurve`** (**per-tx gross**). Optional **unlimited** CL8Y → TimeCurve via **`Cl8yTimeCurveUnlimitedApprovalFieldset`** + disclosure (localStorage **`yieldomega.erc20.cl8yTimeCurveUnlimited.v1`**); mitigates blast radius vs audit **H-01** coupling ([GitLab #143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143)). | [wallet-connection §143](../frontend/wallet-connection.md#erc20-approval-sizing-h-01-gitlab-143), [timecurve-views §143](../frontend/timecurve-views.md#erc20-approval-sizing-gitlab-143), § [INV-ERC20-APPROVAL-143](#frontend-erc20-approval-sizing-gitlab-143), [`cl8yTimeCurveApprovalPreference.ts`](../../frontend/src/lib/cl8yTimeCurveApprovalPreference.ts), [`ensureCl8yTimeCurveAllowance.ts`](../../frontend/src/lib/ensureCl8yTimeCurveAllowance.ts) |
| **Frontend — Arena CL8Y approve skip when sufficient ([#224](https://gitlab.com/PlasticDigits/yieldomega/-/issues/224))** | **`INV-FRONTEND-224-CL8Y-APPROVE`:** Simple **`submitBuy`**, Arena **`handleBuy`**, and WarBow **`ensureTcAllowance`** share **`ensureCl8yTimeCurveAllowance`** — submit **`approve(TimeCurve, …)`** only when **`allowance < cl8yTimeCurveApprovalAmountWei(need, pref)`** (not **`allow < need`** alone). Skips redundant approves when on-chain allowance is already **`type(uint256).max`** or meets sized headroom ([#143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143)). | § [INV-FRONTEND-224-CL8Y-APPROVE](#arena-cl8y-approve-guard-gitlab-224), [manual QA #224](manual-qa-checklists.md#manual-qa-issue-224), [wallet-connection §143](../frontend/wallet-connection.md#erc20-approval-sizing-h-01-gitlab-143) |
...
| **Indexer — ingestion liveness + RPC timeouts ([GitLab #168](https://gitlab.com/PlasticDigits/yieldomega/-/issues/168))** | Block ingestion **retries** after fatal errors with **exponential backoff**; JSON-RPC uses **reqwest** **per-request** timeouts (**`INDEXER_RPC_REQUEST_TIMEOUT_SEC`**, default **5s**, clamped **1–120s**). **`GET /v1/status`** includes **`ingestion_alive`** + **`last_indexed_at_ms`** (schema **≥ 1.16.1**) for stale-data detection (**`INV-INDEXER-168`**). | [§ #168](#indexer-ingestion-liveness-and-rpc-timeouts-gitlab-168), [`main.rs`](../../indexer/src/main.rs), [`rpc_http.rs`](../../indexer/src/rpc_http.rs), [indexer README — #168](../../indexer/README.md#ingestion-supervision--rpc-timeouts-gitlab-168) |
...
| **TimeCurve Arena — WarBow hero actions (issue #101)** | `PageHeroArcadeBanner` exposes **Steal / Guard / Revenge** directly in the hero through `WarbowHeroActions`. Suggested steal targets are discovery-only rows from `warbowLadderPodium()` and `/v1/timecurve/warbow/leaderboard`, deduped and filtered by the onchain **2×–10×** BP band when viewer BP is **positive** ([GitLab #211](https://gitlab.com/PlasticDigits/yieldomega/-/issues/211)); **near-miss** rows show wallets below **2×** or above **10×** with disabled hero **STEAL** chrome. Selecting an in-band row feeds the existing `stealVictimInput`, so live `battlePoints` / `stealsReceivedOnDay` / `stealsCommittedByAttackerOnDay` reads and `describeStealPreflight` remain the eligibility gate. Guard/revenge CTAs share wrong-network and `buyFeeRoutingEnabled` barriers. **Revenge** lists **all** open **(victim, stealer)** windows from the indexer when configured ([GitLab #135](https://gitlab.com/PlasticDigits/yieldomega/-/issues/135), [§ #135](#warbow-per-stealer-revenge-windows-gitlab-135)). **Claim flag** when the viewer holds the pending slot — [GitLab #218](https://gitlab.com/PlasticDigits/yieldomega/-/issues/218), [§ #218](#arena-warbow-claim-flag-hero-gitlab-218). | [timecurve-views — Arena WarBow hero actions](../frontend/timecurve-views.md#arena-warbow-hero-actions-issue-101), [`WarbowHeroActions.tsx`](../../frontend/src/pages/timeCurveArena/WarbowHeroActions.tsx), [`WarbowClaimFlagHeroCard.tsx`](../../frontend/src/pages/timeCurveArena/WarbowClaimFlagHeroCard.tsx), [`WarbowHeroActions.test.tsx`](../../frontend/src/pages/timeCurveArena/WarbowHeroActions.test.tsx), [`warbowStealBpBand.ts`](../../frontend/src/lib/warbowStealBpBand.ts), [participant play skill](../../skills/play-timecurve-warbow/SKILL.md) |
| **TimeCurve Arena — WarBow revenge hero row preview ([GitLab #236](https://gitlab.com/PlasticDigits/yieldomega/-/issues/236))** | **`INV-FRONTEND-236-REVENGE-HERO`:** The Arena revenge hero card (**`WarbowHeroActions`** revenge block) shows **no** "You have N open counter-hits" / "Earliest expiry" summary paragraph and **no** UTC **`UnixTimestampDisplay`** on rows. Each row renders (1) **Target BP** from live batched **`useReadContracts`** on **`battlePoints(stealer)`** keyed off **`pendingRevengeTargets`** (loading hint while pending), (2) **Gain** via **`warbowRevengeDrainBp(stealerBp)`** mirroring onchain **`_warbowStealDrainBp`** (**10%** drain — **`WARBOW_STEAL_DRAIN_BPS = 1000`**); zero-drain rows show a muted "Revenge zero (no BP to drain)" hint instead of **`+0 BP`**, and (3) **Valid To: HH:MM:SS** ticking from **`expiry_exclusive - ledgerNowSec`** with the same **`effectiveLedgerSec`** as guard / phase, clamped to **`00:00:00`** past expiry. Display-only; onchain reads + 8s indexer poll for **`pending-revenge`** unchanged. | [§101](#timecurve-arena-warbow-hero-actions-issue-101), [`WarbowHeroActions.tsx`](../../frontend/src/pages/timeCurveArena/WarbowHeroActions.tsx), [`warbowRevengeRowPreview.ts`](../../frontend/src/lib/warbowRevengeRowPreview.ts), [`warbowRevengeRowPreview.test.ts`](../../frontend/src/lib/warbowRevengeRowPreview.test.ts), [`WarbowHeroActions.test.tsx`](../../frontend/src/pages/timeCurveArena/WarbowHeroActions.test.tsx) |
| **TimeCurve Arena — WarBow claim flag hero ([GitLab #218](https://gitlab.com/PlasticDigits/yieldomega/-/issues/218))** | **`INV-FRONTEND-218-WARBOW-CLAIM-FLAG`:** When **`warbowPendingFlagOwner`** === viewer and **`warbowPendingFlagPlantAt > 0`**, hero **`WarbowClaimFlagHeroCard`** shows disabled **`Claim flag HH:MM:SS`** during silence (ledger **`effectiveLedgerSec`**) then enabled **Claim flag** → **`runWarBowClaimFlag()`**. Helper copy tracks **`WARBOW_FLAG_CLAIM_BP`** / **2×** penalty. | [§218](#arena-warbow-claim-flag-hero-gitlab-218), [timecurve-views §218](../frontend/timecurve-views.md#arena-warbow-claim-flag-hero-gitlab-218), [`warbowClaimFlagCopy.ts`](../../frontend/src/lib/warbowClaimFlagCopy.ts), [manual QA (#218)](manual-qa-checklists.md#manual-qa-issue-218) |
| **TimeCurve Arena — WarBow steal victim field ([GitLab #195](https://gitlab.com/PlasticDigits/yieldomega/-/issues/195))** | **`INV-FRONTEND-195-WARBOW-STEAL-FORM`:** Invalid / empty steal-victim validation for **`WarbowSection`** stays in **`data-testid="warbow-steal-victim-form-status"`** under the address input — not **`pvpErr`** hero/post-section banners and not the section-wide **`warbowActionHint`** strip on idle load. Helpers: [`warbowStealVictimInput.ts`](../../frontend/src/lib/warbowStealVictimInput.ts). | [§195](#timecurve-arena-warbow-steal-victim-validation-gitlab-195), [timecurve-views §195](../frontend/timecurve-views.md#arena-warbow-steal-victim-field-gitlab-195) |
| **TimeCurve Arena — WarBow indexer leaderboard + battle-feed refresh ([GitLab #182](https://gitlab.com/PlasticDigits/yieldomega/-/issues/182))** | **`INV-FRONTEND-182-WARBOW-IDX`:** With **`VITE_INDEXER_URL`**, **`useTimeCurveArenaModel`** refetches **`GET /v1/timecurve/warbow/leaderboard`** and **`GET /v1/timecurve/warbow/battle-feed`** on **`refetchAll`**, **BP-moving event watches**, and a **~1.5s backoff** poll while **`/timecurve/arena`** is mounted — **`warbowRank`**, chasing-pack rows, and rivalry feed stay aligned after BP-changing txs and when other wallets move the ladder. Stale parallel completes are dropped via a **sequence guard**. See also **`INV-FRONTEND-WARBOW-PODIUM-LIVE`**. | [§182](#timecurve-arena-warbow-indexer-refresh-gitlab-182), [§ live WarBow podium](#live-warbow-podium-simple-arena-gitlab-warbow-podium-live), [`useTimeCurveArenaModel.tsx`](../../frontend/src/pages/timeCurveArena/useTimeCurveArenaModel.tsx), [timecurve-views §182](../frontend/timecurve-views.md#arena-warbow-indexer-leaderboard-feed-refresh-gitlab-182), [manual QA (#182)](manual-qa-checklists.md#manual-qa-issue-182) |
| **TimeCurve Arena — WarBow Chasing pack list ([GitLab #189](https://gitlab.com/PlasticDigits/yieldomega/-/issues/189))** | **`INV-FRONTEND-189-WARBOW-CHASING-PACK`:** The **Chasing pack** panel (**`WarbowSection`**) renders **every** row returned for the indexed WarBow ladder (**no** legacy **`slice(0, 6)`** cap). Long lists scroll inside **`.warbow-chasing-pack-scroll`** (**`max-height: min(70vh, 28rem)`**, **`overflow-y: auto`**) so **7+** participants do not clip or blow out the split layout — helper **`warbowLeaderboardForChasingPackDisplay`**, Vitest **`warbowChasingPackLeaderboard.test.ts`**. | [§189](#timecurve-arena-warbow-chasing-pack-gitlab-189), [`warbowChasingPackLeaderboard.ts`](../../frontend/src/pages/timeCurveArena/warbowChasingPackLeaderboard.ts), [timecurve-views §189](../frontend/timecurve-views.md#arena-warbow-chasing-pack-scroll-gitlab-189), [manual QA (#189)](manual-qa-checklists.md#manual-qa-issue-189) |
| **TimeCurve Arena — post-timer settlement CTAs ([GitLab #188](https://gitlab.com/PlasticDigits/yieldomega/-/issues/188))** | **`INV-FRONTEND-188-ARENA-SETTLEMENT`:** When the phase machine is **`saleExpiredAwaitingEnd`** (**`ledgerSecInt > deadlineSec`**, **`ended == false`**), **`/timecurve/arena`** must **not** hide **End sale** / **Redeem charms** / **Distribute prizes** behind the live-standings layout. **`TimeCurve.endSale()`** is **permissionless** once the timer predicate holds; **`redeemCharms`** / **`distributePrizes`** stay **disabled** (with copy + **`title`**) until **`ended == true`** onchain — matching `TimeCurve.sol` **`require(ended, …)`** gates. | [§188](#timecurve-arena-settlement-panel-timer-expired-gitlab-188), [`TimeCurveArenaView.tsx`](../../frontend/src/pages/timeCurveArena/TimeCurveArenaView.tsx), [timecurve-views §188](../frontend/timecurve-views.md#arena-settlement-panel-timer-expired-gitlab-188), [manual QA (#188)](manual-qa-checklists.md#manual-qa-issue-188) |
| **TimeCurve responsive layout containment ([GitLab #201](https://gitlab.com/PlasticDigits/yieldomega/-/issues/201))** | **`INV-FRONTEND-201-RESPONSIVE-LAYOUT`:** On phone widths, the Simple buy panel must not reserve desktop coin-stack side padding; it hides the decorative cutout, collapses the slider/input row, and keeps `scrollWidth <= viewport`. On tablet widths, the Home product-card grid reflows to two columns before returning to three desktop columns, Arena WarBow **Guard** / **Counterpunch Revenge** cards wrap headings and revenge rows, and TimeCurve spotlight padding no longer squeezes the action grid outside its panel. Mobile TimeCurve routes keep primary content clear of the fixed Blockie Hills dock, and long hero / buy-panel numeric strings wrap inside their cards. | [§201](#timecurve-responsive-layout-gitlab-201), [design — Home grid](../frontend/design.md#home-product-card-grid-gitlab-198), [timecurve-views §201](../frontend/timecurve-views.md#timecurve-responsive-layout-gitlab-201), [`index.css`](../../frontend/src/index.css), [`RootLayout.tsx`](../../frontend/src/layout/RootLayout.tsx), [`timeCurveResponsiveLayoutCss.test.ts`](../../frontend/src/lib/timeCurveResponsiveLayoutCss.test.ts), [`timecurve.spec.ts`](../../frontend/e2e/timecurve.spec.ts), [manual QA (#201)](manual-qa-checklists.md#manual-qa-issue-201) |
| **TimeCurve — buy hub derived numeric display ([GitLab #191](https://gitlab.com/PlasticDigits/yieldomega/-/issues/191))** | **`INV-FRONTEND-191-BUY-HUB-DERIVED`:** On **Simple** and **Arena** buy checkout, **band** CL8Y (and routed pay hints), **CHARM preview**, **Worth at launch ≈** CL8Y, Arena **buy summary** chips, and Simple **recent buys** CL8Y/CHARM cells use **`formatBuyHubDerivedCompact`** (**4** significant figures). **Hero** per-CHARM tiles keep **fixed** fractional formatting for visible per-block ticks; **DOUB-at-launch** compact on the rate board stays **5** sigfigs. | [§191](#timecurve-buy-hub-derived-numeric-display-gitlab-191), [`timeCurveBuyHubFormat.ts`](../../frontend/src/lib/timeCurveBuyHubFormat.ts), [timecurve-views §191](../frontend/timecurve-views.md#timecurve-buy-hub-numeric-display-gitlab-191), [manual QA (#191)](manual-qa-checklists.md#manual-qa-issue-191) |
| **TimeCurve — buy projected-effects pills ([GitLab #227](https://gitlab.com/PlasticDigits/yieldomega/-/issues/227))** | **`INV-FRONTEND-227-BUY-PROJECTED`:** Simple **Projected effects** rail: one **`+{x}s`** timer pill (live hard-reset math), per-source **BP** pills, distinct defended-streak chips, **`-{3dp} asset`** spend, **Become Last Buyer**; no **time-booster credit** duplicate. | [§227](#timecurve-buy-projected-effects-gitlab-227), [`timeCurveBuyPreview.ts`](../../frontend/src/lib/timeCurveBuyPreview.ts), [timecurve-views §227](../frontend/timecurve-views.md#timecurve-buy-projected-effects-gitlab-227), [manual QA (#227)](manual-qa-checklists.md#manual-qa-issue-227) |
| **TimeCurve — USD equivalent hints ([GitLab #192](https://gitlab.com/PlasticDigits/yieldomega/-/issues/192))** | **`INV-FRONTEND-192-USD-AFFORDANCE`:** Visible **USD-shaped** aggregations on **`/timecurve`** routes (**Arena** hero **TOTAL USD** = **`totalRaised` × `CL8Y_USD_PRICE_PLACEHOLDER`**; **Simple** podium **≈ $… USD** = static **0.98×** CL8Y→USDM display shape via **`fallbackPayTokenWeiForCl8y`**) must surface **non-alarming** **staleness / basis** copy: **relative age** when the underlying **CL8Y** read or **prize preview** last changed, plus **`title`** tooltips stating the rate is **not** a live oracle. | [§192](#usd-equivalent-staleness-affordance-gitlab-192), [`cl8yUsdEquivalentDisplay.ts`](../../frontend/src/lib/cl8yUsdEquivalentDisplay.ts), [`TimeCurveArenaView.tsx`](../../frontend/src/pages/timeCurveArena/TimeCurveArenaView.tsx), [`TimeCurveSimplePodiumSection.tsx`](../../frontend/src/pages/timecurve/TimeCurveSimplePodiumSection.tsx), [timecurve-views §192](../frontend/timecurve-views.md#usd-equivalent-staleness-gitlab-192), [manual QA (#192)](manual-qa-checklists.md#manual-qa-issue-192) |
| **Protocol AUDIT — DOUB projection card ([GitLab #229](https://gitlab.com/PlasticDigits/yieldomega/-/issues/229), design pass [#235](https://gitlab.com/PlasticDigits/yieldomega/-/issues/235))** | **`INV-FRONTEND-229-DOUB-PROJECTION`:** **`/timecurve/protocol`** shows **`data-testid="timecurve-protocol-doub-projection"`** only when **`protocolPhase !== saleStartPending`**. Stats derive from onchain **`totalRaised` / `totalTokensForSale` / `totalCharmWeight` / `currentPricePerCharmWad`** via [`doubProjectionStats.ts`](../../frontend/src/lib/doubProjectionStats.ts) + [`timeCurvePodiumMath.ts`](../../frontend/src/lib/timeCurvePodiumMath.ts). **251M** policy supply; **implied market cap** uses **launch-anchor** CL8Y/DOUB (not clearing alone). **CHARM→DOUB** uses **`EmptyDataPlaceholder`** when **`totalCharmWeight == 0`** (**`INV-FRONTEND-200`**). **USD** uses **Kumbaya** CL8Y spot (manual **↻**). **Kumbaya floor** = **0.25×** anchor. Accordion **Reserve routing and launch anchors** must match clearing / launch / Kumbaya rows. **#235:** cards stay grouped by supply/redemption, price anchors, and market/wallet lens with phone-safe wrapping; no new reads or formulas. | [§229](#timecurve-protocol-doub-projection-gitlab-229), [`TimeCurveProtocolDoubProjectionSection.tsx`](../../frontend/src/pages/timecurve/TimeCurveProtocolDoubProjectionSection.tsx), [timecurve-views §229](../frontend/timecurve-views.md#timecurve-protocol-doub-projection-gitlab-229), [`doubProjectionStats.test.ts`](../../frontend/src/lib/doubProjectionStats.test.ts), [`doubProjectionLayoutCss.test.ts`](../../frontend/src/lib/doubProjectionLayoutCss.test.ts), [manual QA (#229/#235)](manual-qa-checklists.md#manual-qa-issue-229) |
| **Protocol AUDIT — Platform usage ([GitLab #231](https://gitlab.com/PlasticDigits/yieldomega/-/issues/231))** | **`INV-INDEXER-231-PLATFORM-USAGE`:** **`GET /v1/timecurve/platform-usage`** (schema **≥ 1.27.0** with **`sale`** velocity window; **≥ 1.26.0** baseline) returns network aggregates from **`idx_timecurve_buy`** + WarBow action tables; unique wallets = union of buy + WarBow participants; wallet page sorted by **`SUM(amount)`**; velocity filters buys by **`block_timestamp`** vs anchor (**chain_timer** head or **`MAX(block_timestamp)`**), or by **`block_timestamp >= sale_start_sec`** for **`velocity_window=sale`**. **`INV-FRONTEND-231-PLATFORM-USAGE`:** **`TimeCurveProtocolPlatformUsageSection`** on **`/timecurve/protocol`**; **`EmptyDataPlaceholder`** / muted copy when indexer unset or offline — no fake zeros. | [§231](#timecurve-platform-usage-gitlab-231), [`indexer/src/api.rs`](../../indexer/src/api.rs), [`integration_stage2.rs`](../../indexer/tests/integration_stage2.rs), [`TimeCurveProtocolPlatformUsageSection.tsx`](../../frontend/src/pages/timecurve/TimeCurveProtocolPlatformUsageSection.tsx), [manual QA (#231)](manual-qa-checklists.md#manual-qa-issue-231) |
| **Frontend — stats / charts empty states ([GitLab #200](https://gitlab.com/PlasticDigits/yieldomega/-/issues/200))** | **`INV-FRONTEND-200-EMPTY-STATS`:** Player-facing **stat cards**, **rate-board** slots, **standings visuals**, **referral CHARM totals**, and **stake-at-launch** tiles must not collapse to a bare **em dash** when data is loading, wallet-gated, or absent. Use **`EmptyDataPlaceholder`** + **`statFromContractRead` / `statFromOptionalString`** (Arena **`WhatMattersSection`**, standings **`StandingsVisuals`**, referral earnings banner, Simple podium prize preview, stake tiles) so copy reads as **intentional empty / loading** — checklist UX **5.12**. **`INV-FRONTEND-200-NO-INDEXER-PILL`:** Simple **Live reserve podiums** omit the former **`PageSection` `actions`** pill (**“Indexer-backed snapshot”** / **“Refreshing podiums…”**) — contributor-only copy that flickered on refetch ([GitLab #200](https://gitlab.com/PlasticDigits/yieldomega/-/issues/200)). | [§200](#frontend-stats-charts-empty-states-gitlab-200), [timecurve-views §200](../frontend/timecurve-views.md#stats-charts-empty-states-gitlab-200), [`EmptyDataPlaceholder.tsx`](../../frontend/src/components/EmptyDataPlaceholder.tsx), [`statDisplayFromContractRead.tsx`](../../frontend/src/lib/statDisplayFromContractRead.tsx), [`TimeCurveSimplePodiumSection.tsx`](../../frontend/src/pages/timecurve/TimeCurveSimplePodiumSection.tsx) |
| **TimeCurve — WarBow ladder snapshot drift + **`distributePrizes`** latch (#129)** | **GitLab #172:** automatic onchain WarBow podium maintenance and **`refreshWarbowPodium`** are removed. **`finalizeWarbowPodium(first, second, third)`** is **`onlyOwner`**, **`ended && !prizesDistributed`**, validates **left-packed** ranks with **strict live `battlePoints`** ordering (including trailing **`address(0)`** rules), sets **`warbowPodiumFinalized`**, emits **`WarbowPodiumFinalized`**. When **`reservePodiumPayoutsEnabled`** and **`acceptedAsset.balanceOf(podiumPool) > 0`**, **`distributePrizes`** requires **`warbowPodiumFinalized`** before paying (**zero** balance skips WarBow finalize — [GitLab #133](#timecurve-empty-podium-pool-settlement-gitlab-133)). **`/timecurve/protocol`** loads indexer **`GET /v1/timecurve/warbow/refresh-candidates`** as a **reference** list for governance ordering (**`INV-INDEXER-160-WARBOW-REFRESH-CANDIDATES`**, **`INV-WARBOW-172-*`**). **Harness:** [`SimulateAnvilRichState.s.sol`](../../contracts/script/SimulateAnvilRichState.s.sol), [`verify-timecurve-post-end-gates-anvil.sh`](../../scripts/verify-timecurve-post-end-gates-anvil.sh). | [§ #129](#warbow-podium-snapshot-drifts-gitlab-129), [§ #149](#gitlab-149-warbow-arena-indexer-hardening), [§ #133](#timecurve-empty-podium-pool-settlement-gitlab-133), [issue #129](https://gitlab.com/PlasticDigits/yieldomega/-/issues/129), [GitLab #172](https://gitlab.com/PlasticDigits/yieldomega/-/issues/172), [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md), [timecurve-views — #129](../frontend/timecurve-views.md#warbow-ladder-podium-snapshot-mismatch-issue-129) |
| **TimeCurve Simple — live reserve podiums (issue #113)** | **`INV-FRONTEND-113-PODIUM-UX`:** `/timecurve` shows a compact read-only `PageSection` immediately above **Recent buys / Live ticker** with all four fixed v1 reserve categories. With **`VITE_INDEXER_URL`**, winner addresses come from **`GET /v1/timecurve/podiums`** (DB-backed live predictions while the sale is active — see **`INV-INDEXER-PODIUM-PREDICT-LIVE`**); without the indexer, `usePodiumReads` falls back to **`TimeCurve.podium(category)`** RPC. **`INV-FRONTEND-113-PODIUM-FALLBACK`:** Empty winner slots use a neutral **em dash** (or equivalent empty affordance), not wallet-connect wording, when no address is available for that slot. The frontend uses `PODIUM_CONTRACT_CATEGORY_INDEX` when calling the contract directly; `Buy` logs invalidate podium cache; prize CL8Y hints stay client-derived from **`PodiumPool`** balance. | [timecurve-views — Simple live podiums](../frontend/timecurve-views.md), [`TimeCurveSimplePodiumSection.tsx`](../../frontend/src/pages/timecurve/TimeCurveSimplePodiumSection.tsx), [`usePodiumReads.ts`](../../frontend/src/pages/timecurve/usePodiumReads.ts), [`TimeCurveSimplePodiumSection.test.tsx`](../../frontend/src/pages/timecurve/TimeCurveSimplePodiumSection.test.tsx), [`timecurve.spec.ts`](../../frontend/e2e/timecurve.spec.ts), [manual QA](manual-qa-checklists.md#manual-qa-issue-113), [§113](#timecurve-simple-live-reserve-podiums-issue-113), [§ indexer podiums](#indexer-timecurve-podiums-live-prediction) |
| **Time Booster podium score display ([GitLab #228](https://gitlab.com/PlasticDigits/yieldomega/-/issues/228))** | **`INV-FRONTEND-228-TIMEBOOSTER-FMT`:** Simple reserve podium score lines and Arena podium spotlight leader values for **Time Booster** (`categoryIndex === 3`) format cumulative **`totalEffectiveTimerSecAdded`** via shared **`formatTimeBoosterPodiumSec`** — locale integer **&lt; 60 s**, **`mm:ss`** **60 s – 59 m 59 s**, **`HH:MM:SS`** **1 h – 23 h 59 m 59 s**, **`d:HH:MM:SS`** **≥ 24 h** — not raw **`Ns`** / **`N s`**. Zero-second slots use **`Score: —`** on Simple. Display-only; indexer/API/onchain payloads unchanged. | [§228](#time-booster-podium-tiered-duration-gitlab-228), [`timeBoosterPodiumFormat.ts`](../../frontend/src/pages/timecurve/timeBoosterPodiumFormat.ts), [timecurve-views §228](../frontend/timecurve-views.md#time-booster-podium-tiered-duration-gitlab-228), [manual QA (#228)](manual-qa-checklists.md#manual-qa-issue-228) |
| **Indexer — `GET /v1/timecurve/podiums` live prediction (schema ≥ 1.20.0)** | **`INV-INDEXER-PODIUM-PREDICT-LIVE`:** While **`sale_ended`** is **false** in the head **`chain_timer`** snapshot, **all four** podium rows returned to the frontend must reflect **best-effort indexed state** (last-three buys for Last Buy; WarBow BP from latest **`idx_timecurve_buy`** + WarBow tables for per-wallet BP; defended streak from buys + **`idx_timecurve_warbow_ds_continued`**; time booster from latest **`buyer_total_effective_timer_sec`** per buyer), ordered like onchain monotonic rules (**metric desc**, **address asc** tie-break). When **`sale_ended`** is **true**, each row mirrors **`podium(category)`** at **`read_block_number`**. Each row includes **`podium_prediction`**; Last Buy also keeps **`last_buy_prediction`** during the live sale for backward compatibility. | [`indexer/src/api.rs`](../../indexer/src/api.rs) (`timecurve_podiums`), [§113](#timecurve-simple-live-reserve-podiums-issue-113), [design — podiums HTTP](../indexer/design.md#timecurve-podiums-http) |
...
| **Anvil E2E (Playwright) — pay mode + workers ([issue #87](https://gitlab.com/PlasticDigits/yieldomega/-/issues/87))** | **`/arena`** buy panel chooses pay asset with **toggle buttons** (`aria-label` “Show price in …”), not legacy **radio** inputs. Stable hooks: **`data-testid="arena-paywith-cl8y"`** / **`eth`** / **`usdm`**. E2E must not depend on removed **`input[name="timecurve-pay-with"]`**. With **`ANVIL_E2E=1`**, `frontend/playwright.config.ts` sets **`workers: 1`** and **`fullyParallel: false`**: one Anvil + one default mock account → **no cross-file parallel wallet txs** (nonce / sale / referral races). Unrelated: default CI **`npm run test:e2e`** (no Anvil) may use **5** workers. | [e2e-anvil.md](e2e-anvil.md), [`anvil-arena-wallet-writes.spec.ts`](../../frontend/e2e/anvil-arena-wallet-writes.spec.ts), [`playwright.config.ts`](../../frontend/playwright.config.ts), [§ Anvil E2E Playwright](#anvil-e2e-playwright-concurrency-and-pay-mode-selectors-issue-87) |
...
<a id="placeholder-split-layout-figure-gitlab-163"></a>

### Placeholder split-layout hero figure (GitLab #163)

**Intent:** [GitLab #163](https://gitlab.com/PlasticDigits/yieldomega/-/issues/163) — on **`/rabbit-treasury`**, **`/collection`**, **`/kumbaya`**, and **`/sir`**, the left **scene card** (`.placeholder-figure` + `<img>`) shared a **two-column** **`.split-layout`** row with **`PageSection`**. Grid items **stretch** to the row’s block size; the image did **not**, so users saw **empty panel** inside the border and misaligned chrome on **large width** and **landscape** layouts.

**Invariant (`INV-FRONTEND-163`):** **`.split-layout > .placeholder-figure`** uses **`align-self: start`** so the bordered frame **shrink-wraps** the image; **`width: 100%`** + **`max-width: min(42rem, 100%)`** uses the column up to the same cap as **`placeholder-figure--wide`**; **`min-width: 0`** avoids grid overflow glitches.

**Automated:** [`frontend/src/lib/placeholderSplitLayoutCss.test.ts`](../../frontend/src/lib/placeholderSplitLayoutCss.test.ts) (parses **`index.css`** for the rule block). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-163`](manual-qa-checklists.md#manual-qa-issue-163). **Docs:** [design.md — Placeholder split panels](../frontend/design.md#placeholder-split-panels-gitlab-163).
...
**Intent:** [GitLab #87](https://gitlab.com/PlasticDigits/yieldomega/-/issues/87) — `bash scripts/e2e-anvil.sh` runs **one** Anvil, **one** preview server, and **`e2e/anvil-arena-*.spec.ts`** with **`ANVIL_E2E=1`**. `test.describe.configure({ mode: "serial" })` inside a file does **not** prevent **other** spec files from running in parallel, so a multi-worker run can interleave **writes** to the same chain and account. **Invariant:** when **`ANVIL_E2E=1`**, use **`workers: 1`** and disable **`fullyParallel`** for Playwright. **Pay mode:** E2E selects ETH (or other assets) via **`getByTestId("arena-paywith-eth")`** (and siblings **`…-cl8y`**, **`…-usdm`**) on **`/arena`**. **Do not** reintroduce `input[name="timecurve-pay-with"]` without restoring that markup. **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-87`](manual-qa-checklists.md#manual-qa-issue-87).
...
### LaunchGate `/home` hub route — no-env parity (GitLab #199)

**Intent:** [GitLab #199](https://gitlab.com/PlasticDigits/yieldomega/-/issues/199) — QA opened **`/home`** on a **no-env** dev build (`VITE_LAUNCH_TIMESTAMP` unset) and saw **`RootLayout`** chrome with an **empty** `<Outlet />` (green shell / grid only). **`ROUTES_NO_ENV`** listed **`HomePage`** only on the **index** route, so **`path="home"`** had **no** matching child.

**Invariant (`INV-FRONTEND-199-HOME-ROUTE`):** When the launch gate is a no-op, **`ROUTES_NO_ENV`** includes **`path: "home"`** with the same **`HomePage`** element as **`/`** (hub alias). **Post-launch** remains **`/` → `TimeCurveSimplePage`**, **`/home` → `HomePage`**. **No** second `BrowserRouter`; **no** duplicate layout — only the lazy hub surface mounts in the outlet.

**Automated:** [`frontend/e2e/launch-countdown.spec.ts`](../../frontend/e2e/launch-countdown.spec.ts) (`no-env routing` asserts **`/home`** shows the **`YieldOmega`** `h1`). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-199-home-route-launchgate`](manual-qa-checklists.md#manual-qa-issue-199-home-route-launchgate). **Docs:** [`timecurve-views.md` — LaunchCountdown → Simple handoff](../frontend/timecurve-views.md#launchcountdown--simple-handoff).

<a id="branded-404-catch-all-gitlab-223"></a>

<a id="timecurve-protocol-doub-projection-gitlab-229"></a>

### TimeCurve protocol — DOUB projection card (GitLab #229, design pass #235)

**Intent:** [GitLab #229](https://gitlab.com/PlasticDigits/yieldomega/-/issues/229) + design pass [#235](https://gitlab.com/PlasticDigits/yieldomega/-/issues/235) — operators scanning **`/timecurve/protocol`** (AUDIT) get a prominent, grouped **DOUB projection** stat grid without expanding **Raw contract and operator context**.

**Invariant (`INV-FRONTEND-229-DOUB-PROJECTION`):**

| Rule | Detail |
|------|--------|
| **Visibility** | Section renders **only** when sale phase is **not** **`saleStartPending`** (hidden pre-open per product sign-off). |
| **Supply constant** | **251M** whole DOUB (200M + 21.5M + 28.5M + **1M airdrops**); [`PROJECTED_DOUB_SUPPLY_WHOLE`](../../frontend/src/lib/doubProjectionStats.ts). |
| **Market cap** | **`projectedSupplyWad × launchLiquidityAnchorWad(clearing) / 1e18`** — launch anchor (**1.275×** clearing), not clearing alone. |
| **CHARM→DOUB** | **`doubPerCharmAtLaunchWad`**; **`undefined` → `EmptyDataPlaceholder`** when no CHARM minted (**#200**). |
| **Parity** | Clearing / launch / Kumbaya lower match raw accordion **Reserve routing and launch anchors**. |
| **USD** | **Kumbaya USDM** spot per 1 CL8Y; load on mount, manual refresh only. **Kumbaya floor** = **0.25×** launch anchor. |
| **Readability (#235)** | The ten cards are grouped into **Supply and redemption**, **Price anchors**, and **Market and wallet lens** with responsive wrapping; this is visual structure only. |

**Spec ↔ test:** [`doubProjectionStats.test.ts`](../../frontend/src/lib/doubProjectionStats.test.ts) · [`doubProjectionLayoutCss.test.ts`](../../frontend/src/lib/doubProjectionLayoutCss.test.ts) · [`timecurve.spec.ts`](../../frontend/e2e/timecurve.spec.ts) · [timecurve-views §229](../frontend/timecurve-views.md#timecurve-protocol-doub-projection-gitlab-229) · [manual QA (#229/#235)](manual-qa-checklists.md#manual-qa-issue-229).

<a id="timecurve-platform-usage-gitlab-231"></a>

### TimeCurve protocol — Platform usage (GitLab #231)

**Intent:** [GitLab #231](https://gitlab.com/PlasticDigits/yieldomega/-/issues/231) — operators on **`/timecurve/protocol`** see network-wide sale + WarBow participation without expanding the raw accordion.

**Invariant (`INV-INDEXER-231-PLATFORM-USAGE`):**

| Rule | Detail |
|------|--------|
| **Route** | **`GET /v1/timecurve/platform-usage`**; query **`limit` / `offset`** (clamp **1–200**), **`velocity_window`** **`1h`** (default), **`24h`**, or **`sale`** (whole-sale window, [GitLab #233](https://gitlab.com/PlasticDigits/yieldomega/-/issues/233)). |
...
| **Velocity** | Count buys with **`block_timestamp >= anchor - window`** (**`1h`** / **`24h`**) or **`block_timestamp >= sale_start_sec`** (**`sale`**); **`avg_buys_per_hour = count / window_hours`** where **`sale`** derives **`window_hours = floor((anchor - sale_start) / 3600)`** (min **1**). Anchor: head **`chain_timer.block_timestamp_sec`**, else **`MAX(block_timestamp)`** on buys. **`sale_start`:** head **`chain_timer.sale_start_sec`**, else latest **`idx_timecurve_sale_started.start_timestamp`**. Partial index **`idx_timecurve_buy_block_timestamp`** on **`block_timestamp`** (migration **`20260522120000_idx_timecurve_buy_block_timestamp_gl233`**). Sale-window pre-open (**`sale_start = 0`** or **`anchor < sale_start`**) returns zero buys without divide-by-zero. **`wallets.total`** reuses summary **`unique_buyers`** (no second **`COUNT(DISTINCT buyer)`**). |

**Invariant (`INV-FRONTEND-231-PLATFORM-USAGE`):**
...
**Intent:** [GitLab #223](https://gitlab.com/PlasticDigits/yieldomega/-/issues/223) — unknown client routes previously matched **`RootLayout`** with an **empty** `<Outlet />`. **`ShellRoutes`** now ends with **`path="*"`** → **`NotFoundPage`** (on-brand copy, CTAs to TimeCurve, hub, Referrals).

**Invariant (`INV-FRONTEND-223-NOT-FOUND`):**

| URL | Expected |
|-----|----------|
| `/nope`, `/luck777`, `/timecurve/foo/bar`, `/referrals/extra` | **`NotFoundPage`** inside shell |
| `/timecurve`, `/timecurve/arena`, `/timecurve/protocol`, `/timecurve/{validReferral}` | Existing surfaces — **no** 404 |
| `/timecurve/{invalidSlug}` (too short, reserved, bad charset) | **TimeCurve Simple** without path capture (unchanged) |
| Unknown path + valid **`?ref=`** | 404 UI + pending **`yieldomega.ref.v1`** capture |

**Automated:** [`navigation.spec.ts`](../../frontend/e2e/navigation.spec.ts), [`referral-path.spec.ts`](../../frontend/e2e/referral-path.spec.ts). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-223`](manual-qa-checklists.md#manual-qa-issue-223). **Product:** [`referrals.md` — Client link capture](../product/referrals.md#client-link-capture-frontend).
...
### Arena `Buy CHARM` wrong-chain visual affordance (GitLab #194)

**Intent:** [GitLab #194](https://gitlab.com/PlasticDigits/yieldomega/-/issues/194) — the Arena **arcade** **`Buy CHARM`** CTA already included **`chainMismatch`** in its **`disabled`** cascade, but the **gold / motion** treatment could still read as “live” compared with **`btn-secondary`** post-end CTAs under the same wrong-network state.

**Invariant (`INV-FRONTEND-194-ARENA-BUY-CHAIN`):** When **`useWalletTargetChainMismatch()`** is true, the Arena buy CTA adds **`timecurve-simple__cta--wrong-network`**, sets **`title`** from **`chainMismatchWriteMessage(walletChainId)`**, omits Framer **`primaryButtonMotion`** (no bounce / lift), and relies on scoped CSS for **stronger dimming** plus a **raised** buy-panel **`ChainMismatchWriteBarrier`** overlay **`z-index`** so the gate paints above z-indexed panel layers. **`handleBuy`** wrong-chain and **#144** drift defences stay unchanged.

**Automated:** none required (visual + stacking). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-194-arena-buy-chain-visual`](manual-qa-checklists.md#manual-qa-issue-194-arena-buy-chain-visual). **Docs:** [`timecurve-views.md` §194](../frontend/timecurve-views.md#arena-buy-charm-wrong-chain-visual-gitlab-194); summary table: [`#inv-frontend-194-arena-buy-chain`](#inv-frontend-194-arena-buy-chain).

<a id="frontend-erc20-approval-sizing-gitlab-143"></a>

### Frontend ERC-20 approval sizing (GitLab #143)

**Intent:** [GitLab #143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143) — default **`approve(spender, exactNeed)`** on swap / referral / CL8Y→TimeCurve paths instead of silent **`type(uint256).max`**, reducing allowance blast radius if a **malicious proxy upgrade** ever reused an existing grant (audit **H-01** in `audits/audit_smartcontract_1777813071.md`). **Invariant (`INV-ERC20-APPROVAL-143`):** Kumbaya **WETH / USDM → `swapRouter`** and **USDM → `TimeCurveBuyRouter`** use slippage-bounded **`maxIn`**. **`ReferralRegistry.registerCode`** approves **`registrationBurnAmount`** exactly. **CL8Y → `TimeCurve`** uses the **pending tx** gross unless the user opts in via **`Cl8yTimeCurveUnlimitedApprovalFieldset`** (**`yieldomega.erc20.cl8yTimeCurveUnlimited.v1`**). WarBow steal / guard / revenge share the same CL8Y→TimeCurve sizing read. **Automated:** [`cl8yTimeCurveApprovalPreference.test.ts`](../../frontend/src/lib/cl8yTimeCurveApprovalPreference.test.ts), [`ensureCl8yTimeCurveAllowance.test.ts`](../../frontend/src/lib/ensureCl8yTimeCurveAllowance.test.ts) (**[#224](#arena-cl8y-approve-guard-gitlab-224)**). **Docs:** [`wallet-connection.md` §143](../frontend/wallet-connection.md#erc20-approval-sizing-h-01-gitlab-143), [`timecurve-views` §143](../frontend/timecurve-views.md#erc20-approval-sizing-gitlab-143).

<a id="arena-cl8y-approve-guard-gitlab-224"></a>

### Arena / Simple — skip redundant CL8Y → TimeCurve approve (GitLab #224)

**Intent:** [GitLab #224](https://gitlab.com/PlasticDigits/yieldomega/-/issues/224) — Arena **WarBow** paths previously gated approval on **`allow < need`** while **buy** used **`allow < approveAmt`**, causing **spurious `approve` txs** (wallet reject / simulation noise) when allowance was already **`type(uint256).max`** or otherwise covered the **sized** target from [#143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143).

**Invariant (`INV-FRONTEND-224-CL8Y-APPROVE`):**

| Check | Detail |
|-------|--------|
| **Shared helper** | [`ensureCl8yTimeCurveAllowance`](../../frontend/src/lib/ensureCl8yTimeCurveAllowance.ts) — **`planCl8yTimeCurveApprove`** + on-chain **`allowance` read**. |
| **Guard** | Submit **`approve(TimeCurve, approveAmt)`** only when **`allow < approveAmt`** where **`approveAmt = cl8yTimeCurveApprovalAmountWei(needWei, unlimitedPref)`**. |
| **Call sites** | Arena **`handleBuy`**, WarBow steal/guard/revenge (**`ensureTcAllowance`**), Simple **`submitBuy`**. |
| **Debug** | With Kumbaya buy debug enabled, logs **`cl8y-timecurve-approve:skip`** / **`:submit`** with **`allow` / `needWei` / `approveAmt`**. |

**Automated:** [`ensureCl8yTimeCurveAllowance.test.ts`](../../frontend/src/lib/ensureCl8yTimeCurveAllowance.test.ts). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-224`](manual-qa-checklists.md#manual-qa-issue-224). **Docs:** [wallet-connection §143](../frontend/wallet-connection.md#erc20-approval-sizing-h-01-gitlab-143), [timecurve-views §224](../frontend/timecurve-views.md#arena-cl8y-approve-guard-gitlab-224).
...
| Invariant | Check |
|-----------|-------|
| **Hero visibility** | `PageHeroArcadeBanner` renders `WarbowHeroActions` with wallet context and **Steal / Guard / Revenge** controls; **Claim flag** when the viewer holds the pending slot ([GitLab #218](https://gitlab.com/PlasticDigits/yieldomega/-/issues/218)). |
...
| **Empty state** | No suggested target renders a clear empty state; manual steal address entry lives in the unmounted `WarbowSection` component (Arena route uses hero actions only). |
...
**Automated:** [`WarbowHeroActions.test.tsx`](../../frontend/src/pages/timeCurveArena/WarbowHeroActions.test.tsx) · [`WarbowClaimFlagHeroCard.test.tsx`](../../frontend/src/pages/timeCurveArena/WarbowClaimFlagHeroCard.test.tsx).
**Docs / play:** [timecurve-views — issue #101](../frontend/timecurve-views.md#arena-warbow-hero-actions-issue-101) · [timecurve-views — claim flag (#218)](../frontend/timecurve-views.md#arena-warbow-claim-flag-hero-gitlab-218) · [invariants — per-stealer revenge (#135)](#warbow-per-stealer-revenge-windows-gitlab-135) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).

<a id="arena-warbow-claim-flag-hero-gitlab-218"></a>

### Arena WarBow claim flag hero (GitLab #218)

**Intent:** Users who **plant** a WarBow flag (Simple buy ADVANCED checkbox or router **`plantWarBowFlag`**) must have a wallet path to **`claimWarBowFlag()`** on **`/timecurve/arena`** without remounting the full **`WarbowSection`**.

| Invariant (`INV-FRONTEND-218-WARBOW-CLAIM-FLAG`) | Check |
|-----------|--------|
| **Visibility** | **`WarbowClaimFlagHeroCard`** renders **only** when connected wallet === **`warbowPendingFlagOwner`** and **`warbowPendingFlagPlantAt > 0`**. |
| **Silence countdown** | While **`ledgerNow < plantAt + WARBOW_FLAG_SILENCE_SEC`**, CTA is **disabled** and shows **`Claim flag HH:MM:SS`** using **`effectiveLedgerSec`** (same ledger basis as phase / guard). |
...
<a id="timecurve-arena-warbow-steal-victim-validation-gitlab-195"></a>

### Arena WarBow steal victim field — scoped validation (GitLab #195)

**Intent:** Victim-address validation for **Attempt steal** must not read as a global Arena error on idle load. Empty input must not surface steal-field copy in the section-wide **`warbowActionHint`** strip or duplicate into **`pvpErr`** banners reserved for wrong-chain, paused routing, and onchain revert summaries.

| Invariant (`INV-FRONTEND-195-WARBOW-STEAL-FORM`) | Check |
|-----------|--------|
| **Idle / empty field** | Connected wallet, live sale, **empty** `stealVictimInput`: no victim-validation **`StatusMessage`** under the input; **`warbowActionHint`** does not mention invalid victim hex. |
| **Invalid partial address** | Non-empty input that fails `isAddress` shows **`Enter a valid victim address`** only in **`WarbowSection`** immediately under the victim field (`data-testid="warbow-steal-victim-form-status"`), via **`warbowStealVictimInputFormatError`** in [`warbowStealVictimInput.ts`](../../frontend/src/lib/warbowStealVictimInput.ts). |
| **Empty + Attempt steal** | **`runWarBowSteal`** with no resolved victim sets **`arenaWarbowStealFormErr`** to the empty-attempt copy (`WARBOW_STEAL_VICTIM_EMPTY_ATTEMPT` in code), not **`pvpErr`**. |
| **Clear on edit** | **`arenaWarbowStealFormErr`** clears when **`stealVictimInput`** changes. |

**Automated:** [`warbowStealVictimInput.test.ts`](../../frontend/src/lib/warbowStealVictimInput.test.ts).

**Docs / play:** [timecurve-views — steal victim field (#195)](../frontend/timecurve-views.md#arena-warbow-steal-victim-field-gitlab-195) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md) · [GitLab #195](https://gitlab.com/PlasticDigits/yieldomega/-/issues/195).

<a id="timecurve-arena-warbow-indexer-refresh-gitlab-182"></a>

### TimeCurve Arena — WarBow indexer leaderboard + battle-feed refresh (GitLab #182)

**Intent:** [GitLab #182](https://gitlab.com/PlasticDigits/yieldomega/-/issues/182) — **`warbowLb`** / **`warbowFeed`** from the indexer were loaded **once** on mount; **Your WarBow rank**, chasing-pack list, and rivalry feed stayed frozen until a full reload even though wagmi **`battlePoints`** reads updated.

**Invariant (`INV-FRONTEND-182-WARBOW-IDX`):**

| Check | Detail |
|-------|--------|
| **Parallel fetch** | **`refreshWarbowIndexerAggregates`** requests **`fetchTimecurveWarbowLeaderboard(12, 0)`** and **`fetchTimecurveWarbowBattleFeed(20, 0)`** together. |
| **Connectivity** | When **`indexerBaseUrl()`** is set, **`reportIndexerFetchAttempt(ok)`** runs with **`ok`** iff **both** responses are non-null (same class as other indexer polls). |
| **`refetchAll` hook** | **`refetchAll`** invokes **`void refreshWarbowIndexerAggregates()`** so successful receipt paths already calling **`refetchAll`** (Arena buy, WarBow steal/guard/revenge/flag claim, **`runVoid`**) refresh indexer aggregates **without** waiting for the poll tick. |
| **Poll** | While **`VITE_INDEXER_URL`** is configured, a remount-safe loop schedules **`getIndexerBackoffPollMs(1500)`** after each completion so external ladder / feed activity converges without navigation. |
| **BP-moving logs** | **`useWarbowBpMovingEventWatch`** refetches leaderboard + feed (and invalidates Simple podium query) on **`Buy`**, **`WarBowSteal`**, **`WarBowRevenge`**, **`WarBowFlagClaimed`**, and **`WarBowFlagPenalized`** — see **`INV-FRONTEND-WARBOW-PODIUM-LIVE`**. |
| **Race guard** | Monotonic sequence ref invalidates older **`Promise.all`** completions after unmount or a newer refresh. |

**Automated:** `frontend` **`npm test`** (Vitest). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-182`](manual-qa-checklists.md#manual-qa-issue-182).

**Docs / play:** [timecurve-views — #182](../frontend/timecurve-views.md#arena-warbow-indexer-leaderboard-feed-refresh-gitlab-182) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).

<a id="live-warbow-podium-simple-arena-gitlab-warbow-podium-live"></a>

### Live WarBow podium — Simple + Arena (`INV-FRONTEND-WARBOW-PODIUM-LIVE`)

**Intent:** Simple **WarBow** reserve podium scores and Arena chasing-pack BP must reflect **live Battle Points** across buys **and** PvP evidence (steal / revenge / flag), not lag until the next **`Buy`** or show indexer-only digits that disagree with **`battlePoints(address)`**.

**Invariant (`INV-FRONTEND-WARBOW-PODIUM-LIVE`):**
...
**Intent:** [GitLab #189](https://gitlab.com/PlasticDigits/yieldomega/-/issues/189) — the **Chasing pack** list was **hard-capped** at **six** indexer rows (**`slice(0, 6)`**), so **seventh+** wallets on the ladder never appeared and QA could not stress layout for **7+** participants (orthogonal to per-stealer revenge [#135](#warbow-per-stealer-revenge-windows-gitlab-135)).

**Invariant (`INV-FRONTEND-189-WARBOW-CHASING-PACK`):**

| Check | Detail |
|-------|--------|
| **No silent cap** | **`warbowLeaderboardRows`** maps **`warbowLeaderboardForChasingPackDisplay(warbowLb)`** — the **full** **`GET /v1/timecurve/warbow/leaderboard`** page the client keeps (same source as **`warbowRank`** / steal candidates). |
| Overflow | **`WarbowSection`** wraps the **`RankingList`** in **`.warbow-chasing-pack-scroll`**: bounded **`max-height`** + **`overflow-y: auto`** so long ladders do not break **`.split-layout`** beside **Top rivals**. |
| **Harness** | **`data-testid="warbow-chasing-pack-scroll"`** on the scroll region for E2E / manual verification. |

**Automated:** [`warbowChasingPackLeaderboard.test.ts`](../../frontend/src/pages/timeCurveArena/warbowChasingPackLeaderboard.test.ts). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-189`](manual-qa-checklists.md#manual-qa-issue-189).

**Docs / play:** [timecurve-views — #189](../frontend/timecurve-views.md#arena-warbow-chasing-pack-scroll-gitlab-189) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).

<a id="timecurve-arena-settlement-panel-timer-expired-gitlab-188"></a>

### TimeCurve Arena — settlement panel when timer expired (GitLab #188)

**Intent:** [GitLab #188](https://gitlab.com/PlasticDigits/yieldomega/-/issues/188) — QA expected a **Claim** path near podiums after the round; contractually, **`redeemCharms`** requires **`TimeCurve.ended()`**, but **`saleExpiredAwaitingEnd`** (timer past **`deadline`**, **`ended` still false**) previously kept the **Standings** layout so **End sale** never appeared beside **Redeem** / **Distribute**.

**Invariant (`INV-FRONTEND-188-ARENA-SETTLEMENT`):**

| Check | Detail |
|-------|--------|
| **Panel visible** | **`saleExpiredAwaitingEnd \|\| saleEnded`** swaps the status **`PageSection`** into the settlement **action row** (not only when **`saleEnded`**). |
| **End sale** | Rendered while **`ended == false`**; **`data-testid="timecurve-arena-end-sale"`**. |
| **Redeem / Distribute** | Always visible in that row; **disabled** with **`title` + muted copy** until **`ended == true`** (matches onchain **`require(ended)`**). |
| **What matters** | Summary cards + **`WhatMattersSection`** use settlement chrome for **`timerExpiredAwaitingEnd`** (preview copy, **`distributeHint`** **End the sale first.**). |

**Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-188`](manual-qa-checklists.md#manual-qa-issue-188).

**Docs / play:** [timecurve-views — #188](../frontend/timecurve-views.md#arena-settlement-panel-timer-expired-gitlab-188) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).

<a id="timecurve-buy-hub-derived-numeric-display-gitlab-191"></a>

### TimeCurve buy hub — derived numeric display (GitLab #191)

**Intent:** [GitLab #191](https://gitlab.com/PlasticDigits/yieldomega/-/issues/191) — adjacent **CL8Y / CHARM / launch** numbers on the buy checkout must not mix **arbitrary** sig-fig policies (e.g. **3** vs **4** on the same card). Umbrella visual QA: [#164](https://gitlab.com/PlasticDigits/yieldomega/-/issues/164) §6.

**Invariant (`INV-FRONTEND-191-BUY-HUB-DERIVED`):**

| Check | Detail |
|-------|--------|
| **Shared formatter** | **`formatBuyHubDerivedCompact`** (`TIMECURVE_BUY_HUB_DERIVED_SIGFIGS = 4`) in [`timeCurveBuyHubFormat.ts`](../../frontend/src/lib/timeCurveBuyHubFormat.ts) drives **live band** strings, **CHARM preview**, **Worth at launch ≈**, Arena **`buyProjectedEffects`** CHARM/CL8Y lines, and Simple **recent buys** CL8Y/CHARM columns. |
| **Arena parity** | **`useTimeCurveArenaModel`** exposes **`buyAddsCl8yAtLaunch`** (same **`participantLaunchValueCl8yWei`** as Simple) so Arena shows **Worth at launch ≈** beside **You add** CHARM — [`useTimeCurveArenaModel.tsx`](../../frontend/src/pages/timeCurveArena/useTimeCurveArenaModel.tsx), [`TimeCurveArenaView.tsx`](../../frontend/src/pages/timeCurveArena/TimeCurveArenaView.tsx). |
| **Hero tiles unchanged** | **Price now** / **1 CHARM at launch** hero values keep **`formatPriceFixed6`** / **`formatEthRateHero`** so sub‑wei ticks remain visible — documented in [timecurve-views §191](../frontend/timecurve-views.md#timecurve-buy-hub-numeric-display-gitlab-191). |
| **DOUB tile** | Rate-board **DOUB per CHARM at launch** stays **`formatCompactFromRaw(..., { sigfigs: 5 })`** (redemption density UX). |

**Automated:** [`timeCurveBuyHubFormat.test.ts`](../../frontend/src/lib/timeCurveBuyHubFormat.test.ts). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-191`](manual-qa-checklists.md#manual-qa-issue-191).

**Docs / play:** [timecurve-views — #191](../frontend/timecurve-views.md#timecurve-buy-hub-numeric-display-gitlab-191) · [`skills/README.md`](../../skills/README.md) (TimeCurve row).

<a id="timecurve-buy-projected-effects-gitlab-227"></a>

### TimeCurve buy projected-effects pills (GitLab #227)

**Intent:** [GitLab #227](https://gitlab.com/PlasticDigits/yieldomega/-/issues/227) — Simple buy **Projected effects** must show **numeric, per-source** chips that track the hero countdown (especially under the **13m** hard-reset band), not prose like **“Hard-reset timer toward 15m”** or combined BP strings.

**Invariant (`INV-FRONTEND-227-BUY-PROJECTED`):**

| Check | Detail |
|-------|--------|
| **Timer pill** | **`previewBuyTimerSecondsAdded`** ([`timeCurveBuyPreview.ts`](../../frontend/src/lib/timeCurveBuyPreview.ts)) → single **`+{x}s`** when **`x > 0`**; **`x`** recomputes every hero-timer tick; under **780s** remaining uses hard-reset math (**`900 − remaining`**, cap-aware). |
| **No time-booster duplicate** | Rail must **not** emit **`time-booster credit`** (podium Time Booster remains [#228](#time-booster-podium-tiered-duration-gitlab-228)). |
| **BP decomposition** | **`previewWarbowBuyEffects`** emits separate **`+{n} BP {label}`** pills (Base / Reset / Clutch / Streak break / Ambush). |
| **Defended streak** | Distinct **continue / break / start** copy; holder from **`recentBuys`** when in **&lt;15m** window — document staleness if indexer head ≠ onchain **`_dsLastUnderWindowBuyer`**. |
| **Spend chip** | **`formatBuyProjectedSpendLine`**: **`-{3dp} {CL8Y\|ETH\|USDM}`**. |
| **Last buyer** | **`Become Last Buyer`**. |

**Automated:** [`timeCurveBuyPreview.test.ts`](../../frontend/src/lib/timeCurveBuyPreview.test.ts) · [`timeCurveBuyProjectedEffects.test.ts`](../../frontend/src/pages/timecurve/timeCurveBuyProjectedEffects.test.ts). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-227`](manual-qa-checklists.md#manual-qa-issue-227).

**Docs / play:** [timecurve-views — #227](../frontend/timecurve-views.md#timecurve-buy-projected-effects-gitlab-227) · [`play-timecurve-doubloon/SKILL.md`](../../skills/play-timecurve-doubloon/SKILL.md).

<a id="usd-equivalent-staleness-affordance-gitlab-192"></a>

### USD equivalent staleness + affordance (GitLab #192)

**Intent:** [GitLab #192](https://gitlab.com/PlasticDigits/yieldomega/-/issues/192) — **USD-shaped** labels were easy to misread as **live** quotes. The UI must keep **CL8Y** as the authoritative spend/read while labeling **fixed** conversion layers.

**Invariant (`INV-FRONTEND-192-USD-AFFORDANCE`):**

| Check | Detail |
|-------|--------|
| **Arena total** | **TOTAL USD** = onchain **`totalRaised`** (wei → human) × **`CL8Y_USD_PRICE_PLACEHOLDER`** (**`1`** in [`arenaPageHelpers.ts`](../../frontend/src/pages/timeCurveArena/arenaPageHelpers.ts)); muted line **“CL8Y total seen … · USD is illustrative (1 CL8Y = $1)”** updates when the serialized **`totalRaised`** read changes; block wrapper **`title`** = [`ARENA_TOTAL_USD_EQUIV_TITLE`](../../frontend/src/lib/cl8yUsdEquivalentDisplay.ts). |
| **Simple podiums** | **≈ $… USD** per placement uses **`fallbackPayTokenWeiForCl8y(..., "usdm")`** (**0.98×** static display shape in [`kumbayaDisplayFallback.ts`](../../frontend/src/lib/kumbayaDisplayFallback.ts)); each **≈** row has **`title`** = [`SIMPLE_PODIUM_USD_EQUIV_TITLE`](../../frontend/src/lib/cl8yUsdEquivalentDisplay.ts); section footnote states **static** rate + optional **“Prize CL8Y preview seen …”** when indexer-backed preview updates. |
| **Quiet refresh** | Relative-age labels tick on an **~8s** interval — no per-second DOM churn. |

**Automated:** [`cl8yUsdEquivalentDisplay.test.ts`](../../frontend/src/lib/cl8yUsdEquivalentDisplay.test.ts) (`formatRelativeFreshnessEnglish`). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-192`](manual-qa-checklists.md#manual-qa-issue-192).

**Docs / play:** [timecurve-views — #192](../frontend/timecurve-views.md#usd-equivalent-staleness-gitlab-192) · [`play-timecurve-doubloon/SKILL.md`](../../skills/play-timecurve-doubloon/SKILL.md).

<a id="frontend-stats-charts-empty-states-gitlab-200"></a>

### Frontend — stats and charts empty states (GitLab #200)

**Intent:** [GitLab #200](https://gitlab.com/PlasticDigits/yieldomega/-/issues/200) — checklist **5.12**: charts and stat tiles must show **explicit** loading / empty / wallet-gated copy instead of a lone **em dash**, which reads as a broken layout.

**Invariant (`INV-FRONTEND-200-EMPTY-STATS`):**

| Check | Detail |
|-------|--------|
| **Arena “What matters now”** | **`WhatMattersSection`** passes **`isConnected`**; wallet-scoped stats (**CHARM weight**, **Battle Points**) use connect hints when disconnected; pending vs failed reads use **Loading…** vs **unavailable** strings wrapped in **`EmptyDataPlaceholder`**. |
...
**Docs / play:** [timecurve-views — #200](../frontend/timecurve-views.md#stats-charts-empty-states-gitlab-200) · [`skills/README.md`](../../skills/README.md) (contributor index).

<a id="timecurve-responsive-layout-gitlab-201"></a>

### TimeCurve responsive layout containment (GitLab #201)

**Intent:** [GitLab #201](https://gitlab.com/PlasticDigits/yieldomega/-/issues/201) — the TimeCurve Simple buy panel and Arena WarBow defense/revenge cards must stay inside phone/tablet safe areas, without clipped right edges or headings stranded outside card borders.

**Invariant (`INV-FRONTEND-201-RESPONSIVE-LAYOUT`):**

| Check | Detail |
|-------|--------|
| **Phone Simple buy panel** | At ≤520 px, `.timecurve-simple__buy-panel` hides the decorative `.panel-cutout--coin-stack`, clears the extra `.chain-write-gate` right padding, and collapses `.timecurve-simple__slider-row` to one column so the CTA / slider / amount input fit within the panel. |
| **Tablet Arena WarBow cards** | At ≤960 px, `.page--timecurve .data-panel--spotlight` reclaims desktop right padding, the WarBow hero grid uses wider card tracks, `warbow-hero-card--steal` spans the row, and `.warbow-hero-card__head` / `.warbow-hero-revenge-list li` wrap instead of overflowing. |
| **Viewport guard** | Playwright checks `documentElement.scrollWidth - innerWidth <= 1` on the 390×844 Simple viewport and the iPad Mini Arena viewport when the WarBow surface is configured. |

**Automated:** [`timeCurveResponsiveLayoutCss.test.ts`](../../frontend/src/lib/timeCurveResponsiveLayoutCss.test.ts); [`timecurve.spec.ts`](../../frontend/e2e/timecurve.spec.ts).

**Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-201`](manual-qa-checklists.md#manual-qa-issue-201).

**Docs / play:** [timecurve-views — #201](../frontend/timecurve-views.md#timecurve-responsive-layout-gitlab-201) · [`skills/README.md`](../../skills/README.md) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).
...
| Invariant ID | Check |
|--------------|-------|
| **`INV-WARBOW-134-RANK`** | **`warbowSteal`** requires **`battlePoints(attacker) > 0`** and **`2 × battlePoints(attacker) ≤ battlePoints(victim) ≤ 10 × battlePoints(attacker)`** — revert **`TimeCurve: steal 2x rule`** (lower bound / zero attacker) or **`TimeCurve: steal 10x cap`** (victim too far above attacker — [GitLab #211](https://gitlab.com/PlasticDigits/yieldomega/-/issues/211)). |
...
**Forge:** [`TimeCurve.t.sol`](../../contracts/test/TimeCurve.t.sol) — `test_warbow_steal_revert_zero_attacker_bp`, `test_warbow_steal_revert_10x_cap`, `test_warbow_steal_attacker_daily_limit_fourth_needs_bypass`; [`TimeCurveWarBowCl8yBurns.t.sol`](../../contracts/test/TimeCurveWarBowCl8yBurns.t.sol) — fuzz setups grant stealer BP (**one buy**) and victim **`≥ 2×`** and **`≤ 10×`** (**two buys** at flat BP rates). **Frontend:** [`warbowStealBpBand.ts`](../../frontend/src/lib/warbowStealBpBand.ts), [`timeCurveUx.ts` — `describeStealPreflight`](../../frontend/src/lib/timeCurveUx.ts), Arena `WarbowSection` / `WarbowHeroActions`, [`revertMessage.ts`](../../frontend/src/lib/revertMessage.ts). **Docs / play:** [primitives — Steal](../product/primitives.md#steal-warbowsteal) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).

<a id="warbow-podium-snapshot-drifts-gitlab-129"></a>

### WarBow ladder snapshot, governance finalize, and **`distributePrizes`** latch ([GitLab #129](https://gitlab.com/PlasticDigits/yieldomega/-/issues/129); governance **#172**)

**Intent:** The **`warbowLadderPodium()`** view reflects the latched governance podium after **`finalizeWarbowPodium`** (**GitLab #172**); during the sale it may lag live **`battlePoints`** until steals / buys settle. **`distributePrizes`** must not pay a stale WarBow slice when **`reservePodiumPayoutsEnabled`** and podium CL8Y is positive.

| Invariant ID | Check |
|--------------|-------|
| **`INV-WARBOW-172-GOV-FINALIZE`** | **`finalizeWarbowPodium(first, second, third)`** is **`onlyOwner`**, **`ended && !prizesDistributed`**, enforces **left-packed** **`address(0)`** suffixes, **strict** non-increasing **`battlePoints`** along occupied ranks (with **`uint160`** tie-break where applicable), writes **`_warbowPodium`**, sets **`warbowPodiumFinalized = true`**, emits **`WarbowPodiumFinalized`**. |
...
**Forge:** [`TimeCurve.t.sol`](../../contracts/test/TimeCurve.t.sol) — finalize + **`distributePrizes`** paths. **Frontend:** **`warbowPodiumFinalized()`** reads; **`/timecurve/protocol`** governance panel — [`TimeCurveProtocolWarbowRefreshSection.tsx`](../../frontend/src/pages/timecurve/TimeCurveProtocolWarbowRefreshSection.tsx). **Anvil harness:** **`SimulateAnvilRichState` Part 2**, [`verify-timecurve-post-end-gates-anvil.sh`](../../scripts/verify-timecurve-post-end-gates-anvil.sh) row **4**. **Indexer (optional vs [#112](#indexer-emitted-event-coverage-gitlab-112)):** decode **`WarbowPodiumFinalized`** if UI needs history.

**Docs / play:** [timecurve-views — WarBow podium (#129 / #172)](../frontend/timecurve-views.md#warbow-ladder-podium-snapshot-mismatch-issue-129) · [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md) · [empty **`PodiumPool`** settlement — #133](#timecurve-empty-podium-pool-settlement-gitlab-133).
...
<a id="indexer-timecurve-podiums-live-prediction"></a>

**Indexer live prediction (`INV-INDEXER-PODIUM-PREDICT-LIVE`, schema ≥ 1.20.0):** While the head snapshot reports **`sale_ended: false`**, **`GET /v1/timecurve/podiums`** builds every category row from **Postgres** (indexed `Buy` + WarBow tables) so WarBow / defended streak / time booster never echo empty pre-finalize **`podium()`** slots. After **`sale_ended: true`**, the same endpoint returns **authoritative** **`podium(category)`** tuples at **`read_block_number`**. Rows carry **`podium_prediction`**; Last Buy retains **`last_buy_prediction`** during the live sale for older clients.
...
| **Data path (indexer on)** | `usePodiumReads` prefers **`GET /v1/timecurve/podiums`** — live DB predictions while the sale is active; see **`INV-INDEXER-PODIUM-PREDICT-LIVE`**. |
| **Data path (indexer off)** | `usePodiumReads` uses **`TimeCurve.podium(category)`** via `PODIUM_CONTRACT_CATEGORY_INDEX` (WarBow may stay empty until post-end finalize). |
...
| **Empty slot copy** | **`INV-FRONTEND-113-PODIUM-FALLBACK`:** Zero-address slots use a neutral **em dash**, not wallet-connect phrasing. |
| **Near-real-time refresh** | `Buy` logs refetch podium reads immediately; TanStack Query polls podiums on a short interval while the indexer URL is set. |
...
**Docs / play:** [timecurve-views](../frontend/timecurve-views.md) · [manual QA #113](manual-qa-checklists.md#manual-qa-issue-113) · [`play-timecurve-doubloon/SKILL.md`](../../skills/play-timecurve-doubloon/SKILL.md) · [indexer design — podiums HTTP](../indexer/design.md#timecurve-podiums-http).

<a id="time-booster-podium-tiered-duration-gitlab-228"></a>

### Time Booster podium — tiered duration display (GitLab #228)

**Intent:** [GitLab #228](https://gitlab.com/PlasticDigits/yieldomega/-/issues/228) — **Time Booster** cumulative effective timer seconds are hard to scan as raw **`300s added`** / **`300 s`** on Simple reserve podiums and Arena podium spotlights. Format **display-only** tiered clocks aligned with [`formatTimer.ts`](../../frontend/src/pages/timecurve/formatTimer.ts).

**Invariant (`INV-FRONTEND-228-TIMEBOOSTER-FMT`):**

| Check | Detail |
|-------|--------|
| **Shared formatter** | **`formatTimeBoosterPodiumSec`** in [`timeBoosterPodiumFormat.ts`](../../frontend/src/pages/timecurve/timeBoosterPodiumFormat.ts) — whole seconds only; negatives clamp to **0**. |
| **Tier boundaries** | **&lt; 60 s:** locale integer, no **`s`** suffix; **60 s – 59 m 59 s:** **`mm:ss`** via **`formatMmSsCountdown`**; **1 h – 23 h 59 m 59 s:** **`HH:MM:SS`** via **`formatCountdown`**; **≥ 24 h:** **`d:HH:MM:SS`** (day count + remainder clock from **`formatLaunchCountdown`**). |
| **Simple copy** | **`formatSimplePodiumScoreLine`** category **3** → **`Score: <formatted> added`**; zero value → **`Score: —`**. |
| **Arena parity** | **`formatPodiumLeaderboardValue`** category **3** uses the same formatter (no trailing **` s`**). |
| **Out of scope** | Buy projected-effects **`+Xs time-booster credit`** ([`timeCurveBuyProjectedEffects.ts`](../../frontend/src/pages/timecurve/timeCurveBuyProjectedEffects.ts)) unchanged unless product requests parity. |

**Automated:** [`timeBoosterPodiumFormat.test.ts`](../../frontend/src/pages/timecurve/timeBoosterPodiumFormat.test.ts) · [`timeCurveSimplePodiumScore.test.ts`](../../frontend/src/pages/timecurve/timeCurveSimplePodiumScore.test.ts) · [`TimeCurveSimplePodiumSection.test.tsx`](../../frontend/src/pages/timecurve/TimeCurveSimplePodiumSection.test.tsx).

**Docs / play:** [timecurve-views §228](../frontend/timecurve-views.md#time-booster-podium-tiered-duration-gitlab-228) · [manual QA #228](manual-qa-checklists.md#manual-qa-issue-228) · [`skills/README.md`](../../skills/README.md).
...
**Intent:** When the HTTP indexer (`VITE_INDEXER_URL`) fails during a session, participants see an explicit **offline / retrying** signal, indexer pollers **back off** (5s → 15s → 30s) after a short streak of failures, and **Recent buys** does not use the **“first buy of the round”** empty copy while the feed is unreachable ([issue #96](https://gitlab.com/PlasticDigits/yieldomega/-/issues/96)).
...
| **Adaptive intervals** | **`getIndexerBackoffPollMs(fastMs)`** returns **`fastMs`** when healthy; after threshold, **5s / 15s / 30s** capped tiers. |
...
### Indexer ingestion liveness + RPC timeouts (issue #168)

**`INV-INDEXER-168`:** Without supervision, a single **`ingestion::run`** failure stopped the background task while **`GET /v1/...`** continued returning **200** from frozen Postgres ([GitLab #168](https://gitlab.com/PlasticDigits/yieldomega/-/issues/168)). Hung JSON-RPC calls could stall ingestion or the chain-timer poller indefinitely because **reqwest** defaulted to **no** timeout.

| Invariant | Check |
|-----------|--------|
| **Supervised retry** | [`main.rs`](../../indexer/src/main.rs) loops **`ingestion::run`** with **exponential backoff** (base **1s**, max **60s**) after **`Err`**. |
| **RPC HTTP timeout** | [`rpc_http::reqwest_http_provider`](../../indexer/src/rpc_http.rs) builds **`Http::with_client`** using **`reqwest::Client::builder().timeout(...)`**; used by [`ingestion.rs`](../../indexer/src/ingestion.rs) and [`chain_timer.rs`](../../indexer/src/chain_timer.rs). **`INDEXER_RPC_REQUEST_TIMEOUT_SEC`** parsed in [`config.rs`](../../indexer/src/config.rs) (default **5**, clamp **1–120**). |
| **`/v1/status` fields** | **`ingestion_alive`**: **`true`** only in the **active** ingestion loop ( **`INGESTION_ENABLED`** on + non-empty registry addresses); **`false`** in API-only idle modes. **`last_indexed_at_ms`**: wall-clock millis after last successful block commit; **`0`** if never. Schema **`≥ 1.16.1`** ([`api.rs`](../../indexer/src/api.rs)). |

**Automated:** [`integration_stage2.rs`](../../indexer/tests/integration_stage2.rs) (`GET /v1/status` asserts fields).

**Doc:** [indexer README — #168](../../indexer/README.md#ingestion-supervision--rpc-timeouts-gitlab-168) · [indexer design — runtime resilience](../indexer/design.md#indexer-runtime-resilience-gitlab-168) · third-party agent index [`skills/README.md`](../../skills/README.md).
...
<a id="referral-self-referral-pending-purge-gitlab-222"></a>

**Self-referral pending purge ([GitLab #222](https://gitlab.com/PlasticDigits/yieldomega/-/issues/222)):** When the connected wallet’s cached registered slug (**`getStoredMyReferralCodeForWallet`**) normalizes to the same value as **`getPendingReferralCode`**, the app **removes** **`yieldomega.ref.v1`** from both stores (**`INV-REFERRAL-222-PENDING-PURGE`**) on wallet connect/change, pending capture, and **`setStoredMyReferralCodeForWallet`** — so TimeCurve buys do not keep auto-applying a self-referral that would revert onchain. Onchain **`TimeCurve: self-referral`** behavior is unchanged; third-party pending codes still persist across successful buys ([#85](#referrals-page-visual-issue-64)). **Scope limit:** no purge when **`myrefcode`** is absent (unregistered wallet testing its own future slug).

**Automated:** [`referralSelfReferralPending.test.ts`](../../frontend/src/lib/referralSelfReferralPending.test.ts). **Manual QA:** [`manual-qa-checklists.md#manual-qa-issue-222`](manual-qa-checklists.md#manual-qa-issue-222). **Product:** [referrals.md § #222](../product/referrals.md#referral-self-referral-pending-purge-issue-222).
...
| Global summary + pagination ([GitLab #225](https://gitlab.com/PlasticDigits/yieldomega/-/issues/225)) | **`INV-INDEXER-225-REFERRAL-LB-GLOBAL`:** **`GET /v1/referrals/referrer-leaderboard`** (schema **≥ 1.25.0**) includes **`total`** (distinct referrer union count), **`total_codes_registered`** (**`COUNT(*)`** on **`idx_referral_code_registered`**), **`total_referred_buys`** (**`COUNT(*)`** on **`idx_timecurve_referral_applied`**), and **`total_referrer_charm_wad`** (**`SUM(referrer_amount)`** on **`idx_timecurve_referral_applied`**) on every page. **`INV-FRONTEND-225-REFERRAL-LB`:** **`ReferralLeaderboardSection`** summary strip labels **“(global)”** and reads those fields (not page-local **`reduce`** sums); numbered pagination uses **`limit`/`offset`** with indexer **`rank`** preserved — [`referralLeaderboardPagination.ts`](../../frontend/src/lib/referralLeaderboardPagination.ts), [`ReferralLeaderboardPagination.tsx`](../../frontend/src/pages/referrals/ReferralLeaderboardPagination.tsx). |
...
**Automated:** `cargo test` **`postgres_gitlab204_referrer_leaderboard_includes_registry_registrations`** · **`postgres_gitlab177_referrer_leaderboard_dense_rank`** · **`postgres_gitlab225_referrer_leaderboard_global_totals_and_pagination`**.

<a id="referrer-leaderboard-global-pagination-gitlab-225"></a>

### Referrer leaderboard — global totals + pagination (GitLab #225)

**Intent:** The `/referrals` **Guide leaderboard** summary strip must show **network-wide** registration / buy / CHARM totals from the indexer, not sums of the visible page. Users must be able to **page through all guides** without losing dense competitive **`rank`** semantics ([GitLab #177](https://gitlab.com/PlasticDigits/yieldomega/-/issues/177)).

| ID | Check |
|----|--------|
| **`INV-INDEXER-225-REFERRAL-LB-GLOBAL`** | **`GET /v1/referrals/referrer-leaderboard`** (schema **≥ 1.25.0**) returns **`total`**, **`total_codes_registered`**, **`total_referred_buys`**, and **`total_referrer_charm_wad`** on every paginated response; aggregates match full-table counts/sums (not **`LIMIT`/`OFFSET`** window sums). |
| **`INV-FRONTEND-225-REFERRAL-LB`** | **`ReferralLeaderboardSection`** uses indexer global fields for the summary strip (**“Codes registered (global)”**, etc.), keeps globals stable while paging, and renders **`ReferralLeaderboardPagination`** when **`total > limit`**. Row **`rank`** values come from JSON as-is (not **`offset + index + 1`**). |

**Implementation:** [`indexer/src/api.rs`](../../indexer/src/api.rs) (`SCHEMA_VERSION` **1.25.0**); UI: [`ReferralLeaderboardSection.tsx`](../../frontend/src/pages/referrals/ReferralLeaderboardSection.tsx). Contributor manual QA: [`manual-qa-checklists.md#manual-qa-issue-225`](manual-qa-checklists.md#manual-qa-issue-225). Third-party agent index: [`skills/README.md`](../../skills/README.md).
...
**`INV-INDEXER-165` — Referral applied rows: bare column equality + buyer btree ([GitLab #165](https://gitlab.com/PlasticDigits/yieldomega/-/issues/165)):** Full-stack audit **L-02** (`audits/audit_full_1777967937.md`). **Invariant:** **`idx_timecurve_referral_applied.buyer`** and **`.referrer`** are always **lowercase** at insert; route parameters are lowercased before bind. **Do not** wrap those columns in **`lower()`** in `WHERE` / `JOIN` clauses — it prevents index use. Symmetric **btree** indexes: **`idx_timecurve_referral_applied_referrer`**, **`idx_timecurve_referral_applied_buyer`** (migration `20260506210000_idx_timecurve_referral_applied_buyer`). **Unrelated:** **`INV-INDEXER-157`** (HTTP 500 redaction) is unchanged.
...
- Steal path — `test_warbow_steal_drains_ten_percent_and_routes_spend_via_fee_router`, `test_warbow_steal_revert_2x_rule`, `test_warbow_steal_burn_is_one_cl8y_wad`
- Revenge — `test_warbow_revenge_once`

<a id="timecurve-warbow-cl8y-burns-issue-70"></a>

**WarBow CL8Y → FeeRouter (2026-05-19)** — *Public **`warbowSteal` / `warbowRevenge` / `warbowActivateGuard`** pull fixed WAD from the caller, route gross via **`FeeRouter.distributeFees`** (canonical buy split), and increment **`totalRaised`**; **`claimWarBowFlag`** has no CL8Y leg.* **`msg.sender`** must **not** be **`BURN_SINK`** (**[`INV-WARBOW-123-BURN-CALLER`](#inv-warbow-123-burn-caller)**). **`INV-WARBOW-EVENT-NAME`:** **`WarBowCl8yBurned`** / indexer **`cl8y_burned`** are **historical names** — post-upgrade they log **nominal spend**, not “100% to `0x…dEaD`” ([primitives — historical event](../product/primitives.md#historical-warbowcl8yburned-event-name-2026-05-19-upgrade)). Pre-2026-05-19 txs sent 100% to dead. Fuzz: [`TimeCurveWarBowCl8yBurns.t.sol`](../../contracts/test/TimeCurveWarBowCl8yBurns.t.sol) (`testFuzz_warbow_*_routes_via_fee_router`, `test_warbow_steal_increases_podium_pool_by_twenty_percent`, **`test_warbow_*_reverts_when_caller_is_burn_sink`**). Play skill: [`skills/play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md). [CL8Y flow audit](../onchain/cl8y-flow-audit.md).
...
| Invariant | Meaning | Evidence |
|-----------|---------|----------|
| **`INV-FRONTEND-232-FOOTER-SITE-LINKS`** | Routes that render **`.app-footer`** show a standalone **`FooterSiteLinksCard`** (`data-testid="footer-site-links-card"`) **below** the agent card (not inside it). Pills use repo PNG icons or brand SVGs where available; labels include **Agent SKILL.md** (raw GitHub `skills/play-active-timecurve/SKILL.md`). All use **`target="_blank"`** + **`rel="noreferrer noopener"`** + **`.footer-link-pill`**. Nav: **`data-testid="footer-site-links"`**. | [`FooterSiteLinksCard.tsx`](../../frontend/src/components/FooterSiteLinksCard.tsx), [`FooterSiteLinks.tsx`](../../frontend/src/components/FooterSiteLinks.tsx), [`footerSiteLinks.ts`](../../frontend/src/lib/footerSiteLinks.ts), [`RootLayout.tsx`](../../frontend/src/layout/RootLayout.tsx) |
| **`INV-FRONTEND-232-SIMPLE-PARITY`** | **`/timecurve`** hides the global footer but renders the same **`FooterSiteLinksCard`** at the bottom of the Simple page (sibling below **`TimeCurveSimpleAgentCard`**). | [`TimeCurveSimplePage.tsx`](../../frontend/src/pages/TimeCurveSimplePage.tsx), [`RootLayout.tsx`](../../frontend/src/layout/RootLayout.tsx) |
...
| `test_ecostrategy.py` | EcoStrategy audit scenarios — **`WarBowWorld`** mirrors `warbowSteal` / `warbowRevenge` / `warbowActivateGuard` (2× rule, UTC-day steal caps, drain bps, revenge window); scenarios **A–F** via **`python3 -m ecostrategy`** ([GitLab #161](https://gitlab.com/PlasticDigits/yieldomega/-/issues/161), coverage gaps [GitLab #167](https://gitlab.com/PlasticDigits/yieldomega/-/issues/167)); **D** final-window ordering branches (**[`INV-SIM-169-D`](#ecostrategy-scenario-d-isolated-after-tick-gitlab-169)** — [GitLab #169](https://gitlab.com/PlasticDigits/yieldomega/-/issues/169)); **E** FeeRouter splits + Rabbit **`receiveFee`** vs direct sink (H-01 narrative) + launch anchor; **F** sybil capital split + operator share metrics | `test_two_x_rule_blocks_steal`, `test_steal_drain_matches_bps`, `test_guard_reduces_drain`, `test_daily_limit_blocks_without_bypass`, `test_revenge_moves_bp_from_stealer`, `test_utc_rollover_fourth_steal_without_bypass`, `test_flag_interrupt_after_silence_applies_penalty`, `test_flag_interrupt_before_silence_clears_without_penalty`, `test_flag_claim_awards_bp`, `test_shares_sum_to_gross_and_rabbit_is_remainder`, `test_scenarios_finish_and_b_weight_stack`, `test_scenario_c_records_pvp_events`, `test_scenario_d_ordering_branches`, `test_scenario_d_warbow_guards_parity_seed_42`, `test_scenario_e_receive_fee_books_coverage`, `test_simulate_sale_buy_cooldown_reduces_buys_vs_short_cooldown`, `test_simulate_sale_utc_offset_in_metrics`, `test_scenario_f_operator_metrics` |

#### EcoStrategy audit scenarios (GitLab #161)

Python **`simulations/ecostrategy/`** models shark-vs-believer outcomes under EcoStrategy audit assumptions ([`audits/audit_ecostrategy_1777969776.md`](../../audits/audit_ecostrategy_1777969776.md)): **`WarBowWorld`** replaces the Monte Carlo **toy** PvP steal (`pvp_steal_prob` in `monte_carlo.run_single_sale`) with **`TimeCurve`-aligned** eligibility and drains. Scenarios **D–E** add final-window tick ordering and FeeRouter / Rabbit booking slices per EcoStrategy audit threads; **scenario F** plus **`INV-SIM-167`** extend the harness for buy cooldown, sybil capital splits, WarBow flag interrupt/claim, and UTC rollover starts ([GitLab #167](https://gitlab.com/PlasticDigits/yieldomega/-/issues/167) — [§ below](#ecostrategy-sim-coverage-gitlab-167)). **`INV-SIM-161`**: mirrored numeric constants are enumerated in **`simulations/ecostrategy/constants.py`** (`MIRRORED_FROM_CONTRACT` vs `APPROXIMATIONS_AND_SCENARIO_ONLY`); Poisson arrivals and wallet budgets remain scenario-only. **`INV-SIM-169-D`**: **`run_scenario_d`** builds **separate** **`_scenario_c_style_after_tick`** closures for **`buy_first`** and **`pvp_first`** so **`guard_once`** / **`leader_guarded`** does not leak across paired runs — otherwise **`pvp_first_warbow_guards`** can read **0** while **`buy_first_warbow_guards`** is **1**, inflating ordering deltas ([GitLab #169](https://gitlab.com/PlasticDigits/yieldomega/-/issues/169)); see **`run_scenario_d`** in [`scenarios.py`](../../simulations/ecostrategy/scenarios.py). Third-party agents: [`skills/README.md`](../../skills/README.md#ecostrategy-scenarios-gitlab-161-modeling-entry). Play skill: [`skills/play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md).

<a id="ecostrategy-sim-coverage-gitlab-167"></a>

#### EcoStrategy sim coverage — cooldown, sybil slice, flag ladder, UTC rollover ([GitLab #167](https://gitlab.com/PlasticDigits/yieldomega/-/issues/167))

**`INV-SIM-167`:** **`_simulate_sale`** gates successful buys with **`TimeCurveParams.buyCooldownSec`** (default **300** via **`canonical_timecurve_params`**, mirroring **`TimeCurve.buyCooldownSec`** / **`nextBuyAllowedAt`**). **`WarBowWorld`** models **`WARBOW_FLAG_CLAIM_BP`**, **`WARBOW_FLAG_SILENCE_SEC`**, interrupting-buy penalty (**`2 × WARBOW_FLAG_CLAIM_BP`** BP debit on the prior holder when a different wallet buys after silence), optional **`plant_flag_after_buy`**, and **`try_claim_flag`**. **`start_unix_offset_sec`** shifts simulated chain time so UTC-day boundaries fall inside short default horizons (L-01-eco rollover harness). **Scenario F** splits **`operator_capital`** across **`wallets_per_operator`** wallets and reports **`operator_*`** share metrics (M-03-eco sybil slice). Discrete-tick ordering remains an approximation — see **`APPROXIMATIONS_AND_SCENARIO_ONLY`** in **`simulations/ecostrategy/constants.py`**.
...
| [TimeCurveWarBowCl8yBurns.t.sol](../../contracts/test/TimeCurveWarBowCl8yBurns.t.sol) | 7 | Fuzz: WarBow CL8Y spend routes via **`FeeRouter`** (canonical buy split + **`totalRaised`**); podium 20% slice; explicit **`BURN_SINK` caller revert** (**[`INV-WARBOW-123-BURN-CALLER`](#inv-warbow-123-burn-caller)**, [#123](https://gitlab.com/PlasticDigits/yieldomega/-/issues/123); [#70](https://gitlab.com/PlasticDigits/yieldomega/-/issues/70)) |
