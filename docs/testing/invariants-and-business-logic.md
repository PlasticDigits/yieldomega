...
| **`INV-FRONTEND-338-WYWA-MODAL`** | Play `/` persists **`yieldomega.arena.lastClosedAt.v1`** on tab hide; skips first visit and indexer-offline; fetches session summary indexer-first; glass **`WhileYouWereAwayModal`** shows elapsed time, buy/podium/player aggregates, wallet rank delta, and winner congratulations ([#338](https://gitlab.com/PlasticDigits/yieldomega/-/issues/338)) | `WhileYouWereAwayModal.test.tsx`, `arenaSessionClose.test.ts`, `indexerApi.test.ts`, `cd frontend && npm run typecheck && npm run lint && npm test` · [arena-views §338](../frontend/arena-views.md#while-you-were-away-modal-gitlab-338) · screenshots [issue-338](../testing/screenshots/issue-338/README.md) |
