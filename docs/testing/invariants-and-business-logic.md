...
| **Indexer** | Decode canonical logs, **exhaustively cover emitted events** stored in Postgres for history / future UI ([§ #112](#indexer-emitted-event-coverage-gitlab-112); [scoped issue discussion](https://gitlab.com/PlasticDigits/yieldomega/-/issues/112)); idempotent persist; chain pointer + reorg rollback of **all** `idx_*` tables. **Per-block durability:** event rows + `indexed_blocks` + `chain_pointer` for one height commit together ([§ #140](#indexer-transactional-block-ingestion-gitlab-140)). **`TimeCurveBuyRouter`** `BuyViaKumbaya` + `TimeCurve` `Buy` correlation for multi-asset entry metadata ([issue #67](https://gitlab.com/PlasticDigits/yieldomega/-/issues/67)). | [`REORG_STRATEGY.md`](../../indexer/REORG_STRATEGY.md), [`ingestion.rs`](../../indexer/src/ingestion.rs), [`persist.rs`](../../indexer/src/persist.rs), [`reorg.rs`](../../indexer/src/reorg.rs), [design — emitted logs](../indexer/design.md), [integrations/kumbaya.md](../integrations/kumbaya.md) |
...
| **TimeCurve — WarBow ladder snapshot drift + **`distributePrizes`** latch (#129)** | **`refreshWarbowPodium(candidates)`** (permissionless) re-applies live **`battlePoints`** heap updates and clears **`warbowPodiumFinalized`**. **`finalizeWarbowPodium(candidates)`** (`onlyOwner`, **ended**, **`!prizesDistributed`**) rebuilds the podium from **`bp > 0`** candidates and sets **`warbowPodiumFinalized`**. When **`reservePodiumPayoutsEnabled`** and **`acceptedAsset.balanceOf(podiumPool) > 0`**, **`distributePrizes`** requires **`warbowPodiumFinalized`** before paying (**zero** balance skips WarBow finalize — [GitLab #133](#timecurve-empty-podium-pool-settlement-gitlab-133)). **Arena** maps a **Claim your WarBow position (refresh snapshot)** CTA via [`timeCurveWarbowSnapshotClaim.ts`](../../frontend/src/lib/timeCurveWarbowSnapshotClaim.ts) (sale-active heuristic). **Harness:** [`SimulateAnvilRichState.s.sol`](../../contracts/script/SimulateAnvilRichState.s.sol), [`verify-timecurve-post-end-gates-anvil.sh`](../../scripts/verify-timecurve-post-end-gates-anvil.sh). | [§ #129](#warbow-podium-snapshot-drifts-gitlab-129), [§ #133](#timecurve-empty-podium-pool-settlement-gitlab-133), [issue #129](https://gitlab.com/PlasticDigits/yieldomega/-/issues/129), [`play-timecurve-warbow/SKILL.md`](../../skills/play-timecurve-warbow/SKILL.md), [timecurve-views — #129](../frontend/timecurve-views.md#warbow-ladder-podium-snapshot-mismatch-issue-129) |
...
| Invariant ID | Check |
|--------------|-------|
| **`INV-WARBOW-129-REFRESH`** | **`refreshWarbowPodium(candidates)`** is **`external`**, **non-payable**, re-applies **`_updateWarbowPodium(acc)`** for each non-zero **`candidates[i]`**, emits **`WarbowPodiumRefreshed`**, and sets **`warbowPodiumFinalized = false`**. |
