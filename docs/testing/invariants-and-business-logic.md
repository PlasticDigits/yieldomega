...
| **TimeCurve Arena — WarBow indexer leaderboard + battle-feed refresh ([GitLab #182](https://gitlab.com/PlasticDigits/yieldomega/-/issues/182))** | **`INV-FRONTEND-182-WARBOW-IDX`:** With **`VITE_INDEXER_URL`**, **`useTimeCurveArenaModel`** refetches **`GET /v1/timecurve/warbow/leaderboard`** and **`GET /v1/timecurve/warbow/battle-feed`** on **`refetchAll`** (post-receipt for Arena **`buy`** / Kumbaya / WarBow txs / **`runVoid`**) and on a **~5s backoff** poll while **`/timecurve/arena`** is mounted — **`warbowRank`**, chasing-pack rows, and rivalry feed stay aligned after BP-changing txs and when other wallets move the ladder. Stale parallel completes are dropped via a **sequence guard**. | [§182](#timecurve-arena-warbow-indexer-refresh-gitlab-182), [`useTimeCurveArenaModel.tsx`](../../frontend/src/pages/timeCurveArena/useTimeCurveArenaModel.tsx), [timecurve-views §182](../frontend/timecurve-views.md#arena-warbow-indexer-leaderboard-feed-refresh-gitlab-182), [manual QA (#182)](manual-qa-checklists.md#manual-qa-issue-182) |
...
| **Poll** | While **`VITE_INDEXER_URL`** is configured, a remount-safe loop schedules **`getIndexerBackoffPollMs(5000)`** after each completion so external ladder / feed activity converges without navigation. |
