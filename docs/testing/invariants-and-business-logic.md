...
| **Frontend — stats / charts empty states ([GitLab #200](https://gitlab.com/PlasticDigits/yieldomega/-/issues/200))** | **`INV-FRONTEND-200-EMPTY-STATS`:** Player-facing **stat cards**, **rate-board** slots, **standings visuals**, **referral CHARM totals**, and **stake-at-launch** tiles must not collapse to a bare **em dash** when data is loading, wallet-gated, or absent. Use **`EmptyDataPlaceholder`** + **`statFromContractRead` / `statFromOptionalString`** (Arena **`WhatMattersSection`**, standings **`StandingsVisuals`**, referral earnings banner, Simple podium prize preview, stake tiles) so copy reads as **intentional empty / loading** — checklist UX **5.12**. | [§200](#frontend-stats-charts-empty-states-gitlab-200), [timecurve-views §200](../frontend/timecurve-views.md#stats-charts-empty-states-gitlab-200), [`EmptyDataPlaceholder.tsx`](../../frontend/src/components/EmptyDataPlaceholder.tsx), [`statDisplayFromContractRead.tsx`](../../frontend/src/lib/statDisplayFromContractRead.tsx) |
...
**Automated:** [`statDisplayFromContractRead.test.tsx`](../../frontend/src/lib/statDisplayFromContractRead.test.tsx).
