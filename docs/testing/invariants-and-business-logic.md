...
| **`INV-FRONTEND-277-APPROVAL-UNCHANGED`** | Approval economics unchanged: skip when allowance covers **need + 50 bps**; new approves use **`maxUint256`** when opted in ([#143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143), [#224](https://gitlab.com/PlasticDigits/yieldomega/-/issues/224)) | [`arenaDoubApprovalPreference.test.ts`](../../frontend/src/lib/arenaDoubApprovalPreference.test.ts) · [`ensureCl8yKumbayaAllowance.test.ts`](../../frontend/src/lib/ensureCl8yKumbayaAllowance.test.ts) · [`ensureDoubTimeArenaAllowance.test.ts`](../../frontend/src/lib/ensureDoubTimeArenaAllowance.test.ts) |
...
| **`INV-TIME-ARENA-TIMER-MULTI`** | One buy extends **all four** `podiumDeadline[i]` by **category-specific** extension (+120 / +60 / +480 / +300) | `test_multi_podium_deadline_extend` ([#271](https://gitlab.com/PlasticDigits/yieldomega/-/issues/271)) |
...
| **`INV-TIME-ARENA-PODIUM-ROLL`** | `rollPodiumEpoch(cat)` after expiry; epoch bump; pays winners; clears podium slots; Time Booster / Defended Streak epoch scores reset (generation bump) | `test_roll_podium_after_expiry`, `test_roll_podium_settlement_pays_and_clears_scores`, `test_time_booster_score_resets_on_epoch_roll`, `test_defended_streak_best_resets_on_epoch_roll` ([#247](https://gitlab.com/PlasticDigits/yieldomega/-/issues/247)) |
| **`INV-TIME-ARENA-PODIUM-EPOCH-SCORE-RESET`** | Time Booster ranks by **`epochTimerSecAdded`** (not lifetime `totalEffectiveTimerSecAdded`); Defended Streak ranks by **`epochBestDefendedStreak`**; stale generation reads zero; mainnet **`migrateEpochPodiumScores`** (`reinitializer(2)`) executed 2026-07-03 | `test_time_booster_score_resets_on_epoch_roll`, `test_time_booster_displaced_reinsert_uses_epoch_metric`, `test_defended_streak_best_resets_on_epoch_roll` · indexer `arena_wallet_stats.rs` epoch-window simulation |
...
| **`INV-TIME-ARENA-WARBOW-DOUB`** | WarBow spends are DOUB pulls: steal and revenge are **`epochCharmAnchor / 5`**, guard is **`epochCharmAnchor / 2`**, and bypass remains fixed at **`50_000e18`**; prices use the post-autoroll epoch anchor, not `effectiveCharmPriceWad()` daily growth. **100%** routes to podium vaults; **`totalDoubRaised`** increments; no stranded balance on **`TimeArena`** ([#310](https://gitlab.com/PlasticDigits/yieldomega/-/issues/310), [#367](https://gitlab.com/PlasticDigits/yieldomega/-/issues/367)) | `test_warbow_steal_pulls_doub`, `test_warbow_guard_pulls_doub`, `test_warbow_revenge_pulls_doub`, `test_warbow_steal_limit_override_pulls_doub`, `test_warbow_steal_routes_doub_split`, `test_warbow_flag_claim_zero_doub` ([#252](https://gitlab.com/PlasticDigits/yieldomega/-/issues/252), [#310](https://gitlab.com/PlasticDigits/yieldomega/-/issues/310), [#367](https://gitlab.com/PlasticDigits/yieldomega/-/issues/367)) |
| **`INV-TIME-ARENA-WARBOW-STEAL-BAND`** | `warbowSteal` accepts only **`attackerBP ≤ victimBP ≤ 50 × attackerBP`** with attacker BP > 0; equal-BP steals are allowed, while higher-BP attackers cannot target lower-BP wallets. | `test_warbow_steal_*band*`, bot `steal_band_state`, simulation `TestWarBowWorld.test_steal_bp_band_blocks_steal` ([#366](https://gitlab.com/PlasticDigits/yieldomega/-/issues/366)) |
...
| **`INV-TIME-ARENA-WARBOW-RANK-312`** | WarBow podium top-3 matches brute-force ≤100 players; tie-break lower address; O(1) ≤6-address merge (global + off-podium buffer) — off-podium displacement without reinsert is a gas tradeoff; qualifying buy/BP update with strictly higher BP (or equal BP + better tie-break) must enter the merge set | `test_warbow_ranking_matches_brute_force`, `test_warbow_tie_break_lower_address_wins`, `test_warbow_off_podium_tie_break_promotes_to_global`, `test_warbow_higher_bp_buy_enters_podium`, `TimeArenaWarbowBenchmark` gas probes ([#312](https://gitlab.com/PlasticDigits/yieldomega/-/issues/312), [#353](https://gitlab.com/PlasticDigits/yieldomega/-/issues/353)) · [benchmark § tradeoff](warbow-ranking-benchmark-312.md#6-address-tracking-tradeoff-gas-vs-accuracy) |
| **`INV-TIME-ARENA-PAUSE-MUTATING`** | `setPaused(true)` blocks `buy`, `buyWithCred`, `warbowSteal` / `warbowActivateGuard` / `warbowRevenge`, `claimWarBowFlag`, `rollPodiumEpoch`, `claimCred`, and `topUpPodiumPools` via `_requireLive` (`TimeArena: paused`); autoroll on buy/WarBow blocked because `_requireLive()` runs first | `test_pause_blocks_*` ([#316](https://gitlab.com/PlasticDigits/yieldomega/-/issues/316), [#349](https://gitlab.com/PlasticDigits/yieldomega/-/issues/349)) · [detail §316](#timearena-negative-tests-dry-gas-gitlab-316) |
| **`INV-TIME-ARENA-WARBOW-REVERT-MATRIX`** | WarBow ingress reverts: out-of-band steal (`victimBP < attackerBP` or `victimBP > 50 × attackerBP`), daily cap (`steal limit`), revenge window, flag holder/silence, double finalize, bad victim/epoch | `test_warbow_*_reverts_*`, `test_finalize_warbow_podium_reverts_*` ([#316](https://gitlab.com/PlasticDigits/yieldomega/-/issues/316), [#366](https://gitlab.com/PlasticDigits/yieldomega/-/issues/366)) |
...
| **`INV-FRONTEND-347-CLAIM-CRED-LIFECYCLE`** | **`ArenaCharmCredCard` Claim CRED**: `writeContractAsync` → **`waitForWriteReceipt`** → **`invalidateArenaWalletStatsQueries`**; button stays disabled through receipt wait; reverted txs do not invalidate; write errors surface via **`friendlyRevertFromUnknown`** ([#347](https://gitlab.com/PlasticDigits/yieldomega/-/issues/347) · gap [#342](https://gitlab.com/PlasticDigits/yieldomega/-/issues/342)) | `arenaCharmCredClaim.test.ts`, `ArenaCharmCredCard.test.tsx`, `cd frontend && npm run typecheck && npm run lint && npm test` · [arena-views §347](../frontend/arena-views.md#claim-cred-post-tx-lifecycle-gitlab-347) |
...
| **`INV-INDEXER-CRED-PENDING-ONCHAIN`** | Wallet stats **`claimable_cred`** / **`pending_cred_accrual`** prefer **`TimeArena.pendingCred`** when RPC is configured (schema **≥ 2.26.0**); must not zero claimable solely because **`idx_play_cred_claim`** has a prior row for that epoch (residual after **`migrateLastBuyEpochToPodium`**) | `arena_wallet_stats.rs` (`pending_cred_onchain`, `pending_cred_for_epoch_prefer_onchain`), `api_arena.rs` |
...
| **`INV-INDEXER-336-WALLET-LEVEL-HISTORY`** | `GET /v1/arena/wallet/{address}/stats` includes **`level_history`** (levels 1–5; **`reached_at`** UTC ISO-8601 or JSON **`null`**). Level 1 = earliest **`idx_arena_buy`**; levels 2–5 = earliest **`idx_arena_level_up`** per **`new_level`**. Reorg rollback clears rolled-back milestones. | `arena_wallet_stats.rs` unit tests, `integration_stage2.rs::arena_wallet_stats_level_history_and_reorg`, `bash scripts/verify-wallet-profile-anvil.sh`, `bash scripts/verify-indexer-reorg-anvil.sh` ([#336](https://gitlab.com/PlasticDigits/yieldomega/-/issues/336), [#351](https://gitlab.com/PlasticDigits/yieldomega/-/issues/351)) |
| **`INV-INDEXER-351-REORG-ANVIL`** | Live Anvil reorg smoke: orphan-fork ingest → `evm_revert` + empty remine → ingestion **`find_common_ancestor`** + **`rollback_after`** + re-ingest; **`chain_pointer`** matches RPC head hash; rolled-back **`idx_arena_level_up`** rows removed; no duplicate **`idx_arena_buy`**; wallet **`level_history`** matches on-chain **`level()`** | `ingestion.rs`, `reorg.rs`, `integration_stage2.rs::arena_wallet_stats_level_history_and_reorg`, `bash scripts/verify-indexer-reorg-anvil.sh` ([#351](https://gitlab.com/PlasticDigits/yieldomega/-/issues/351)) · [REORG_STRATEGY.md §351](../../indexer/REORG_STRATEGY.md) · [e2e-anvil §324](e2e-anvil.md#verify-anvil-script-helpers-gitlab-324) |
...
| **`INV-FRONTEND-361-WARBOW-STEAL-CAP-UX`** | Play **`/`** WarBow viewer summary shows attacker UTC-day steal usage **`X/3`** plus live countdown to the next UTC-day reset; steal card renders **`describeStealPreflight`** inline above **Steal** (warning/error before submit); clock uses indexer head when configured; UTC-day cap reads use onchain **`stealsCommittedByAttackerOnDay`** / **`stealsReceivedOnDay`** only (no duplicate BP/guard RPC mirrors) ([#361](https://gitlab.com/PlasticDigits/yieldomega/-/issues/361), parent [#360](https://gitlab.com/PlasticDigits/yieldomega/-/issues/360)) | `warbowUtcDayReset.test.ts`, `ArenaWarbowHeroPanel.test.tsx`, `timeArenaUx.test.ts`, `indexerFirstDisplay.test.ts`, `cd frontend && npm run typecheck && npm run lint && npm test` · [arena-views §361](../frontend/arena-views.md#warbow-steal-cap-ux-gitlab-361) · [`play-time-arena-warbow`](../../skills/play-time-arena-warbow/SKILL.md) |
| **`INV-FRONTEND-365-BUY-RESULT-SHARE`** | Successful CHARM buy on **`/`** shows one closable **transaction-result share card** (`arena-buy-result-share-popover`) with grouped timer/rank/BP/XP rows (max ~7); preview on tx success upgrades to indexer-confirmed copy when viewer buy is at head; **Copy summary** + **Copy tx link**; no auto-dismiss; defers while level-up celebration active; rapid buys replace card; **`prefers-reduced-motion`** skips enter animation ([#365](https://gitlab.com/PlasticDigits/yieldomega/-/issues/365)) | `arenaBuyShareSummary.test.ts`, `ArenaBuyResultSharePopover.test.tsx`, `mobileBuyResultShareLayout.test.ts`, `cd frontend && npm run typecheck && npm run lint && npm test`, `cd frontend && CI=1 npm run test:e2e -- --workers=1 e2e/buy-result-share-screenshots.spec.ts`, `cd frontend && CI=1 npm run test:e2e -- --workers=5 e2e/anvil-arena-03-wallet-writes.spec.ts` · [arena-views §365](../frontend/arena-views.md#post-buy-result-share-gitlab-365) · [manual QA §365](manual-qa-checklists.md#manual-qa-issue-365) |
| **`INV-FRONTEND-337-BUY-EFFECT-TOASTS`** | *(Legacy — superseded on play `/` by #365)* Successful CHARM buy emitted one glass toast per effect ([#337](https://gitlab.com/PlasticDigits/yieldomega/-/issues/337)) | `arenaBuyEffectToastLines.test.ts`, `ArenaEffectToastStack.test.tsx` · [arena-views §337](../frontend/arena-views.md#post-buy-effect-toasts-gitlab-337) |
| **`INV-FRONTEND-362-WARBOW-VIEWER-SUMMARY-FLAG`** | When the viewer holds a planted WarBow flag (`showClaimFlagControl`), **`warbow-hero-card--viewer-summary`** shows a third **`FLAG:`** line (`data-testid="warbow-hero-viewer-summary-flag"`) with silence countdown or **claim now**; uses the same `flagSilenceEndSec` + `ledgerNowSec` math as the Flag subcard; no flag line when unplanted; indexer-first play uses `arenaV2WarbowFlagSupplementContracts` for planted-flag owner/plant-at (timers JSON omits them) ([#362](https://gitlab.com/PlasticDigits/yieldomega/-/issues/362), parent [#360](https://gitlab.com/PlasticDigits/yieldomega/-/issues/360)) | `ArenaWarbowHeroPanel.test.tsx`, `arenaV2SaleSessionBridge.test.ts`, `warbowClaimFlagState.ts`, `cd frontend && npm run typecheck && npm test` · [arena-views §362](../frontend/arena-views.md#warbow-viewer-summary-flag-gitlab-362) · play skill [`play-time-arena-warbow`](../../skills/play-time-arena-warbow/SKILL.md) |
| **`INV-FRONTEND-361-WARBOW-STEAL-QUOTA-UX`** | Play WarBow viewer summary shows attacker steal usage **`X / 3`** and live **UTC-day reset** countdown; steal card renders **`describeStealPreflight`** inline above Steal when victim is at daily cap (bypass off); caps use one-shot `readWarbowStealCapDisplay` when indexer is on (no recurring display RPC mirrors — [#301](https://gitlab.com/PlasticDigits/yieldomega/-/issues/301)); UTC math matches onchain `SECONDS_PER_DAY` ([#361](https://gitlab.com/PlasticDigits/yieldomega/-/issues/361), parent [#360](https://gitlab.com/PlasticDigits/yieldomega/-/issues/360)) | `warbowUtcDayReset.test.ts`, `ArenaWarbowHeroPanel.test.tsx`, `timeArenaUx.test.ts`, `cd frontend && npm run typecheck && npm test` · [arena-views §361](../frontend/arena-views.md#warbow-steal-quota-ux-gitlab-361) · play skill [`play-time-arena-warbow`](../../skills/play-time-arena-warbow/SKILL.md) |
| **`INV-FRONTEND-343-TRANSITION-UX`** | Play **`/`** derives podium transition UX (`live`, `expired-pending-roll`, `settling`, `epoch-advanced`) from indexer timers/podiums; hero + side chips share **`chainNowSec`** skew; expired timers show settlement copy (not silent `00:00` freeze); optional **`rollPodiumEpoch`** CTA behind **`ChainMismatchWriteBarrier`**; no browser RPC display mirrors ([#343](https://gitlab.com/PlasticDigits/yieldomega/-/issues/343)) | `arenaTransitionState.test.ts`, `arenaChainNow.test.ts`, `indexerFirstDisplay.test.ts`, `cd frontend && npm run typecheck && npm run lint && npm test` · [arena-views §343](../frontend/arena-views.md#play-route-transition-ux-gitlab-343) · [time-arena § epoch roll](../product/time-arena.md) |
| **`INV-FRONTEND-354-PLAY-INDEXER-STATUS`** | Play **`/`** mounts shared **`IndexerStatusBar`** when the global footer is hidden; offline shows **`INDEXER · offline · retrying`** (same copy as AUDIT footer); stale cached display data is not labeled live; banner clears on recovery without blocking read-only play ([#354](https://gitlab.com/PlasticDigits/yieldomega/-/issues/354), [#96](https://gitlab.com/PlasticDigits/yieldomega/-/issues/96)) | `ArenaSimplePage.test.tsx`, `indexerConnectivity.test.ts`, `cd frontend && npm run typecheck && npm run lint && npm test` · [arena-views §354](../frontend/arena-views.md#play-route-indexer-offline-banner-gitlab-354) · [manual QA §96](manual-qa-checklists.md#manual-qa-issue-96) |
...
| **`INV-INDEXER-344-INGEST-LAG`** | **`GET /v1/arena/timers`** and **`GET /v1/arena/podiums`** expose **`indexed_through_block`** (`chain_pointer.block_number` decimal string) alongside head **`read_block_number`**; value never exceeds **`read_block_number`**; schema **≥ 2.19.0** ([#344](https://gitlab.com/PlasticDigits/yieldomega/-/issues/344)) | `api_arena.rs`, `integration_stage2.rs::api_arena_ingest_lag_fields_smoke` · [data-flows § ingest lag](../architecture/data-flows.md#ingest-lag-vs-head-poller-gitlab-344) |
| **`INV-INDEXER-346-INGEST-LOG-ORDER`** | `eth_getLogs` rows sorted by **`(block_number, transaction_index, log_index)`** before decode/persist; **`PodiumTimerArmed`** → **`idx_arena_podium_timer_armed`**; reorg rollback clears timer-arm rows; schema **≥ 2.20.0** ([#346](https://gitlab.com/PlasticDigits/yieldomega/-/issues/346)) | `ingestion.rs`, `decoder.rs`, `persist.rs`, `integration_stage2.rs::last_buy_epoch_order_independent_on_shuffled_logs`, `bash scripts/verify-last-buy-epoch-anvil.sh` · [design §346](../indexer/design.md#ingest-log-order-gitlab-346) |
...
| **`INV-FRONTEND-342-TRANSITION-E2E`** | Gap-analysis [#342](https://gitlab.com/PlasticDigits/yieldomega/-/issues/342) P2 #10: indexer-first Playwright covers timer zero → **`rollPodiumEpoch`** → **`epoch-advanced`** / epoch increment, leaderboard refresh, and claim-ready state after Last Buy epoch end; stable `data-testid` transition hooks ([#343](https://gitlab.com/PlasticDigits/yieldomega/-/issues/343)); no stale epoch after ingest catches up ([#350](https://gitlab.com/PlasticDigits/yieldomega/-/issues/350)) | `e2e/anvil-arena-06-transitions.spec.ts`, `arenaE2eHelpers.ts`; `YIELDOMEGA_E2E_INDEXER=1 bash scripts/e2e-anvil.sh` · spec only: `cd frontend && CI=1 npm run test:e2e -- --workers=5 e2e/anvil-arena-06-transitions.spec.ts` · [e2e-anvil.md §350](e2e-anvil.md) · [arena-views §343](../frontend/arena-views.md#play-route-transition-ux-gitlab-343) |
| **`INV-INDEXER-364-ARENA-EVENTS`** | `GET /v1/arena/events` lists podium settlements + Last Buy epoch starts from **`idx_arena_podium_epoch`** / **`idx_arena_last_buy_epoch_started`**; `GET /v1/arena/events/{id}` returns winners, chart buy sample, tx identity; `limit` clamped ≤200; bad ids → **400**; schema **≥ 2.24.0** ([#364](https://gitlab.com/PlasticDigits/yieldomega/-/issues/364)) | `arena_events.rs`, `integration_stage2.rs::postgres_stage2_persist_all_events_and_rollback_after`, `bash scripts/verify-arena-events-anvil.sh` · [design §364](../indexer/design.md#arena-events-http-gitlab-364) |
| **`INV-FRONTEND-364-ARENA-EVENT-PAGES`** | **`/audit`** top **`ArenaEventDirectorySection`** (filter + search + links); permanent read-only pages at **`/audit/events/{id}`** with title, winners, chart, tx replay, CTAs to **`/`** and **`/audit`**; indexer-only display (**`INV-FRONTEND-301`**); route registered before referral catch-all ([#364](https://gitlab.com/PlasticDigits/yieldomega/-/issues/364)) | `ArenaEventDirectorySection.test.tsx`, `indexerApi.test.ts`, `e2e/anvil-audit-events.spec.ts`, `cd frontend && npm run typecheck && npm run lint && npm test` · [arena-views §364](../frontend/arena-views.md#arena-event-directory-gitlab-364) |

<a id="frontend-post-buy-result-share-gitlab-365"></a>

### Frontend post-buy result share card (GitLab [#365](https://gitlab.com/PlasticDigits/yieldomega/-/issues/365))

| ID | Property | Evidence |
|----|----------|----------|
| **`INV-FRONTEND-365-BUY-RESULT-SHARE`** | (see summary row above) | `arenaBuyShareSummary.test.ts`, `ArenaBuyResultSharePopover.test.tsx`, `mobileBuyResultShareLayout.test.ts`; screenshot bounding-box asserts at 390×844, 768×1024, 1280×800 · [screenshots issue-365](screenshots/issue-365/README.md) |
...
<a id="frontend-warbow-viewer-summary-flag-gitlab-362"></a>

### WarBow viewer summary flag timer (GitLab [#362](https://gitlab.com/PlasticDigits/yieldomega/-/issues/362))

| ID | Property | Evidence |
|----|----------|----------|
| **`INV-FRONTEND-362-WARBOW-VIEWER-SUMMARY-FLAG`** | (see summary row above) | `ArenaWarbowHeroPanel.test.tsx` — `warbow-hero-viewer-summary-flag` during silence and post-silence; line omitted when `showClaimFlagControl` false · [arena-views §362](../frontend/arena-views.md#warbow-viewer-summary-flag-gitlab-362) |

<a id="frontend-warbow-steal-quota-ux-gitlab-361"></a>

### WarBow steal daily-cap UX (GitLab [#361](https://gitlab.com/PlasticDigits/yieldomega/-/issues/361))

| ID | Property | Evidence |
|----|----------|----------|
| **`INV-FRONTEND-361-WARBOW-STEAL-QUOTA-UX`** | (see summary row above) | `warbowUtcDayReset.test.ts`, `ArenaWarbowHeroPanel.test.tsx` — `warbow-hero-viewer-summary-steal-quota`, `warbow-hero-steal-preflight` · [arena-views §361](../frontend/arena-views.md#warbow-steal-quota-ux-gitlab-361) |
...
<a id="frontend-transition-e2e-gitlab-350"></a>

### Frontend transition E2E for #342 gap analysis (GitLab [#350](https://gitlab.com/PlasticDigits/yieldomega/-/issues/350))

Parent gap analysis [#342](https://gitlab.com/PlasticDigits/yieldomega/-/issues/342) P2 #10 — browser regression for indexer + frontend transition wiring (epoch roll visibility, leaderboard refresh, claim state).

| ID | Property | Evidence |
|----|----------|----------|
| **`INV-FRONTEND-342-TRANSITION-E2E`** | (see summary row above) | `YIELDOMEGA_E2E_INDEXER=1 bash scripts/e2e-anvil.sh` · `cd frontend && CI=1 npm run test:e2e -- --workers=5 e2e/anvil-arena-06-transitions.spec.ts` · `bash scripts/verify-podium-live-anvil.sh` (stack smoke) · [e2e-anvil.md §350](e2e-anvil.md) |
...
| **`INV-DOCS-320-PAUSE-OPS`** | Ops docs: **`claimWarBowFlag`**, **`rollPodiumEpoch`**, **`claimCred`**, **`topUpPodiumPools`** gated by **`paused`** via **`_requireLive()`** | [pause-and-final-signoff.md](../operations/pause-and-final-signoff.md), [final-signoff-and-value-movement.md](../operations/final-signoff-and-value-movement.md) ([#349](https://gitlab.com/PlasticDigits/yieldomega/-/issues/349)) |
...
| ID | Property | Automated evidence |
|----|----------|-------------------|
| **`INV-TIME-ARENA-PODIUM-TIMER-TABLE`** | `podiumTimerExtensionSec`, `podiumInitialTimerSec`, `podiumTimerCapSec`, reset bands match product table (+120/+60/+480/+300; 24h/12h/24h/48h initial; caps = 4× initial) | `ArenaPodiumTimerConfig.sol`, `test_start_arena_initial_deadlines_differ_by_category`, `verify-podium-timers-anvil.sh` |
| **`INV-TIME-ARENA-PODIUM-TIMER-SETTER`** | Owner **`setPodiumTimerConfig`** validates via `ArenaPodiumTimerConfig.validateOne`, syncs cat-0 legacy shims, emits **`PodiumTimerConfigUpdated`**; mid-epoch retune applies on next qualifying buy | `TimeArena.t.sol::test_setPodiumTimerConfig_*` |
...
| ID | Property | Automated evidence |
|----|----------|-------------------|
| **`INV-TIME-ARENA-PAUSE-MUTATING`** | Pause blocks all `_requireLive` mutators: `buy`, `buyWithCred`, WarBow steal/guard/revenge, `claimWarBowFlag`, `rollPodiumEpoch`, `claimCred`, `topUpPodiumPools` | `test_pause_blocks_*` ([#349](https://gitlab.com/PlasticDigits/yieldomega/-/issues/349)) |
| **`INV-TIME-ARENA-WARBOW-REVERT-MATRIX`** | Exact onchain revert strings for the 1×–50× steal band, daily cap, revenge expiry, flag holder/silence, double finalize, bad victim/epoch | `test_warbow_*_reverts_*`, `test_finalize_warbow_podium_reverts_*` |
...
| ID | Property | Automated evidence |
|----|----------|-------------------|
| **`INV-TIME-ARENA-WARBOW-DOUB`** | Steal and revenge cost **`epochCharmAnchorWad / 5`**; guard costs **`epochCharmAnchorWad / 2`**; bypass remains fixed **`50_000e18`**. Prices use the post-autoroll epoch anchor—not `effectiveCharmPriceWad()` growth—and all action spends retain the same 100% podium routing plus `totalDoubRaised` accounting. | `test_warbow_*_pulls_doub`, `test_warbow_steal_routes_doub_split` ([#367](https://gitlab.com/PlasticDigits/yieldomega/-/issues/367)) · [parameters](../../contracts/PARAMETERS.md#warbow-doub-costs-252-366-367) · [arena-v2](../product/arena-v2.md#warbow-doub) · [time-arena](../product/time-arena.md#warbow-pvp-doub) |
| **`INV-TIME-ARENA-WARBOW-STEAL-BAND`** | `warbowSteal` accepts attacker BP > 0 only when **`attackerBP ≤ victimBP ≤ 50 × attackerBP`**. Equal BP is valid; a higher-BP attacker cannot target a lower-BP victim. | `test_warbow_steal_*band*`, bot `steal_band_state`, `TestWarBowWorld.test_steal_bp_band_blocks_steal` ([#366](https://gitlab.com/PlasticDigits/yieldomega/-/issues/366)) · [play skill](../../skills/play-time-arena-warbow/SKILL.md) |
| **`INV-TIME-ARENA-WARBOW-REVERT-MATRIX`** | The unchanged out-of-band revert is **`TimeArena: steal band`**; it covers `victimBP < attackerBP` and `victimBP > 50 × attackerBP`, alongside the existing daily-cap, revenge, flag, finalize, victim, and epoch cases. | `test_warbow_*_reverts_*`, `test_finalize_warbow_podium_reverts_*` ([#366](https://gitlab.com/PlasticDigits/yieldomega/-/issues/366)) |
...
| `PodiumTimerArmed` | `ArenaPodiumTimerArmed` | `idx_arena_podium_timer_armed` ([#346](https://gitlab.com/PlasticDigits/yieldomega/-/issues/346)) |
...
<a id="indexer-live-reorg-anvil-gitlab-351"></a>

### Indexer live reorg Anvil smoke (GitLab [#351](https://gitlab.com/PlasticDigits/yieldomega/-/issues/351))

Parent gap analysis: [#342](https://gitlab.com/PlasticDigits/yieldomega/-/issues/342) P2 #11 · bundles **M-I4**.

**INV-INDEXER-351-REORG-ANVIL:** `bash scripts/verify-indexer-reorg-anvil.sh` exercises end-to-end reorg handling beyond synthetic `rollback_after` in `integration_stage2.rs`:

1. Boot Anvil + native Postgres (`:5433`) + indexer via [`verify_indexer_stack.sh`](../../scripts/lib/verify_indexer_stack.sh).
2. Ingest a DOUB **`buy`**, snapshot with **`evm_snapshot`**, ingest orphan-fork **`buyWithCred`** ( **`ArenaLevelUp`** ).
3. **`evm_revert`** to the snapshot and remine empty blocks so the next RPC block’s **`parent_hash`** mismatches the stored **`chain_pointer`**.
4. Assert ingestion logs reorg + rollback; **`idx_arena_level_up`** cleared; exactly one **`idx_arena_buy`**; **`chain_pointer`** block/hash matches RPC head; **`GET /v1/arena/wallet/{address}/stats`** **`level_history`** matches on-chain **`TimeArena.level`**.

Fails loudly (non-zero exit + indexer log tail) when ingest divergence persists after **`YIELDOMEGA_REORG_HEAL_TIMEOUT_SEC`** (default **90**). Resets the app DB on each run — no orphan state for subsequent verifies.

Doc: [`indexer/REORG_STRATEGY.md`](../../indexer/REORG_STRATEGY.md) · [`scripts/qa/README.md`](../../scripts/qa/README.md) · play skill [`skills/script-with-timearena-local/SKILL.md`](../../skills/script-with-timearena-local/SKILL.md).
...
| **Indexer** | Arena + referral decode; per-block tx ([#140](#indexer-transactional-block-ingestion-gitlab-140)); reorg rollback + live Anvil drill ([#351](#indexer-live-reorg-anvil-gitlab-351)) | [`REORG_STRATEGY.md`](../../indexer/REORG_STRATEGY.md), [indexer design](../indexer/design.md) |
...
**INV-FRONTEND-96:** **`reportIndexerFetchAttempt`**: offline after **3** failures; pollers back off **5s → 15s → 30s**; **`IndexerStatusBar`** shows retry state on AUDIT footer routes and play **`/`** ([#354](https://gitlab.com/PlasticDigits/yieldomega/-/issues/354)).
...
**INV-FRONTEND-REFERRAL-BUY-PREFLIGHT:** Pending referral codes in **`yieldomega.ref.v2`** persist across buys. Buy submit resolves **`referralRegistry`** at submit time (multicall latch or direct **`TimeArena.referralRegistry()`** read). **`ownerOfCode`** zero reads retry once; transient zero **does not** clear pending storage. RPC failures during registry / owner reads **abort** the buy. Pending storage clears only for blocked slugs, malformed codes, or confirmed self-referral. [`referralBuyPreflight.test.ts`](../../frontend/src/lib/referralBuyPreflight.test.ts) · [`referralStorage.test.ts`](../../frontend/src/lib/referralStorage.test.ts) · [referrals.md § Browser capture](../product/referrals.md#browser-capture-and-buy-attribution)
