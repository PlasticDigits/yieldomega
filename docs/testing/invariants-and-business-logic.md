...
| **`INV-TIME-ARENA-CRED-POOL-ACCRUE`** | `buyWithCred` adds `CRED_PER_BUY` (35e18) to `epochCredPool[epoch]`; CHARM weight matches DOUB path ([#311](https://gitlab.com/PlasticDigits/yieldomega/-/issues/311)) | `test_buy_with_cred`, `test_cred_accrue_on_doub_buy`, `test_cred_pro_rata_mixed_doub_and_cred_buyers`, `test_buyWithCred_epoch_boundary_credits_correct_pool` |
