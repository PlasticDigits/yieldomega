...
|-------|---------------------------|----------------------------------------|
| **1 — Unit tests** | `unit-tests` workflow succeeds: **Foundry**, **Rust** + Postgres (`integration_stage2`), **Vitest**, **Playwright** (`npx playwright install --with-deps chromium` then `npm run test:e2e`), **Python** simulations. **`slither`** workflow green for contract changesets (high-severity gate). | **Postgres:** unset `YIELDOMEGA_PG_TEST_URL` locally → `integration_stage2` skips DB work but passes; set a URL for full coverage. **Playwright (CI):** install browsers with `npx playwright install chromium` (and `chromium-headless-shell` if your Playwright version defaults to the shell build). **Anvil E2E** (`scripts/e2e-anvil.sh`) is **local** or via the optional **`e2e-anvil`** workflow (`workflow_dispatch`); not a merge gate — chain-touching specs skip unless `ANVIL_E2E=1`. See [e2e-anvil.md](e2e-anvil.md). Gitleaks on every push/PR. |
