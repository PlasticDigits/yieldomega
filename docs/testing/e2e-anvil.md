...
| `VITE_KUMBAYA_TIME_ARENA_BUY_ROUTER` | Optional — ETH pay-mode E2E; must match onchain `timeArenaBuyRouter` ([#264](https://gitlab.com/PlasticDigits/yieldomega/-/issues/264)). `e2e-anvil.sh` also sets legacy `VITE_KUMBAYA_TIMECURVE_BUY_ROUTER` when fixtures run. |
