...
| **Indexer-first E2E** | Spawned by script; URL inlined at build | Live indexer status bar + protocol podiums from `GET /v1/arena/*` | `YIELDOMEGA_E2E_WITH_INDEXER=1 bash scripts/e2e-anvil.sh` (adds `anvil-indexer-first.spec.ts`; needs native Postgres on `:5433` or Docker `yieldomega-pg`) |
...
```bash
YIELDOMEGA_E2E_WITH_INDEXER=1 bash scripts/e2e-anvil.sh
```

Runs the default suite plus [`anvil-indexer-first.spec.ts`](../../frontend/e2e/anvil-indexer-first.spec.ts) (live indexer status + protocol podiums). Requires host Postgres ([`bootstrap-cloud-postgres-native.sh`](../../scripts/bootstrap-cloud-postgres-native.sh) on Cloud VMs) or Docker `yieldomega-pg`. Override indexer port with `INDEXER_PORT` (default **3102** for E2E).
