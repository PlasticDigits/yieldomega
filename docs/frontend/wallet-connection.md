...
**Shared submit guard ([GitLab #224](https://gitlab.com/PlasticDigits/yieldomega/-/issues/224)):** Arena **`submitBuy`** (accepted-asset / DOUB spend), WarBow DOUB pulls, and protocol donate pools call [`ensureDoubTimeArenaAllowance`](../../frontend/src/lib/ensureDoubTimeArenaAllowance.ts) with **`unlimitedPreferred`** from [`readArenaDoubUnlimitedApproval`](../../frontend/src/lib/arenaDoubApprovalPreference.ts). Kumbaya / legacy CL8Y-token paths use [`ensureCl8yKumbayaAllowance`](../../frontend/src/lib/ensureCl8yKumbayaAllowance.ts). **`approve`** runs only when **`allowance < arenaDoubApprovalAmountWei(need, pref)`** (exact + 50 bps headroom, or **`maxUint256`** when opted in).
