...
**Shared submit guard ([GitLab #224](https://gitlab.com/PlasticDigits/yieldomega/-/issues/224)):** Simple **`submitBuy`**, Arena **`handleBuy`**, and WarBow CL8Y pulls call [`ensureCl8yKumbayaAllowance`](../../frontend/src/lib/ensureCl8yKumbayaAllowance.ts) — **`approve`** is submitted only when **`allowance < arenaDoubApprovalAmountWei(need, pref)`**, so wallets with existing **`type(uint256).max`** (or sufficient exact allowance) are not prompted again.
