...
**`/timecurve` (Simple)** hides the global footer ([`RootLayout`](../../frontend/src/layout/RootLayout.tsx)); the same **`IndexerStatusBar`** is rendered above **Recent buys**. **Site-wide outbound links** (social, GitLab/GitHub, Kumbaya CL8Y swap, CL8Y bridge, **Agent SKILL.md**) live in standalone **`FooterSiteLinksCard`** at the bottom of the Simple page — parity with global footer routes ([GitLab #232](https://gitlab.com/PlasticDigits/yieldomega/-/issues/232), [`INV-FRONTEND-232-SIMPLE-PARITY`](../testing/invariants-and-business-logic.md#global-footer-site-link-ribbon-gitlab-232)). **Recent buys** empty copy: **Waiting for the first buy of this round** only when the last buys poll **succeeded** with zero rows **and** connectivity is not offline; otherwise prefer **Cannot reach indexer · cached data may be stale** (and a stale hint above the list when cached rows exist).
...
3. **Guard + revenge are obvious:** guard is a visible hero CTA with spend and
   ...
   **Per-row BP context + live countdown ([GitLab #236](https://gitlab.com/PlasticDigits/yieldomega/-/issues/236)):** the hero revenge list drops the redundant "You have N open counter-hits. Earliest expiry …" summary paragraph and the per-row UTC `UnixTimestampDisplay`. Each row instead surfaces (a) **Target BP** — live `battlePoints(stealer)` from a batched `useReadContracts` keyed off `pendingRevengeTargets` (loading hint while pending); (b) **Gain** — `warbowRevengeDrainBp(stealerBp)` mirroring onchain `_warbowStealDrainBp` (10% drain via `WARBOW_STEAL_DRAIN_BPS = 1000`); zero-drain rows show a muted "Revenge zero (no BP to drain)" hint instead of a misleading `+0 BP`; and (c) **Valid To: HH:MM:SS** — live countdown computed from `expiry_exclusive - ledgerNowSec` (same interpolated chain "now" as guard / phase, `effectiveLedgerSec`), clamped to `00:00:00` once the row slips past expiry (the 8s indexer poll then drops the row). Drain math helper: [`warbowRevengeRowPreview.ts`](../../frontend/src/lib/warbowRevengeRowPreview.ts).
...
### Global footer — site link ribbon ([issue #232](https://gitlab.com/PlasticDigits/yieldomega/-/issues/232))

[`FooterSiteLinksCard`](../../frontend/src/components/FooterSiteLinksCard.tsx) renders on every route that shows **`.app-footer`**, and on **`/timecurve`** at the page bottom: a **`data-panel`** card **below** the agent card (not nested inside it). Canonical hrefs and icon mapping live in [`footerSiteLinks.ts`](../../frontend/src/lib/footerSiteLinks.ts). **`Agent SKILL.md`** points at raw GitHub Markdown for [`skills/play-active-timecurve/SKILL.md`](../../skills/play-active-timecurve/SKILL.md) (phase router for agents — pre-open / live / post-end). **Manual QA:** [`../testing/manual-qa-checklists.md#manual-qa-issue-232`](../testing/manual-qa-checklists.md#manual-qa-issue-232). **Invariants:** [`INV-FRONTEND-232-*`](../testing/invariants-and-business-logic.md#global-footer-site-link-ribbon-gitlab-232).
