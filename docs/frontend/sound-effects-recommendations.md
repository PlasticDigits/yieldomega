...
**Shipped behavior** (see [GitLab #68](https://gitlab.com/PlasticDigits/yieldomega/-/issues/68), [GitLab #71](https://gitlab.com/PlasticDigits/yieldomega/-/issues/71) — BGM resume, [invariants — frontend audio](../testing/invariants-and-business-logic.md#timecurve-frontend-album-1-bgm-and-sfx-bus-issue-68)):
...
- **TimeCurve Simple + Arena buy:** `coin_hit_shallow` immediately after the wallet returns a **`tx` hash** for **`TimeCurve.buy`** or **`buyViaKumbaya`** (before receipt); `charmed_confirm` after **successful receipt** · **TimeCurve Arena:** `warbow_twang` (throttled ~**18 s** in `WebAudioMixer`) on **indexed‑ladder podium** moments only — **`shouldPlayWarbowRankStinger`** ([`warbowRankSfxPolicy.ts`](../../frontend/src/audio/warbowRankSfxPolicy.ts)): **enter top 3** from unranked/deeper **or climb within top 3** (**not** shallow rank moves like **10 → 4**). **`kumbaya_whoosh`** is **reserved** for ETH/USDM/CL8Y pay‑mode / route deltas — **not** played on quote refetch until explicitly wired ([#68 discussion](https://gitlab.com/PlasticDigits/yieldomega/-/issues/68); inventory [#108](https://gitlab.com/PlasticDigits/yieldomega/-/issues/108)).
...
- **Mobile dock vs nav:** **`INV-AUDIO-103`** ([GitLab #103](https://gitlab.com/PlasticDigits/yieldomega/-/work_items/103)) — on **`max-width: 720px`**, **`RootLayout`** **`.app-header`** uses extra **`margin-top`** so the bordered menu card clears the fixed **`AlbumPlayerBar`** bubble; tablet/desktop unchanged. Token + Vitest: [`mobileAlbumDockLayout.ts`](../../frontend/src/audio/mobileAlbumDockLayout.ts); map: [invariants §103](../testing/invariants-and-business-logic.md#mobile-album-dock-layout-issue-103).

**Code map:** `frontend/src/audio/` (`WebAudioMixer`, `audioPlaybackState.ts`, `AudioEngineProvider`, `AlbumPlayerBar`, `playGameSfx.ts`, `warbowRankSfxPolicy.ts`), `frontend/src/layout/RootLayout.tsx` (player chrome), `useTimeCurveSaleSession.ts` + `lib/timeCurveKumbayaSingleTx.ts` (buy submit SFX), `useTimeCurveArenaModel.tsx`, `useArenaWarbowRankSfx.ts` (Arena WarBow stinger), `useTimeCurveSimplePageSfx.ts` (peer + timer).
