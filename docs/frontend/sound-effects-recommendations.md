...
This document proposes **in-game and UI** sound for the frontend. It is aligned with **`frontend/public/music/albums/glass_arena`** (full **Glass Arena** album: sixteen instrumental tracks; see `manifest.json` there) and the onchain game loop (**TimeArena** buys, **CHARM** / **DOUB**, **WarBow**, **podium prizes**).
...
**Shipped behavior** (see [GitLab #68](https://gitlab.com/PlasticDigits/yieldomega/-/issues/68), [GitLab #71](https://gitlab.com/PlasticDigits/yieldomega/-/issues/71) — BGM resume, invariants — frontend audio):

- **Web Audio graph:** `master` gain → destination; **`bgmGain`** (sequential MP3 **Glass Arena**, tracks **1–16** from `manifest.json`) and **`sfxGain`** (decoded `.wav` one-shots) are **independent** buses into `master`.
...
**Code map:** `frontend/src/audio/` (`WebAudioMixer`, `audioPlaybackState.ts`, `AudioEngineProvider`, `AlbumPlayerBar`, `playGameSfx.ts`, `warbowRankSfxPolicy.ts`), `frontend/src/layout/RootLayout.tsx` (player chrome), `useArenaSaleSession.ts` + `lib/timeArenaKumbayaSingleTx.ts` (buy submit SFX), `useArenaModel.tsx`, `useArenaWarbowRankSfx.ts` (Arena WarBow stinger), `useArenaSimplePageSfx.ts` (peer + timer).
