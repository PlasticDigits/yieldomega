...
**Code map:** `frontend/src/audio/` (`WebAudioMixer`, `audioPlaybackState.ts`, `AudioEngineProvider`, `AlbumPlayerBar`), `frontend/src/layout/RootLayout.tsx` (player chrome), `useTimeCurveSaleSession.ts` (buy/redeem + pay-mode SFX), `useTimeCurveSimplePageSfx.ts` (peer + timer).
