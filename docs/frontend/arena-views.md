# Arena frontend (play `/` · AUDIT `/arena/protocol`)
...
**Redirects:** bare **`/arena`** and **`/timecurve`** → **`/`**; **`/timecurve/protocol`** → **`/arena/protocol`**; referral segments **`/arena/:code`** preserved — [`LaunchGate.tsx`](../../src/app/LaunchGate.tsx). **Do not** rename onchain revert substrings in [`revertMessage.ts`](../../src/lib/revertMessage.ts).
...
Global shell/design direction: [frontend design §290](./design.md#cyberminimalist-glass-app-shell-gitlab-290). Route-level copy stays compact: header **AUDIT** pill links to **`/arena/protocol`**; mechanics live in tooltips, state rows, and action-adjacent feedback rather than default explanatory paragraphs.
...
- **Timer podium carousel:** `ArenaTimerPodiumCarousel` cycles the four podium timer slides in the primary column.
