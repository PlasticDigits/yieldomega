...
Nodes **must** reject creations that exceed these bounds. **Ethereum L1** and **default Anvil** (no flag) still enforce **EIP-170** on the runtime code of each created contract: **0x6000** bytes = **24,576** (~24 KiB). **This repository** starts Anvil with **`--code-size-limit 524288`** (512 KiB = **0x80000**; Foundry’s flag is **decimal-only**—`0x80000` is rejected) in [`scripts/start-local-anvil-stack.sh`](../../scripts/start-local-anvil-stack.sh), `scripts/e2e-anvil.sh`, and `scripts/anvil-export-bot-env.sh` so local deploys are not incorrectly capped at EIP-170. The local full-stack launcher also passes **`--gas-limit ${YIELDOMEGA_ANVIL_GAS_LIMIT:-60000000}`** because the current `DeployDev` broadcast can exceed Anvil's **30M** default block gas limit while still being a local dev convenience script, not a MegaEVM protocol limit. **`forge script` applies the same EIP-170 limit during its pre-broadcast simulation** independently of the RPC node; repo shell entrypoints pass **`--code-size-limit 524288`** on `forge script` as well (e.g. [`start-local-anvil-stack.sh`](../../scripts/start-local-anvil-stack.sh), [`scripts/lib/anvil_deploy_dev.sh`](../../scripts/lib/anvil_deploy_dev.sh), [`contracts/script/anvil_rich_state.sh`](../../contracts/script/anvil_rich_state.sh)). If you run a **plain** `anvil` or another node without that flag, a deploy that fits MegaETH can still **fail** with a code-size error—set the limit to **524288** on **both** Anvil and `forge script` (or `anvil --disable-code-size-limit` / `forge script --disable-code-size-limit` where appropriate) or split/shrink for L1-style tooling.

**How to verify in this repo:** from the repo root, run `bash scripts/check-megaevm-contract-sizes.sh` (or `cd contracts && FOUNDRY_PROFILE=ci forge build` then inspect artifacts). CI uses this script instead of `forge build --sizes`, because Foundry’s `--sizes` gate still enforces **EIP-170** / **EIP-3860** even when `code_size_limit = 524288` is set in [`foundry.toml`](../../contracts/foundry.toml) for test/script simulation. Optional human-readable table: `cd contracts && forge build --sizes` (expect EIP-170 failure on `TimeArena`; margins are informational only).
...
- Fuzz timer and ordering edge cases for **TimeArena**; property tests for treasury invariants where possible (see `RetiredV1ReserveMath` in [`contracts/README.md`](../../contracts/README.md) and Python sims in [`simulations/README.md`](../../simulations/README.md)).
