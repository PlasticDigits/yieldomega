...
- **First buy ever** (DOUB or CRED, per wallet): schedules **`FIRST_BUY_CRED_BONUS = 1100e18`** (1100 CRED) in **`epochFixedCredBonus[lastBuyEpoch + 1]`** — onboarding pairs with **`ONBOARDING_STARTER_CHARM_WAD = 10e18`** ([#268](https://gitlab.com/PlasticDigits/yieldomega/-/issues/268), [#299](https://gitlab.com/PlasticDigits/yieldomega/-/issues/299)).
...
- **Player level cap 5** ([#299](https://gitlab.com/PlasticDigits/yieldomega/-/issues/299)): `MAX_PLAYER_LEVEL = 5` in [`ArenaXp`](../../contracts/src/arena/libraries/ArenaXp.sol); surplus XP at max level is discarded. Cached **`level`** + **`xpTowardNext`** on buy path ([#265](https://gitlab.com/PlasticDigits/yieldomega/-/issues/265)). Timer / epoch rolls **do not** reset XP ([#250](https://gitlab.com/PlasticDigits/yieldomega/-/issues/250)).
...
- AUDIT console: **`/arena/protocol`**.
- Legacy **`/arena`** (no segment) and **`/timecurve`** → **`/`**; **`/timecurve/protocol`** → **`/arena/protocol`**; referral segments **`/arena/:code`** and **`/timecurve/:code`** preserved ([#266](https://gitlab.com/PlasticDigits/yieldomega/-/issues/266)).
- Header nav: brand/home → **`/`**; **AUDIT** pill → **`/arena/protocol`** (no `ArenaSubnav` BUY/AUDIT sub-row on the play surface).
