...
**Buy energy ([#332](https://gitlab.com/PlasticDigits/yieldomega/-/issues/332)):** per wallet, one charge accrues every **300s**, charges cap at **5**, and charged buys require a **15s** burst gap. `buyEnergyState(wallet)` is the canonical UX read; `nextBuyAllowedAt(wallet)` is computed from that state. Exhausted wallets revert **`TimeArena: no buy charges`**; burst-gap buys revert **`TimeArena: burst cooldown`**. Long-run pacing remains one buy per 5 minutes per wallet.
