...
- **WarBow refresh candidates (GitLab #160):** `GET /v1/timecurve/warbow/refresh-candidates` (schema **≥ 1.15.0**) serves a deduped **`candidates`** list for **`refreshWarbowPodium`** calldata **during the sale** — see **`INV-INDEXER-160-WARBOW-REFRESH-CANDIDATES`** in [invariants §149](../testing/invariants-and-business-logic.md#gitlab-149-warbow-arena-indexer-hardening).
