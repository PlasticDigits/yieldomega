...
- <a id="arena-platform-usage-http-gitlab-231"></a>**Platform usage (`GET /v1/arena/platform-usage`, schema ≥ 2.12.0, [GitLab #231](https://gitlab.com/PlasticDigits/yieldomega/-/issues/231), [#319](https://gitlab.com/PlasticDigits/yieldomega/-/issues/319)):** Network-wide sale + WarBow aggregates from **`idx_arena_buy`** (partial index **`idx_arena_buy_block_timestamp`**) plus **`idx_arena_warbow_steal`** / **`idx_arena_warbow_guard`** for WarBow **`doub_spent`** totals and buy **velocity** windows (`1h` / `24h` / `sale`) anchored on the head poller **`block_timestamp_sec`** / **`arena_start_sec`**. Wallet leaderboard paginates with `limit`/`offset`. Legacy **`GET /v1/timecurve/platform-usage`** is not served.
