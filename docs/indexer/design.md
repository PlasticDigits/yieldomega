...
- **timecurve_buys** — buyer, amount (spend), tokens minted or credited if emitted, block, tx index for ordering. **`TimeCurve.buy` and `TimeCurve.buyFor`** share the same canonical **`Buy`** log on the **TimeCurve** contract (indexed `buyer` is always the participant). When **`TimeCurveBuyRouter`** is in the address registry, **`BuyViaKumbaya`** logs are indexed in **`idx_timecurve_buy_router_kumbaya`** and joined into **`/v1/arena/buys`** as optional **`entry_pay_asset`** / **`router_attested_gross_cl8y`** ([issue #67](https://gitlab.com/PlasticDigits/yieldomega/-/issues/67), [`kumbaya.md`](../integrations/kumbaya.md#issue-65-single-tx-router)).
...
HTTP (schema **≥ 2.5.0**): **`GET /v1/arena/timers`** (Last Buy + four podium deadlines + head **`last_buy_epoch`** / **`podium_epochs`**), **`GET /v1/arena/podiums`** (UX-ordered live leaders from **`idx_arena_podium_live`** + WarBow scores; **`podium_prediction: true`** when DB-derived — [#273](https://gitlab.com/PlasticDigits/yieldomega/-/issues/273)), **`GET /v1/arena/buys`**, **`GET /v1/arena/wallet/{address}/stats`**. Replaces legacy **`/v1/timecurve/*`** for Arena reads ([#266](https://gitlab.com/PlasticDigits/yieldomega/-/issues/266)). Map: **`INV-INDEXER-254-ARENA-SCHEMA`**, **`INV-INDEXER-PODIUM-PREDICT-LIVE`** · play skills [`skills/play-active-time-arena`](../../skills/play-active-time-arena/SKILL.md).
