...
Idle **`peak_calls_10s` ≤ 50** operator target is met (**20**). Catch-up and active-arena bursts (**100** / 10s) come from ingestion (`eth_getBlockByNumber` + `eth_getLogs`), not chain-timer fan-out. Artifact: `docs/indexer/benchmarks/rpc-benchmark-20260615T021945Z.json`.

### Post–ingest Multicall3 batching (120s scenarios, [#356](https://gitlab.com/PlasticDigits/yieldomega/-/issues/356))

`BENCHMARK_SCENARIO_SEC=120 bash scripts/benchmark-indexer-rpc-anvil.sh` after per-block ingest batching (`docs/indexer/benchmarks/rpc-benchmark-20260628T055944Z.json`):

| Scenario | calls/min (1m) | `eth_call:warbow_score` + `eth_call:podium_live` (cumulative) | `eth_call:ingestion` (cumulative) |
|----------|----------------|----------------------------------------------------------------|-----------------------------------|
| active arena | **181** | **0** (was **297** in `20260615T021945Z`) | **33** |

Ingest warbow + live-podium reads collapse into **`eth_call:ingestion`** Multicall3 batches (**~100%** reduction in legacy `warbow_score` / `podium_live` caller counts on the same harness).

## Prioritized mitigation strategies

| Strategy | Est. RPC reduction | Status / reactivity |
|----------|-------------------|---------------------|
| **Ingestion-side RPC coalescing** — per-block Multicall3 for warbow + live podium ([#356](https://gitlab.com/PlasticDigits/yieldomega/-/issues/356)) | **~80%+** fewer `eth_call:warbow_score` + `eth_call:podium_live` on active arena | **Shipped** — [`arena_ingest_rpc.rs`](../../indexer/src/arena_ingest_rpc.rs) · one `aggregate3` per block at log block tag |
...
| **Derive live podium snapshots from logs** where `INV-INDEXER-PODIUM-PREDICT-LIVE` allows | Superseded for RPC load by per-block ingest Multicall3 ([#356](https://gitlab.com/PlasticDigits/yieldomega/-/issues/356)) | Block-tagged `podium()` parity preserved via batch path; test via `verify-podium-live-anvil.sh` |
...
| **Ingestion-side RPC coalescing** — batch `eth_getBlockByNumber` during catch-up | **~40–60%** peak reduction on catch-up bursts | Slightly higher indexer lag during burst; monitor `last_indexed_at_ms` ([#168](https://gitlab.com/PlasticDigits/yieldomega/-/issues/168)) |
