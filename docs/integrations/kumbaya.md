...
**ERC-20 `approve` sizing ([GitLab #143](https://gitlab.com/PlasticDigits/yieldomega/-/issues/143)):** **WETH** / **USDM → `swapRouter`** (two-step) and **USDM → `TimeCurveBuyRouter`** (single-tx, when a separate `approve` is needed) use the quoted **`maxIn`** for that leg — not unlimited allowance. **CL8Y → `TimeCurve`** defaults to **exact** gross per tx with an optional UI opt-in for unlimited — [wallet-connection §143](../frontend/wallet-connection.md#erc20-approval-sizing-h-01-gitlab-143) · [`INV-ERC20-APPROVAL-143`](../testing/invariants-and-business-logic.md#frontend-erc20-approval-sizing-gitlab-143).
