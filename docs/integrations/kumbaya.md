...
**`doubOwedForBuy` preview mutability ([GitLab #315](https://gitlab.com/PlasticDigits/yieldomega/-/issues/315)):** [`TimeArena.doubOwedForBuy(charmWad)`](../../contracts/src/arena/TimeArena.sol) is **`view`** — safe for **`eth_call` / staticcall**. Use it (not `effectiveCharmPriceWad()` alone) when Last Buy is in the **hard-reset band** (`remaining < podiumResetBelowRemainingSec[0]`): the preview samples the same TWAP/spot anchor that `_reanchorEpochCharmPrice` applies **before** the buy pulls DOUB. Outside that band, `doubOwedForBuy` equals `charmWad × effectiveCharmPriceWad() / 1e18`. **`TimeArenaBuyRouter.buyViaKumbaya`** calls `doubOwedForBuy` for gross **`exactOutput`** sizing. Frontend direct-DOUB and indexer-first paths use **`effectiveCharmPriceWad`** at head (indexer `charm_price_wad`); Kumbaya router txs rely on onchain `doubOwedForBuy` at inclusion. **Manipulation note:** TWAP anchor is read at tx time; a same-block sandwich cannot make the executed buy cheaper than `doubOwedForBuy` previewed in the same block. Forge: `TimeArenaEpochCharmPrice.t.sol::test_doubOwedForBuy_matches_buy_*`. Invariant: **`INV-TIME-ARENA-DOUB-OWED-PREVIEW`** · [invariants §315](../testing/invariants-and-business-logic.md#doubowedforbuy-preview-gitlab-315).
