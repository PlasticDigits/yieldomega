...
**`doubOwedForBuy` preview vs `effectiveCharmPriceWad` ([issue #315](https://gitlab.com/PlasticDigits/yieldomega/-/issues/315)):** **`doubOwedForBuy(charmWad)`** is **`view`** / **`eth_call`**-safe. Within an epoch it equals `charmWad × effectiveCharmPriceWad() / 1e18`. In the Last Buy **hard-reset band** (`remaining < 780s`), it samples the same Kumbaya TWAP (MegaETH **4326**) or Anvil spot path as the onchain re-anchor **before** any state write — so it matches the DOUB pulled by an immediate **`buy`** / **`buyFor`**. **`effectiveCharmPriceWad()`** alone still reflects the **prior** epoch anchor until a buy executes. **`TimeArenaBuyRouter.buyViaKumbaya`** calls **`doubOwedForBuy`** for gross **`exactOutput`** sizing. **Manipulation note:** TWAP uses a **15-minute** observation window ([#303](https://gitlab.com/PlasticDigits/yieldomega/-/issues/303)); same-block spot moves cannot underpay relative to the sampled anchor in the same transaction because pricing is fixed at tx entry. Invariant: **`INV-TIME-ARENA-DOUB-OWED-PREVIEW`** · [invariants §315](../testing/invariants-and-business-logic.md#timearena-doub-owed-preview-gitlab-315).
...
- **Pools must exist** for each pay path at the configured **fee tiers** (`VITE_KUMBAYA_FEE_*` or defaults): **CL8Y → DOUB** (router **`PAY_CL8Y`**), **WETH → DOUB** / **stable → WETH → DOUB** (ETH/USDM modes), and **DOUB/CL8Y** + **CL8Y/WETH** for production **charm TWAP** ([#303](https://gitlab.com/PlasticDigits/yieldomega/-/issues/303)). Missing pools → failed quotes / swaps — **not** a protocol bug.
- **MegaETH mainnet (4326) fee tiers:** **DOUB/CL8Y** and **CL8Y/WETH** are **0.01% (100)**; **USDm/WETH** is **0.3% (3000)**. `kumbayaRoutes.ts` **`cl8yWethFee`** matches **CL8Y/WETH**. **ETH and USDM** buy-router quotes use `quoteExactOutputSingle` hops only — production QuoterV2 `quoteExactOutput(bytes)` reverts (`toAddress_outOfBounds`). Submit paths re-read `currentCharmPriceWad` for gross DOUB and apply a small headroom BPS before quoting `maxIn`.

**Arena production charm price (TWAP, not spot — [#303](https://gitlab.com/PlasticDigits/yieldomega/-/issues/303)):** [`DeployProduction.s.sol`](../../contracts/script/DeployProduction.s.sol) sets initial **`TimeArena.charmPriceWad`** from **V3 `observe` TWAP** (Sir **15-minute** window) on **DOUB/CL8Y (100)** → **CL8Y/WETH (100)** → **WETH/USDm (3000)** — **not** direct **DOUB/WETH** and **not** QuoterV2 spot. Formula: `charmPriceWad = floor(1e36 / doubUsdTwap)` so ~**$1** DOUB buys **1 CHARM**; implied spend band **`0.99×`–`10× charmPriceWad`**. Override: **`ARENA_CHARM_PRICE_WAD`**. Ops dry-run: `bash scripts/compute-arena-charm-price-twap.sh`. Library: [`ArenaCharmPriceTwap.sol`](../../contracts/src/oracle/ArenaCharmPriceTwap.sol). Invariant: **`INV-TIME-ARENA-CHARM-TWAP-INIT`** · [invariants §303](../testing/invariants-and-business-logic.md#arena-charm-twap-init-gitlab-303).
...
| **Single-tx gross DOUB** | `TimeArenaBuyRouter` sets **`exactOutput` `amountOut`** from **`TimeArena.doubOwedForBuy(charmWad)`** (`view` — includes hard-reset re-anchor preview ([#315](https://gitlab.com/PlasticDigits/yieldomega/-/issues/315))). |
