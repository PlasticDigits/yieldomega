...
**Arena production charm price (TWAP, not spot — [#303](https://gitlab.com/PlasticDigits/yieldomega/-/issues/303)):** [`DeployProduction.s.sol`](../../contracts/script/DeployProduction.s.sol) sets initial **`TimeArena.charmPriceWad`** from **V3 `observe` TWAP** (Sir **15-minute** window) on the same **DOUB/WETH** and **WETH/USDm** pools — **not** QuoterV2 spot. Formula: `charmPriceWad = floor(1e36 / doubUsdTwap)` so ~**$1** DOUB buys **1 CHARM**; implied spend band **`0.99×`–`10× charmPriceWad`**. Override: **`ARENA_CHARM_PRICE_WAD`**. Ops dry-run: `bash scripts/compute-arena-charm-price-twap.sh`. Library: [`ArenaCharmPriceTwap.sol`](../../contracts/src/oracle/ArenaCharmPriceTwap.sol). Invariant: **`INV-TIME-ARENA-CHARM-TWAP-INIT`**.
