...
- [ ] **C7** — `/timecurve` lands on the **Simple** view: state badge + hero countdown + single buy card (with live rate board) + ticker are visible **above the fold**, while WarBow / podiums / battle feed / raw accordion are **not** visible (they live on Arena / Protocol). The hero countdown renders through the shared `TimeCurveTimerHero` (scene-art backplate, gold `Nd` days chip + tabular `HH:MM:SS` digits, animated rising sparks, urgency-aware glow) — visually a sibling of `LaunchCountdownPage`. Confirm digits stay column-stable across per-second updates and the days chip only appears when `secondsRemaining ≥ 86400`. Cross-page navigation is provided only by the persistent `TimeCurveSubnav` at the top of the route — the simple view no longer renders an in-page "Want more?" tile row. The **global app footer** (indexer status pill + canonical fee sinks panel) is **hidden on `/timecurve` only** so the Simple page stays minimal; navigate to `/timecurve/arena` and `/timecurve/protocol` (and `/`) and confirm the footer reappears unchanged on those routes. Mobile (390×844) layout collapses cleanly (timer + buy panels stack via container query on `.timecurve-simple-page` + `@media` fallback). See [`docs/frontend/timecurve-views.md`](../frontend/timecurve-views.md).
...
- [ ] **C11** — **First-time “I want DOUB” heuristic (issue #40, Recommendations §6):** As a disconnected visitor, read top-to-bottom once: you should see the **TimeCurve subnav** (Simple / Arena / Protocol) → **Time left** + **Buy CHARM** in the side-by-side hub (rate board front and centre, “Connect a wallet…” CTA) → **TimeCurve sale** hero (action-led lede + chain-time deadline) → **Recent buys** (social proof). You should *not* need WarBow or podiums to understand “buy CHARM now, redeem DOUB later”, and you should *not* see a redundant in-page Arena / Protocol tile row (the subnav handles that). After connecting, the primary action should remain **Buy CHARM** with an explicit ≈CHARM preview before submit.
