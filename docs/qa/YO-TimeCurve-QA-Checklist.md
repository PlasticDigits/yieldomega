...
- [ ] **C7** — `/timecurve` lands on the **Simple** view: state badge + hero countdown + single buy card (with live rate board) + ticker are visible **above the fold**, while WarBow / podiums / battle feed / raw accordion are **not** visible (they live on Arena / Protocol). Cross-page navigation is provided only by the persistent `TimeCurveSubnav` at the top of the route — the simple view no longer renders an in-page "Want more?" tile row. The **global app footer** (indexer status pill + canonical fee sinks panel) is **hidden on `/timecurve` only** so the Simple page stays minimal; navigate to `/timecurve/arena` and `/timecurve/protocol` (and `/`) and confirm the footer reappears unchanged on those routes. Mobile (390×844) layout collapses cleanly. See [`docs/frontend/timecurve-views.md`](../frontend/timecurve-views.md).
