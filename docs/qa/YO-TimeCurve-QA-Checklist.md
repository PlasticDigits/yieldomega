...
- [ ] **C7** — `/timecurve` lands on the **Simple** view: state badge + hero countdown + single buy card (with live rate board) + ticker are visible **above the fold**, while WarBow / podiums / battle feed / raw accordion are **not** visible (they live on Arena / Protocol). Cross-page navigation is provided only by the persistent `TimeCurveSubnav` at the top of the route — the simple view no longer renders an in-page "Want more?" tile row. Mobile (390×844) layout collapses cleanly. See [`docs/frontend/timecurve-views.md`](../frontend/timecurve-views.md).
