...
cd bots/timearena && .venv/bin/timearena-bot --allow-anvil-funding swarm  # legacy CLI binary name for `bots/timearena`
```
...
- [ ] **`bots/timearena`** deps installed (venv + `pip install -e ".[dev]"`, or PEP 668 fallback in [`bots/timearena/README.md`](../../bots/timearena/README.md)); legacy CLI **`timearena-bot --help`** (packaged from `bots/timearena`) and `import web3` succeed.
...
- [ ] Swarm has run with Anvil funding: either the stack started it (`SKIP_ANVIL_RICH_STATE=1` path) **or** you ran **`timearena-bot --allow-anvil-funding swarm`** (legacy binary name) manually **or** you intentionally skipped swarm (`START_BOT_SWARM=0`) and documented why.
...
- [ ] **A5** — `cd bots/timearena && python3 -m venv .venv && .venv/bin/pip install -e ".[dev]"` — legacy **`timearena-bot`** / `import web3` works.
