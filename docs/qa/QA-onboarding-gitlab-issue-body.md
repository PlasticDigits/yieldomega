...
## TimeCurve QA checklist (copied from repository)

_Source: [`docs/qa/YO-TimeCurve-QA-Checklist.md`](https://gitlab.com/PlasticDigits/yieldomega/-/blob/main/docs/qa/YO-TimeCurve-QA-Checklist.md) on `main` — keep in sync when the file changes._
...
Use the in-repo file as the checklist for QA; mirror updates to ecosystem-qa when that repo is synced.
...
**Deployment:** finalize addresses in [`contracts/PARAMETERS.md`](https://gitlab.com/PlasticDigits/yieldomega/-/blob/main/contracts/PARAMETERS.md); verify governance roles (admin, params, pauser) per deployment runbook; **FeeRouter** weights and destinations match [fee-routing doc](https://gitlab.com/PlasticDigits/yieldomega/-/blob/main/docs/onchain/fee-routing-and-governance.md).
