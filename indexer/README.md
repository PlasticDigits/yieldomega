...
Schema **≥ 2.18.0** (`x-schema-version`) adds **`polled_at_ms`** — unix millis when the head poller last refreshed the snapshot (same instant as internal `ChainTimerSnapshot.polled_at_ms`). Schema **≥ 2.19.0** adds **`indexed_through_block`** — latest block fully ingested (`chain_pointer.block_number`); compare to **`read_block_number`** to detect head-poller vs ingest lag ([#344](https://gitlab.com/PlasticDigits/yieldomega/-/issues/344)). Frontend hero countdown and legacy sale-state mappers prefer **`polled_at_ms`** over client `Date.now()` ([#301](https://gitlab.com/PlasticDigits/yieldomega/-/issues/301)). Idle poll spacing keeps `polled_at_ms` within **3s** of wall clock when RPC is healthy (**`INV-INDEXER-308-TIMER-FRESHNESS`**). **503** `{ "error": "chain_timer_unavailable" }` when the poller snapshot is unset. Map: [design — arena timers](../docs/indexer/design.md#arena-timers-http-gitlab-216) · **`INV-INDEXER-308-TIMER-FRESHNESS`**, **`INV-INDEXER-344-INGEST-LAG`**.

### Ingest lag visibility (schema ≥ 2.19.0, GitLab [#344](https://gitlab.com/PlasticDigits/yieldomega/-/issues/344))

**`GET /v1/arena/timers`** and **`GET /v1/arena/podiums`** include **`indexed_through_block`** (decimal string from Postgres **`chain_pointer`**) alongside head **`read_block_number`**. When ingest trails the chain-timer poller, **`indexed_through_block` < `read_block_number`** — epoch/deadline head fields may reflect RPC before **`PodiumEpochRolled`** / live podium rows catch up. Map: [data-flows — ingest lag](../docs/architecture/data-flows.md#ingest-lag-vs-head-poller-gitlab-344) · **`INV-INDEXER-344-INGEST-LAG`** · `integration_stage2.rs::api_arena_ingest_lag_fields_smoke`.
